...
!_________________________________________________________________________
  ...
  !real(dp),parameter::SED_dlgA=0.005408757523d0
  real(dp),parameter::SED_dlgA=0.0027043787615d0
#else
...
!-------------------------------------------------------------------------
...
#ifdef STARS_ARE_SINKS
  !If stars are sinks, we put blackbody temperature in age bins
  write(fAges,'(a,a)') trim(sed_dir),"/bb_temperatures.dat"
#else
  ...
  if(.not. ok .or. .not. okAge .or. .not. okZ) then
     if(myid.eq.1) then
        ...
        write(*,*) 'Check if SED-directory contains the files ',  &
                   'bb_temperatures.dat, all_seds.dat'
#else
        write(*,*) 'Check if SED-directory contains the files ',  &
...
#ifdef STARS_ARE_SINKS
  ! Note that this is a dummy because when stars are sinks
  ! we do not use metallicity bins since every star particle
  ! is a black body which only depends on temperature
  nzs = 5
  allocate(zs(5))
  zs(1) = 0.001d0
  zs(2) = 0.005d0
  ...
  if(KatzSED)then
     ...
     allocate(SEDs(nLs,nAges,nzs))
#ifdef STARS_ARE_SINKS
     ! if stars are sinks, we only interpolate temperature
     ! default the array and then overwrite on read
     ! only fill our the first metallicity 
     SEDs(:,:,:) = 1.d-40 ! initialization
     do ia = 1, nAges
        read(10) SEDs(:,ia,1)
     end do
     ! Set the other metallicities to the same value
     do iz = 2,nzs
        SEDs(:,:,iz) = SEDs(:,:,1)
     end do
#else
...
#ifdef STARS_ARE_SINKS
     ! Don't integrate if stars are sinks
     ! Set cumulative to instantaneous
     tbl(:,:,2) = tbl(:,:,1)
#else
...
#ifdef STARS_ARE_SINKS
  logical::no_radiation_trigger
  integer::sse_index_metal,sse_index_mass,sse_index_age_1
  real(dp)::tmp_age_0,tmp_age_1,tmp_dt
  real(dp)::main_seq_lt_yrs
  real(dp)::tmp_lower_teff,tmp_upper_teff
  real(dp)::tmp_lower_rad,tmp_upper_rad
#endif
!-------------------------------------------------------------------------
  ...
  do i=1,npartmax
     ...
     if(pop3 .and. is_pop_III(typep(i)))then
        ...
        ! Don't include Pop III stars that have exploded
        ! Set luminosity to a very low value
        if (.not.is_pre_SN(typep(i))) L_star = 1.d-40 ! Very low but non-zero value
     else
        ! Note that for stars are sinks it doesn't actually matter
        ! what the value of metallicity is
#ifdef STARS_ARE_SINKS
        ! If stars are sinks, age actually represents effective temperature
        
        ! Check if star should radiate
        if (pre_ms(i).gt.1000) then
           ! Set the radiation trigger to false
           no_radiation_trigger=.false.

           ! Get the indices for mass and metallicity in the SSE arrays
           sse_index_metal = MOD(pre_ms(i),10000)
           sse_index_mass  = (pre_ms(i) - sse_index_metal)/10000
           ...
           sse_index_age_1 = FLOOR(((age*1.d9)/main_seq_lt_yrs)*n_sse_ages)

           ! Interpolate the effective temperature and radius of the star 
           tmp_upper_teff = all_sse_probs(sse_index_metal,sse_index_mass,MAX(sse_index_age_1,1),3) ! Teff log10 K
           tmp_lower_teff = all_sse_probs(sse_index_metal,sse_index_mass,MAX(sse_index_age_1,1)+1,3) ! Teff log10 K
           tmp_upper_rad  = all_sse_probs(sse_index_metal,sse_index_mass,MAX(sse_index_age_1,1),4) ! R log10 K
           tmp_lower_rad  = all_sse_probs(sse_index_metal,sse_index_mass,MAX(sse_index_age_1,1)+1,4) ! R log10 Rsun
           ...
           ! Now linearly interpolate radius and store in mass
           ! Note that the SED array for our particular case has units of
           ! photons / cm^2 so we need to multiply by the star's 
           ! surface area
           mass = (((tmp_age_1 - (age*1.d9))/tmp_dt) * tmp_upper_rad) + ((((age*1.d9) - tmp_age_0)/tmp_dt) * tmp_lower_rad)
           mass = (10.d0**mass)
           mass = 4.d0 * pi * ((mass * 6.96d10)**2.d0)

           ! Now linearly interpolate temperature and store in age
           age = (((tmp_age_1 - (age*1.d9))/tmp_dt) * tmp_upper_teff) + ((((age*1.d9) - tmp_age_0)/tmp_dt) * tmp_lower_teff)
           age = 10.**age

        else
           ! Set the radiation trigger to false
           no_radiation_trigger=.true.

           ! Set the age to a very low value
           age = 1.d3 
        endif
#endif
...
#ifdef STARS_ARE_SINKS
        ! Set L_star to a low value for stars that dont radiate
        if (no_radiation_trigger) L_star = 1.d-40 ! Very low but non-zero value
#endif
...
#ifdef STARS_ARE_SINKS
     ! Note the bb fun is erg/s/cm^2/cm
     ! This is better for integration but need to scale down now
     getSEDLuminosity = getSEDLuminosity * 1.d-8 
#else
...
#ifdef STARS_ARE_SINKS
     getSEDLuminosity = getSEDLuminosity*1.d-8/eV_to_erg
#else
...
SUBROUTINE getNPhotonsEmitted_sas(Teff, dt_Gyr, Rstar, ret)
! Compute number of photons emitted by a stellar particle per cm^2
! over a timestep.
! Note that here we don't care about metallicity
! input/output:
...
!-------------------------------------------------------------------------
  use rt_parameters
  use constants, only: pi
  real(dp),intent(in):: Teff, dt_Gyr, Rstar
  real(dp),dimension(nSEDgroups):: ret, Lc0, SEDegy
  real(dp)::Z=0.02d0 !Hard-coded to solar --> doesn't matter (but needed for interpolation)
!-------------------------------------------------------------------------
  ! Lc0 = photon intensity in photons / s / cm^2 
  call inp_SED_table(Teff, Z, 1, .false., Lc0)
  ret = max(Lc0,0.d0)
  ! Multiply by the surface area of the star
  ret = ret * 4.d0 * pi * ( (Rstar * 6.96e+10)**2.d0 )
  ! Multiply by the timestep in seconds
  ret = ret * dt_Gyr * Gyr2sec
  if(SED_isEgy) then ! Integrate correct energy rather than # of photons
     ! Divide emitted energy by group energy -> Photon count
     ret = ret / group_egy(1:nSEDgroups)
  endif
END SUBROUTINE getNPhotonsEmitted_sas
...
#ifdef STARS_ARE_SINKS
  real(dp)::teff_star,rad_star,main_seq_lt_yrs
  real(dp)::tmp_upper_teff,tmp_lower_teff
  real(dp)::tmp_upper_rad,tmp_lower_rad
  real(dp)::tmp_age_0,tmp_age_1,tmp_dt
  integer::sse_index_metal,sse_index_mass,sse_index_age_1
#endif
!-------------------------------------------------------------------------
  ...
  do j = 1, np
...
#ifdef STARS_ARE_SINKS
        ! First check that the star is on the main sequence
        ! if not, set the photon count to zero
        if (pre_ms(ind_part(j)).gt.1000) then
             ! Get the indices for mass and metallicity in the SSE arrays                              
             ...
             ! Interpolate the effective temperature and radius of the star                            
             tmp_upper_teff = all_sse_probs(sse_index_metal,sse_index_mass,MAX(sse_index_age_1,1),3) ! 
             tmp_lower_teff = all_sse_probs(sse_index_metal,sse_index_mass,MAX(sse_index_age_1,1)+1,3) 
             tmp_upper_rad  = all_sse_probs(sse_index_metal,sse_index_mass,MAX(sse_index_age_1,1),4) ! 
             tmp_lower_rad  = all_sse_probs(sse_index_metal,sse_index_mass,MAX(sse_index_age_1,1)+1,4) 
             ...
             ! Now linearly interpolate radius                                       
             rad_star = (((tmp_age_1 - (age*1.d9))/tmp_dt) * tmp_upper_rad) + ((((age*1.d9) - tmp_age_0)/tmp_dt) * tmp_lower_rad)
             rad_star = 10.d0**rad_star ! Units Rsol                                                                       
                                                                                                       
             ! Now linearly interpolate temperature 
             teff_star = (((tmp_age_1 - (age*1.d9))/tmp_dt) * tmp_upper_teff) + ((((age*1.d9) - tmp_age_0)/tmp_dt) * tmp_lower_teff)
             teff_star = 10.**teff_star ! Units K

             ! Now that we have the stellar radius and temperature, get the number of photons
             call getNPhotonsEmitted_sas(teff_star,dt_loc_Gyr,rad_star,part_NpInp(j,1:nSEDgroups))
        else
           part_NpInp(j,1:nSEDgroups) = 0.d0
        endif
#else
     ...
     !TODO(code) fix this bit because we shouldn't multiply by mass in our case
