subroutine backup_part(filename, filename_desc)
...
#endif
  character(len=80) :: filename, filename_desc, field_name
  ...
  integer :: imet, ich ! chemical elements and metal enrichment
  ...
  if (star .or. sink) then
     ...
     if(metal)then
      do imet = 1, nmetals ! metal enrichment
        ...
        write(field_name, '("metallicity_",I0.3)') imet
     ...
     if(imf_varies_complex) then
        ...
        do i=1,npartmax
           if(levelp(i)>0)then
              ipart=ipart+1
              xdp(ipart)=part_imf_slope(i)
           end if
        end do
        call generic_dump("imf_slope", ivar, xdp, unit_out, dump_info, unit_info)
     end if
     ...
     do ich = 1, nchem
        ...
        write(field_name, '("chemical_abundance_",I3)') ich
        call generic_dump(field_name, ivar, xdp, unit_out, dump_info, unit_info)
     end do
     ...
     call generic_dump("current_mass", ivar, xdp, unit_out, dump_info, unit_info)

     ! Write main-sequence birth time
     ipart = 0
     do i = 1, npartmax
        if (levelp(i) > 0) then
           ipart = ipart+1
     ...
     call generic_dump("ms_birth_time", ivar, xdp, unit_out, dump_info, unit_info)

     ! Write pre main sequence
     allocate(ll(1:npart))
     ipart = 0
     do i = 1, npartmax
        if (levelp(i) > 0) then
     ...
     call generic_dump("pre_ms", ivar, ll, unit_out, dump_info, unit_info)
     deallocate(ll)
#endif
