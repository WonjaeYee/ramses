...
!################################################################
!################################################################
subroutine accrete_stars_are_sinks_uold(ilevel)
  use amr_commons
  use pm_commons
  implicit none
  integer::ilevel
  ...
  do icpu=1,ncpu
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              !if (ok_star) write(*,*) idp(ipart),mp_c(ipart),mp(ipart),pre_ms(ipart)
              ! Select only pre main-sequence star particles
              ok_star = ok_star .and. (pre_ms(ipart).lt.0)
        ...
        if(npart2>0)then
           ...
           do jpart=1,npart1
              ...
              if(ip==nvector)then
                 call accrete_sas_uold(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
                 ip=0
                 ig=0
              end if
              ipart=next_part  ! Go to next particle
     ...
     ! End loop over grids
     if(ip>0)call accrete_sas_uold(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
  end do
  ! End loop over cpus

111 format('   Entering accrete_stars_are_sinks_uold for level ',I2)

end subroutine accrete_stars_are_sinks_uold
!################################################################
!################################################################
!################################################################
!################################################################
subroutine accrete_sas_uold(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  ...
  ! that is an active accretor will pull mass from uold at some rate
  !-----------------------------------------------------------------------
  ...
  do ivar=i_fractions,nvar
     do j=1,np
        fractions(j,ivar) = uold(indp(j),ivar)/uold(indp(j),1)
     end do
  end do
  ...
  do j=1,np
     ...
     do irad=1,3
        ...
        initial_total_momentum2(irad) = uold(indp(j),irad+1) * vol_loc(j) ! cell momentum
     ...
     if ((sf_mass+mp_c(ind_part(j))).gt.mp(ind_part(j))) then
        ...
        ! pre-main-sequence by flipping to a positive value
        pre_ms(ind_part(j)) = -pre_ms(ind_part(j))
        ...
        !write(*,*) "Finished accreting cells",myid,idp(ind_part(j))
        !write(*,*) "FFF Accreting ",sf_mass*scale_msun,"Needed ",scale_msun*(mp(ind_part(j))-mp_c(ind_part(j)))
     else
        ! Star will still be accreting
        ! use flag2 as temporary space to mark this
        flag2(indp(j)) = -42
        !write(*,*) "marking accreting cells",myid,idp(ind_part(j)),pre_ms(ind_part(j))
        !write(*,*) "Accreting ",sf_mass*scale_msun,"Needed ",scale_msun*(mp(ind_part(j))-mp_c(ind_part(j))) 
     end if
     ...
     if(metal)then
        do imet=1,nmetals
           if (isCO) then
              if (imet.eq.2) then
                 zp(ind_part(j),imet) = ((zp(ind_part(j),imet)*mp_c(ind_part(j))) + ((uold(indp(j),imetal+imet-1)+(mO_NIST_amu/(mC_NIST_amu+mO_NIST_amu))*uold(indp(j),iCO))*sf_mass))/(mp_c(ind_part(j))+sf_mass)
              else if (imet.eq.8) then
                 zp(ind_part(j),imet) = ((zp(ind_part(j),imet)*mp_c(ind_part(j))) + ((uold(indp(j),imetal+imet-1)+(mC_NIST_amu/(mC_NIST_amu+mO_NIST_amu))*uold(indp(j),iCO))*sf_mass))/(mp_c(ind_part(j))+sf_mass) 
              else
                 zp(ind_part(j),imet) = ((zp(ind_part(j),imet)*mp_c(ind_part(j))) + (uold(indp(j),imetal+imet-1)*sf_mass))/(mp_c(ind_part(j))+sf_mass)
              endif
           else
              zp(ind_part(j),imet) = ((zp(ind_part(j),imet)*mp_c(ind_part(j))) + (uold(indp(j),imetal+imet-1)*sf_mass))/(mp_c(ind_part(j))+sf_mass)
           endif
     ...
     ! Update density 
     uold(indp(j),1) = uold(indp(j),1) - sf_mass/vol_loc(j)

     ! Update velocity
     uold(indp(j),2:4) = uold(indp(j),2:4) - vv(1:3)*sf_mass/vol_loc(j)
     
     ! Update energy 
     e = e * uold(indp(j),1)
     do irad=1,3
        e = e + (0.5d0 * ((uold(indp(j),irad+1)/uold(indp(j),1))**2.d0) * uold(indp(j),1))
     end do
#if NENER>0
     do irad=1,nener
        e=e-uold(indp(j),5+irad)
     ...
     do ivar=imetal,imetal+nmetals+nco-1
        uold(indp(j),ivar)=uold(indp(j),ivar)-uold(indp(j),ivar)*sf_mass/(d*vol_loc(j))
     end do
     ...
     final_total_mass = uold(indp(j),1)*vol_loc(j)+mp_c(ind_part(j))
     ...
     do irad=1,3
        final_total_momentum = uold(indp(j),irad+1) * vol_loc(j) ! cell momentum
     ...
     T2_final = uold(indp(j),5)
     ! Subtract off kinetic energy
     do irad=1,3
        T2_final = T2_final - (0.5d0 * (uold(indp(j),irad+1)**2.d0)/uold(indp(j),1))
     end do
     ! Subtract off non-thermal energy
#if NENER>0
     do irad=1,nener
           T2_final = T2_final - uold(indp(j),5+irad)
     end do
#endif
     T2_final = (gamma-1)*(T2_final/uold(indp(j),1))*scale_T2
  ...
  do j=1,np
     do ivar=i_fractions,nvar
        uold(indp(j),ivar) = fractions(j,ivar) * uold(indp(j),1)
     end do
  end do

end subroutine accrete_sas_uold
