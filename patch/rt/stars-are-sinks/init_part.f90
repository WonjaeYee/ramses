subroutine init_part
  ...
  logical::error,keep_part,eof,jumped,ic_sink=.false.,read_pos=.false.,ok,read_ids=.false.
  character(LEN=100)::filename,filename_x,filename_id
  ...
  if(star.or.sink)then
     ...
     if(imf_varies_complex) then
        allocate(part_imf_slope(npartmax))
        part_imf_slope=2.3 ! initialize to salpeter
     end if
     if(nchem>0)then
     ...
     allocate(mp_c(npartmax))
     mp_c=0.0
     allocate(tp_ms(npartmax))
     tp_ms=0.0
     allocate(pre_ms(npartmax))
     pre_ms=1 ! put all particles on pre-ms
#endif
  end if
  ...
  if(nrestart>0)then
     ...
     if(star.or.sink)then
        ...
        read(ilun)xdp
        mp_c(1:npart2)=xdp
        ! Read main-sequence time
        read(ilun)xdp
        tp_ms(1:npart2)=xdp
        ! Read whether star is pre-main-sequence
        allocate(isp(1:npart2))
        read(ilun)isp
        pre_ms(1:npart2)=isp
        deallocate(isp)
...
100           continue
           ...
           do i=1,jpart
              ...
              if(cc(i)==myid)then
                 ...
                 if(imf_varies_complex) part_imf_slope(ipart) = 2.3 
                 typep(ipart)%family = FAM_DM
#ifndef WITHOUTMPI
              endif
#endif
