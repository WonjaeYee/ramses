...
! Precision for radiative transfer
  ...
  real(dp)::star_group_mass = 4.d0 ! mass below which we group individual stars
  real(dp)::star_lp_mass = 500.d0  ! Star particle mass for grouped stars
  real(dp)::star_sink_initial_mass = 1.d0 ! initial mass for sink particles
  character(LEN=128)::sse_dir=''       ! Dir containing stellar evolution params  ! 
  character(LEN=128)::sse_file=''      ! File containing stellar evolution params !
  logical::sse_supernova=.true.        ! Single star SN feedback
  logical::sse_winds=.true.            ! Single star wind feedback
  character(LEN=128)::lc18_sn_dir      ! File containing SN yields !
#endif
  ...
  ! WJ: connect current version of RAMSES with stars_are_sinks
  ! integer::nrestart_quad=0
  ! logical::restart_remap=.false.
  ! logical::static_dm=.false.
  ! logical::static_gas=.false.
  ! logical::static_stars=.false.
  ! integer,allocatable,dimension(:)::remap_pscalar
  ! logical::unbind=.false.
  ! logical::make_mergertree=.false.
  ! logical::stellar=.false.
  ! real(dp)::delta_tout=HUGE(1.0d0)
  ! real(dp)::delta_aout=HUGE(1.0d0)
  ! real(dp)::walltime_hrs=-1
  ! real(dp)::minutes_dump=1
  ! integer::nlevel_collapse=3
  logical(dp)::finish_run=.false.
  integer::momentum_feedback=0
  integer::strict_equilibrium=0

  character(len=20)::barotropic_eos_form='legacy'
  real(dp)::polytrope_rho=1.0d50
  real(dp)::polytrope_rho_cu=1.0d50
  real(dp)::polytrope_index=1.0d0
  real(dp)::T_eos=10
  real(dp)::mu_gas=1d0
  real(dp)::T2_eos=10
  ...
  real(dp)::aout_next=HUGE(1.0D0)
  real(dp)::tout_next=HUGE(1.0D0)
