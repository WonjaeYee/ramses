...
subroutine dump_all
  ...
  character(LEN=1024)::filename,filename_desc,filedir,filedirini,filecmd
  ...
  if(ndim>1)then
...
#ifdef NOSYSTEM
        call PXFMKDIR(TRIM(filedirini),LEN(TRIM(filedirini)),int(O'755'),info)
        call PXFMKDIR(TRIM(filedir),LEN(TRIM(filedir)),int(O'755'),info)
#else
        call system(filecmd)
#endif
     endif
     ...
     if(pic)then
        ...
        if (pop3)then
           filename=TRIM(filedir)//'pop3_'//TRIM(nchar)//'.out'
           call backup_pop3(filename)
        end if
#ifndef WITHOUTMPI
        if(synchro_when_io) call MPI_BARRIER(MPI_COMM_WORLD,info)
#endif
...
subroutine backup_amr(filename)
  ...
  write(ilun)consv_e,mass_tot_0,rho_tot
...
subroutine output_header(filename)
  ...
  if (myid == 1) then
     ...
     if(star.or.sink) then
        ...
        if(metal) then ! metal enrichment
           ! Metal Enrichment: Hardcoded. 1=Fe, 2=O, 3=N, 4=Mg, 5=Ne, 6=Si, 7=Ca, 8=C, 9=S
           write(ilun,'(a)',advance='no')'metal_Fe '
           write(ilun,'(a)',advance='no')'metal_O '
           write(ilun,'(a)',advance='no')'metal_N '
           write(ilun,'(a)',advance='no')'metal_Mg '
           write(ilun,'(a)',advance='no')'metal_Ne '
           write(ilun,'(a)',advance='no')'metal_Si '
           write(ilun,'(a)',advance='no')'metal_Ca '
           write(ilun,'(a)',advance='no')'metal_C '
           write(ilun,'(a)',advance='no')'metal_S '
        endif
        if(use_initial_mass) then
           ! We store the imass for RT runs. need to save in header for pynbody
           write(ilun,'(a)',advance='no')'imass '
        endif
        if(imf_varies_complex) then
           write(ilun,'(a)',advance='no')'imf_slope '
        end if
#ifdef STARS_ARE_SINKS
        write(ilun,'(a)',advance='no')'current_mass '
        write(ilun,'(a)',advance='no')'tform_ms '
        write(ilun,'(a)',advance='no')'pre_ms '
#endif
     endif
     close(ilun)
...
subroutine create_output_dirs(filedir)
   ...
   if (.not.withoutmkdir) then
     if (myid==1) then
       ...
       call PXFMKDIR(TRIM(filedirini),LEN(TRIM(filedirini)),int(O'755'),info_sys)
       call PXFMKDIR(TRIM(filedir),LEN(TRIM(filedir)),int(O'755'),info_sys)
#else
       filecmd='mkdir -p '//TRIM(filedir)
       ierr=1
