module pm_commons
  ...
  real(dp),allocatable,dimension(:)  ::part_imf_slope      ! IMF slope
  ...
  real(dp),allocatable,dimension(:)  ::mp_c     ! Current mass --> final mass is in mp
  real(dp),allocatable,dimension(:)  ::tp_ms    ! Time the star particle reached the main sequence
  integer, allocatable,dimension(:)  ::pre_ms   ! Is the star pre-main-sequence
#endif
  real(dp),allocatable,dimension(:,:)::weightp  ! weight of cloud parts for sink accretion only
  real(dp),allocatable,dimension(:,:)  ::zp       ! Birth metallicity ! metal enrichment
  ...
  integer(i8b),allocatable,dimension(:)::idp    ! Identity of particle
   real(dp),allocatable,dimension(:,:)::chp      ! Birth chemical abundance (mass fraction)
  ! Tree related arrays
  ...
  ! Kang added, 210615
  real(dp),allocatable,dimension(:,:)::xpop3, xpop3_new, xpop3_all    ! Positions of pop3 stars
  ...
  ! Stellar types (with tag)
  integer(1),parameter :: &
    STAR_POP2_BEFORE_SNe=-2, &
    STAR_POP3_BEFORE_SNe=-3, &
    STAR_POP2_AFTER_SNe=2, &
    STAR_POP3_AFTER_SNe=3, &
    STAR_SINK_PARTICLE=5
...
contains
  ...
  elemental logical pure function is_pre_SN(typep)
    type(part_t), intent(in) :: typep
    is_pre_SN = is_star(typep) .and. ((typep%tag == STAR_POP2_BEFORE_SNe) .or. (typep%tag == STAR_POP3_BEFORE_SNe))
  end function is_pre_SN

  elemental logical pure function is_post_SN(typep)
    type(part_t), intent(in) :: typep
    is_post_SN = is_star(typep) .and. ((typep%tag == STAR_POP2_AFTER_SNe) .or. (typep%tag == STAR_POP3_AFTER_SNe))
  end function is_post_SN

  elemental logical pure function is_pop_II(typep)
     type(part_t), intent(in) :: typep
     is_pop_II = is_star(typep) .and. ((typep%tag == STAR_POP2_BEFORE_SNe) .or. (typep%tag == STAR_POP2_AFTER_SNe))
  end function is_pop_II

  elemental logical pure function is_pop_III(typep)
     type(part_t), intent(in) :: typep
     is_pop_III = is_star(typep) .and. ((typep%tag == STAR_POP3_BEFORE_SNe) .or. (typep%tag == STAR_POP3_AFTER_SNe))
  end function is_pop_III
