...
subroutine virtual_tree_fine(ilevel)
  use pm_commons
  use amr_commons
  use hydro_parameters,only: nmetals ! metal enrichment
  use mpi_mod
  implicit none
  integer::ilevel
  !-----------------------------------------------------------------------
  ...
  if(star.or.sink) then
     if(metal) then
        particle_data_width=twondim+3+nmetals ! metal enrichment
     else
        particle_data_width=twondim+2
     endif
     if(use_initial_mass) particle_data_width = particle_data_width + 1
     if(imf_varies_complex) particle_data_width = particle_data_width + 1
     if (nchem>0) particle_data_width = particle_data_width + nchem
#ifdef STARS_ARE_SINKS
     particle_data_width = particle_data_width + 2 ! for mp_c and tp_ms
     particle_data_width_int = particle_data_width_int + 1 ! for pre_ms
#endif
  endif
...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  integer::current_property,current_property_int
  integer::i,idim,ich
  logical,dimension(1:nvector),save::ok=.true.
  ...
  if(star.or.sink)then
     ...
     if(imf_varies_complex) then
        do i=1,np
           reception(icpu,ilevel)%up(ind_com(i),current_property)=part_imf_slope(ind_part(i))
        end do
        current_property = current_property+1
     end if
#ifdef STARS_ARE_SINKS
     do i=1,np
        reception(icpu,ilevel)%up(ind_com(i),current_property)=mp_c(ind_part(i))
     end do
     current_property = current_property+1

     do i=1,np
        reception(icpu,ilevel)%up(ind_com(i),current_property)=tp_ms(ind_part(i))
     end do
     current_property = current_property+1

     do i=1,np
        reception(icpu,ilevel)%fp(ind_com(i),current_property_int) = pre_ms(ind_part(i))
     end do
     current_property_int = current_property_int + 1
#endif
     do ich=1,nchem
        do i=1,np
  ...
  if (MC_tracer) then
     do i=1,np
        if (is_star_tracer(typep(ind_part(i)))) then
           ...
           reception(icpu, ilevel)%fp(ind_com(i), current_property_int) = &
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  do i=1,np
     ...
     idp   (ind_part(i))=int(emission(icpu,ilevel)%fp(ind_com(i),3), i8b)
  ...
  if(star.or.sink)then
     ...
     if(imf_varies_complex) then
        do i=1,np
           part_imf_slope(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property)
        end do
        current_property = current_property+1
     end if
#ifdef STARS_ARE_SINKS
     do i=1,np
        mp_c(ind_part(i)) = emission(icpu,ilevel)%up(ind_com(i),current_property)
     end do
     current_property = current_property+1

     do i=1,np
        tp_ms(ind_part(i)) = emission(icpu,ilevel)%up(ind_com(i),current_property)
     end do
     current_property = current_property+1

     do i=1,np
        pre_ms(ind_part(i)) = emission(icpu,ilevel)%fp(ind_com(i),current_property_int) 
     end do
     current_property_int = current_property_int + 1
#endif
     do ich=1,nchem
  ...
  if (MC_tracer) then
     do i=1,np
        ...
        partp(ind_part(i)) = emission(icpu,ilevel)%fp(ind_com(i), current_property_int)
