         ...
         if (rt .and. nGroups .gt. 0) then
            ...
            if (TK .le. T_sputter) then ! dust is assumed to be destroyed if too hot
               ...
               if (simple_dust_depletion) then 
                  ...
                  signc_dust(:,1) = (/ 5.190E-17, 7.611E-16, 2.140E-15, 2.830E-15, 2.955E-15, 2.929E-15, 2.442E-15, 1.303E-15 /)*rt_c_cgs
                  dustAbs(:) = signc_dust(:,1)*(nH(icell)/X)*dust_to_gas_mass_ratio_over_mw/(1784268.76d0) ! [cm2/H * H/cm^3 * cm/s]
                  !dustAbs(:) = signc_dust(:,1)*(nH(icell)/X)*dust_to_gas_mass_ratio_over_mw ![cm2/H * H/cm^3 * cm/s]
               end if
            else
               dustAbs(:) = 0d0
               dustSc(iIR) = 0d0
               ...
               do igroup = 1, nGroups                              !  Photoheating
                  if (igroup .eq. LWgroup) then
                     if(isH2Katz) then
                        ...
                        if (H2clumping.lt.1.d-2) then
                           H2heating = 0.d0
                           H2heating_prime = 0.d0
                        endif
                     endif
                  else
...
!           write(777,778) xH2, nH(icell), cr, de, photoRate, beta(ixHI),alpha(ixHI),xHI,xe,ddt(icell)
         ...
         if (include_grain_recombination.and.TK.le.1.d4) then
            ...
            cr = cr + ( DUST_RECOMBINATION(1,G_0,Tk,ne) * xHII * nH(icell) * dust_to_gas_mass_ratio_over_mw)
         end if
         ...
         if (include_grain_recombination.and.TK.le.1.d4) then
            de = de + ( DUST_RECOMBINATION(1,G_0,Tk,ne) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
         end if
         ...
         if (isHe) then
            ...
            if (include_grain_recombination.and.TK.le.1.d4) then
               cr = cr + ( DUST_RECOMBINATION(2,G_0,Tk,ne) * xHeII * nH(icell) * dust_to_gas_mass_ratio_over_mw)
            end if
            ...
            if (include_grain_recombination.and.TK.le.1.d4) then
               de = de + ( DUST_RECOMBINATION(2,G_0,Tk,ne) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
            end if
         ...
         if (carbon_ions) then
               do im=1,n_carbon_ions
                  ...
                  if (include_grain_recombination.and.TK.le.1.d4) then
                     if (im.eq.1) then
                        cr = cr + ( DUST_RECOMBINATION(6,G_0,Tk,ne) * dxC(2) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     else if (im.eq.2) then
                        de = de + ( DUST_RECOMBINATION(6,G_0,Tk,ne) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     end if
                  end if
         ...
         if (magnesium_ions) then
               do im=1,n_magnesium_ions
                  ...
                  if (include_grain_recombination.and.TK.le.1.d4) then
                     if (im.eq.1) then
                        cr = cr + ( DUST_RECOMBINATION(12,G_0,Tk,ne) * dxMg(2) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     else if (im.eq.2) then
                        de = de + ( DUST_RECOMBINATION(12,G_0,Tk,ne) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     end if
                  end if
         ...
         if (silicon_ions) then
               do im=1,n_silicon_ions
                  ...
                  if (include_grain_recombination.and.TK.le.1.d4) then
                     if (im.eq.1) then
                        cr = cr + ( DUST_RECOMBINATION(14,G_0,Tk,ne) * dxSi(2) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     else if (im.eq.2) then
                        de = de + ( DUST_RECOMBINATION(14,G_0,Tk,ne) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     end if
                  end if
         ...
         if (sulfur_ions) then
               do im=1,n_sulfur_ions
                  ...
                  if (include_grain_recombination.and.TK.le.1.d4) then
                     if (im.eq.1) then
                        cr = cr + ( DUST_RECOMBINATION(16,G_0,Tk,ne) * dxS(2) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     else if (im.eq.2) then
                        de = de + ( DUST_RECOMBINATION(16,G_0,Tk,ne) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     end if
                  end if
         ...
         if (iron_ions) then
               do im=1,n_iron_ions
                  ...
                  if (include_grain_recombination.and.TK.le.1.d4) then
                     if (im.eq.1) then
                        cr = cr + ( DUST_RECOMBINATION(26,G_0,Tk,ne) * dxFe(2) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     else if (im.eq.2) then
                        de = de + ( DUST_RECOMBINATION(26,G_0,Tk,ne) * nH(icell) * dust_to_gas_mass_ratio_over_mw)
                     end if
                  end if
