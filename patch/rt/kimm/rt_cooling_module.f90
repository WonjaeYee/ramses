...
!-------------------------------------------------------------------------
  ...
  do i=1,ncell
     ...
     call reduce_xion( xion(1:nIons,i), x_MIN) ! Taysun
...
contains
  ...
  !-----------------------------------------------------------------------
    ...
    call reduce_xion ( dXion, x_MIN ) ! Taysun
    xHII = dXion(ixHII)
    xHI = 1.d0 - xHII
    ...
    if(rt .and. rt_advect) then 
       ...
       do igroup=1,nGroups      ! -------------------Ionization absorbtion
          if(igroup.eq.LWgroup)then
             phAbs(igroup) = nN(ixHI)*(1+fpump)*sigUV ! For LW radiation (isH2Katz)
          else
...
!           write(777,778) xH2, nH(icell), cr, de, photoRate, beta(ixHI),alpha(ixHI),xHI,xe,ddt(icell)
    ...
    if(isH2)then
       ...
       dUU = ABS((xHI-xion(ixHI,icell))) &
         & /(xion(ixHI,icell)+x_MIN)*one_over_x_FRAC
       if(dUU .gt. 1.)then
          code=8; RETURN 
       endif
       fracMax = MAX(fracMax,dUU)
    ...
    if(isH2) then
       ...
       if(xHI.le.dXion(ixHII)) dXion(ixHII)=1.d0-xHI
    endif
    ...
    if(isHe) then
       ...
       de = beta(ixHeII) * ne
       photoRate = 0.d0
       if(rt) photoRate = SUM(signc(:,ixHeII)*dNp)
       ...
       if(haardt_madau) photoRate = photoRate + UVrates(ixHeIII,1) * ss_factor
    ...
    call reduce_xion(dXion,x_MIN)
    xHII   = dXion(ixHII)
    if(isHe) xHeII  = dXion(ixHeII)
    if(isHe) xHeIII = dXion(ixHeIII)
...
!-------------------------------------------------------------------------
  ...
  do while (aexp < aend)
     ...
     call rt_solve_cooling(T2,xion,Np,Fp,p_gas,dNpdt,dFpdt,nH,c_switch   &
                           ,Zsolar,dt_cool,aexp,1,dx_SS) ! dx_SS=0.; no self-shielding
     T2(1)=T2(1)*aexp**2
     aexp = aexp + daexp
...
!************************************************************************
SUBROUTINE reduce_xion(xion,x_MIN)
...
!------------------------------------------------------------------------
  ...
  real(kind=dp)::x_MIN,x_H2
  real(kind=dp),parameter::x_MIN2=0d0 ! added for stability
!------------------------------------------------------------------------
  ...
  xion(1:nIons)=MIN(MAX(xion(1:nIons), x_MIN), 1.d0)

  !Ensure the total hydrogen fraction is 1
  if(isH2)then
     x_H2 = (1.d0 - xion(ixHI) - xion(ixHII))*0.5 
     if(x_H2<0d0)then
        if(xion(ixHI) .gt. xion(ixHII))then
           xion(ixHI)=1.d0-xion(ixHII)-x_MIN2
        else
           xion(ixHII)=1.d0-xion(ixHI)-x_MIN2
        endif
     endif
  endif
  ...
  if(isHe)then
     if(xion(ixHeII)+xion(ixHeIII) .gt. 1.d0)then
        if(xion(ixHeII) .gt. xion(ixHeIII))then
           xion(ixHeII)=1.d0-xion(ixHeIII)-x_MIN2
        else
           xion(ixHeIII)=1.d0-xion(ixHeII)-x_MIN2
        endif
     endif 
  endif
