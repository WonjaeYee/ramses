        ...
        if(npart2>0)then        
           ...
           do jpart=1,npart1
              ...
              if(use_particle_type)then
                 ...
                 if(idp(ipart).lt.0.and.tp(ipart).ne.0) ok_star=.true.
              endif
              if(ok_star)then
                 if(ig==0)then
                    ig=1
...
subroutine feedbk(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  ESN=E_SNII/(M_SNII*2d33)/scale_v**2

  ! Life time radiation specific energy from cgs to code units
  ERAD=1d53/(M_SNII*2d33)/scale_v**2
  ...
  ! record the fractional quantities
  ...
  ! Keep the fractional quantities the same
  do ivar=i_fractions,nvar
     do j=1,np
        unew(indp(j),ivar) = unew(indp(j),1) * fractions(j,ivar)
     end do
  end do
