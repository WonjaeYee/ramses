...
subroutine condinit(x,u,dx,nn)
  ...
  use rt_cooling_module, ONLY:mO_NIST_amu,mN_NIST_amu,mC_NIST_amu,mMg_NIST_amu,mSi_NIST_amu,mS_NIST_amu,mFe_NIST_amu,mNe_NIST_amu,mCa_NIST_amu
  implicit none
  ...
  real(dp)::lise_met
  real(dp) :: table_Solar_Fe_loc,Iron_loc
  real(dp) :: table_Solar_O_loc,Oxygen_loc
  real(dp) :: table_Solar_N_loc,Nitrogen_loc
  real(dp) :: table_Solar_Si_loc,Silicon_loc
  real(dp) :: table_Solar_Ca_loc,Calcium_loc
  real(dp) :: table_Solar_Mg_loc,Magnesium_loc
  real(dp) :: table_Solar_Ne_loc,Neon_loc
  real(dp) :: table_Solar_C_loc,Carbon_loc
  real(dp) :: table_Solar_S_loc,Sulfur_loc
  real(dp) :: Hloc = 0.76d0
...
!        if(metal)q(i,6)=z_ave*0.02 ! z_ave is in solar units
        if(metal) then 
          ! Lise uses TABLE ISM from CLOUDY. 
          ! Abundances from CLOUDY table ISM
          table_Solar_O_loc  = 3.19d-4
          table_Solar_Fe_loc = 6.31d-7
          table_Solar_N_loc  = 7.94d-5
          table_Solar_Mg_loc = 1.26d-5
          table_Solar_Ne_loc = 1.23d-4
          table_Solar_Si_loc = 3.16d-6
          table_Solar_Ca_loc = 4.10d-10
          table_Solar_C_loc  = 2.51d-4
          table_Solar_S_loc  = 3.24d-5

          ! Here we have hard-coded those abundances and converted into mass
          ! fractions
          Oxygen_loc    = z_ave * (table_Solar_O_loc  * mO_NIST_amu)  * Hloc
          Iron_loc      = z_ave * (table_Solar_Fe_loc * mFe_NIST_amu) * Hloc
          Nitrogen_loc  = z_ave * (table_Solar_N_loc  * mN_NIST_amu)  * Hloc
          Magnesium_loc = z_ave * (table_Solar_Mg_loc * mMg_NIST_amu) * Hloc
          Neon_loc      = z_ave * (table_Solar_Ne_loc * mNe_NIST_amu) * Hloc
          Silicon_loc   = z_ave * (table_Solar_Si_loc * mSi_NIST_amu) * Hloc
          Calcium_loc   = z_ave * (table_Solar_Ca_loc * mCa_NIST_amu) * Hloc
          Carbon_loc    = z_ave * (table_Solar_C_loc  * mC_NIST_amu)  * Hloc
          Sulfur_loc    = z_ave * (table_Solar_S_loc  * mS_NIST_amu)  * Hloc

          ! Now make Lise's modifications
          ! Update Si following Izotov 1999
          lise_met = 12.d0 + log10(z_ave * table_Solar_O_loc)
          if (lise_met.le.7.6d0) then
             Silicon_loc = ((z_ave * table_Solar_O_loc) * (10.d0**(-1.50))) * mSi_NIST_amu * Hloc
          else
             Silicon_loc = ((z_ave * table_Solar_O_loc) * (10.d0**(-1.45))) * mSi_NIST_amu * Hloc
          endif

          ! Update Ne, S, and Fe following Izotov 2006
          Neon_loc = (0.088d0*lise_met) - 1.450d0
          Neon_loc = ((z_ave * table_Solar_O_loc) * (10.d0**Neon_loc)) * mNe_NIST_amu * Hloc

          Sulfur_loc  = (-0.026d0*lise_met) - 1.514d0
          Sulfur_loc  = ((z_ave * table_Solar_O_loc) * (10.d0**Sulfur_loc))  * mS_NIST_amu  * Hloc

          Iron_loc = (-0.606d0*lise_met) + 2.994d0
          Iron_loc = ((z_ave * table_Solar_O_loc) * (10.d0**Iron_loc)) * mFe_NIST_amu * Hloc


          ! Update C and N following Nicholls 2017
          Nitrogen_loc  = log10(10.d0**(-1.732d0) + 10.d0**(lise_met - 12.d0 + 2.19d0))
          Nitrogen_loc  = ((z_ave * table_Solar_O_loc) * (10.d0**Nitrogen_loc))  * mN_NIST_amu  * Hloc

          Carbon_loc  = log10(10.d0**(-0.8d0) + 10.d0**(lise_met - 12.d00 + 2.72d0))
          Carbon_loc  = ((z_ave * table_Solar_O_loc) * (10.d0**Carbon_loc))  * mC_NIST_amu  * Hloc

          q(i,imetal+0) = Iron_loc        ! Iron
          q(i,imetal+1) = Oxygen_loc      ! Oxygen
          q(i,imetal+2) = Nitrogen_loc    ! Nitrogen
          q(i,imetal+3) = Magnesium_loc   ! Magnesium
          q(i,imetal+4) = Neon_loc        ! Neon
          q(i,imetal+5) = Silicon_loc     ! Silicon
          q(i,imetal+6) = Calcium_loc     ! Calcium
          q(i,imetal+7) = Carbon_loc      ! Carbon
          q(i,imetal+8) = Sulfur_loc      ! Sulfur
        endif
#ifdef RT
        ...
        if(metal) then ! no metals outside of the disk
          q(i,imetal+0) = 0.d0 ! Iron
          q(i,imetal+1) = 0.d0 ! Oxygen
          q(i,imetal+2) = 0.d0 ! Nitrogen
          q(i,imetal+3) = 0.d0 ! Magnesium
          q(i,imetal+4) = 0.d0 ! Neon
          q(i,imetal+5) = 0.d0 ! Silicon
          q(i,imetal+6) = 0.d0 ! Calcium
          q(i,imetal+7) = 0.d0 ! Carbon
          q(i,imetal+8) = 0.d0 ! Sulfur
        endif

        ! assume everything is ionized outside of the disk
#ifdef RT
        q(i,iIons + 0) = 1.d-6        !HI
        q(i,iIons + 1) = 1.d0 - 1.d-6 !HII
        q(i,iIons + 2) = 1.d-6        !HeII
        q(i,iIons + 3) = 1.d0 - 1.d-6 !HeIII

        ! Initialize the metal ionization fractions (start all as ionized)
        if (oxygen_ions) then
          q(i,ioxygen+n_oxygen_ions-1) = 1.d0 - (1.d-10 * (n_oxygen_ions - 1)) !initialize everything as ionized
          do ivar = 0,n_oxygen_ions-2
             q(i,ioxygen + ivar) = 1.d-10
          enddo
       endif
       if (nitrogen_ions) then
          q(i,initrogen+n_nitrogen_ions-1) = 1.d0 - (1.d-10 * (n_nitrogen_ions - 1)) !initialize everything as ionized
          do ivar = 0,n_nitrogen_ions-2
             q(i,initrogen + ivar) = 1.d-10
          enddo
       endif
       if (carbon_ions) then
          q(i,icarbon+n_carbon_ions-1) = 1.d0 - (1.d-10 * (n_carbon_ions - 1)) !initialize everything as ionized
          do ivar = 0,n_carbon_ions-2
             q(i,icarbon + ivar) = 1.d-10
          enddo
       endif
       if (magnesium_ions) then
          q(i,imagnesium+n_magnesium_ions-1) = 1.d0 - (1.d-10 * (n_magnesium_ions - 1)) !initialize everything as ionized
          do ivar = 0,n_magnesium_ions-2
             q(i,imagnesium + ivar) = 1.d-10
          enddo
       endif
       if (silicon_ions) then
         q(i,isilicon+n_silicon_ions-1) = 1.d0 - (1.d-10 * (n_silicon_ions - 1)) !initialize everything as ionized
         do ivar = 0,n_silicon_ions-2
            q(i,isilicon + ivar) = 1.d-10
         enddo
       endif
       if (sulfur_ions) then
         q(i,isulfur+n_sulfur_ions-1) = 1.d0 - (1.d-10 * (n_sulfur_ions - 1)) !initialize everything as ionized
         do ivar = 0,n_sulfur_ions-2
            q(i,isulfur + ivar) = 1.d-10
         enddo
       endif
       if (iron_ions) then
         q(i,iiron+n_iron_ions-1) = 1.d0 - (1.d-10 * (n_iron_ions - 1)) !initialize everything as ionized
         do ivar = 0,n_iron_ions-2
            q(i,iiron + ivar) = 1.d-10
         enddo
       endif
       if (neon_ions) then
         q(i,ineon+n_neon_ions-1) = 1.d0 - (1.d-10 * (n_neon_ions - 1)) !initialize everything as ionized
