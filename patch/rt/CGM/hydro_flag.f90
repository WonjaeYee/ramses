subroutine hydro_flag(ilevel)
  ...
  !if(ilevel.ge.nlevelmax_TK)return
...
#if NVARNOADVECT>0        
        if(cooling.and.cooling_refine(ilevel)>0.0)then
           call cooling_length_refine(ind_cell,ok,ngrid,ilevel)
        endif
#endif
...
subroutine jeans_length_refine(ind_cell,ok,ncell,ilevel)
  ...
  real(dp)::jeans_refine_nH_code

  ! Conversion factor from user units to cgs units
  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)

  jeans_refine_nH_code = jeans_refine_nH/scale_nH
  ...
  do i=1,ncell
     ...
     ! do this only if nH>jeans_refine_nH
     if(dens.gt.jeans_refine_nH_code) then
        ok(i) = ok(i) .or. ( n_jeans*tail_pix >= lamb_jeans )
     endif

     if(ivar_refine>0)then
        ok(i) = ok(i).and.(uold(indi,ivar_refine)/uold(indi,1) > var_cut_refine)
     endif
...
subroutine cooling_length_refine(ind_cell,ok,ncell,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
  ...
  logical,dimension(1:nvector)::ok
  !-------------------------------------------------
  ! This routine sets flag1 to 1 if cell is larger than 
  ! N times the cooling length
  ! -----------------------------------------------

  integer::i,indi,irad
  real(dp)::cs, cooling_length, n_cooling, cell_size, dens, pth

  if(verbose .and. myid==1) write(*,*) 'Cooling length refine for level ',ilevel

  ! User defined number of cells to resolve the cooling length
  n_cooling = cooling_refine(ilevel)

  ! Compute the length of the pixel
  cell_size = boxlen / (2.d0)**ilevel
  ...
  do i=1,ncell
    ...
    ! Uold ndim+2 here is the total specific energy times density 
    ! (conservative variables), which is total pressure up to (gamma - 1.0)
    ! Remove the non-thermal components to get thermal-only pressure
    pth = uold(indi,ndim+2)  
    pth = pth - 0.5d0*uold(indi,2)**2/dens ! Remove kinetic specific energy
#if NDIM > 1
    pth = pth - 0.5d0*uold(indi,3)**2/dens
#endif
#if NDIM > 2
    pth = pth - 0.5d0*uold(indi,4)**2/dens
#endif
#if NENER>0
    do irad=1,nener
       pth=pth-uold(indi,ndim+2+irad) ! Remove non thermal energy
    end do
#endif

    ! Isothermal sound speed is (gamma -1) * Pth / rho    
    cs  = pth * (gamma - 1.0) / dens 
    ...
    cooling_length = cs * unoadvect(indi,cooling_time_ivar) 

    ! Flag cell for refinement if cooling length is not resolved by N cells
    ! Filter negative values to avoid refining on heating cells
    if (cooling_length > 0.0 .and. n_cooling * cell_size >= cooling_length) then
      ok(i) = .true.
    endif

    ! If refining on other passive variable, make sure this is also captured
    if(ivar_refine>0)then
      ok(i) = ok(i).and.(uold(indi, ivar_refine) / uold(indi, 1) > var_cut_refine)
    endif
  end do ! End loop over cells

end subroutine cooling_length_refine
