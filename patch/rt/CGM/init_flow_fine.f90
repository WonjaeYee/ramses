...
subroutine init_flow  
  use amr_commons
  use hydro_commons, ONLY: nvar, uold, imetal
  implicit none
...
#ifdef RT
  use rt_parameters, only: ixHII, ixHI, iIons
#endif
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
     ...
     do ivar=1,nvar
        ...
        if(ok_file3)then
           ...
           if(ncache>0)then
              ...
              if(cosmo.and.ivar.eq.imetal+0) init_array = z_ave*table_Solar_Fe ! Iron
              if(cosmo.and.ivar.eq.imetal+1) init_array = z_ave*table_Solar_O  ! Oxygen
              if(cosmo.and.ivar.eq.imetal+2) init_array = z_ave*table_Solar_N  ! Nitrogen
              if(cosmo.and.ivar.eq.imetal+3) init_array = z_ave*table_Solar_Mg ! Magnesium
              if(cosmo.and.ivar.eq.imetal+4) init_array = z_ave*table_Solar_Ne ! Neon
              if(cosmo.and.ivar.eq.imetal+5) init_array = z_ave*table_Solar_Si ! Silicon
              if(cosmo.and.ivar.eq.imetal+6) init_array = z_ave*table_Solar_Ca ! Calcium
              if(cosmo.and.ivar.eq.imetal+7) init_array = z_ave*table_Solar_C  ! Carbon
              if(cosmo.and.ivar.eq.imetal+8) init_array = z_ave*table_Solar_S  ! Sulfur
...
#ifdef RT
              if(cosmo)then
                 if((ixHII>0).and.(ivar==(iIons+ixHII-1))) init_array=1.2d-5*xval
                 if((ixHI >0).and.(ivar==(iIons+ixHI-1))) init_array=1d0-1.2d-5*xval
              else
                 if((ixHI >0).and.(ivar==(iIons+ixHI-1))) init_array=1d0
              endif

              ! Initialize the metal ionization fractions (start all as neutral)
              if (oxygen_ions) then
                 if (init_metal_ions_all_ionized) then
                    if (ivar.ge.ioxygen.and.ivar.lt.ioxygen+n_oxygen_ions) init_array = 1.d-10 
                    if (ivar.eq.ioxygen+n_oxygen_ions-1) init_array = 1.d0 - (1.d-10 * (n_oxygen_ions - 1))
                 else
                    if (ivar.eq.ioxygen) init_array = 1.d0 - (1.d-10 * (n_oxygen_ions - 1))
                    if (ivar.gt.ioxygen.and.ivar.lt.ioxygen+n_oxygen_ions) init_array = 1.d-10
                 end if
              endif
              if (nitrogen_ions) then
                 if (init_metal_ions_all_ionized) then
                    if (ivar.ge.initrogen.and.ivar.lt.initrogen+n_nitrogen_ions) init_array = 1.d-10
                    if (ivar.eq.initrogen+n_nitrogen_ions-1) init_array = 1.d0 - (1.d-10 * (n_nitrogen_ions - 1))
                 else
                    if (ivar.eq.initrogen) init_array = 1.d0 - (1.d-10 * (n_nitrogen_ions - 1))
                    if (ivar.gt.initrogen.and.ivar.lt.initrogen+n_nitrogen_ions) init_array = 1.d-10
                 end if
              endif
              if (carbon_ions) then
                 if (init_metal_ions_all_ionized) then
                    if (ivar.ge.icarbon.and.ivar.lt.icarbon+n_carbon_ions) init_array = 1.d-10
                    if (ivar.eq.icarbon+n_carbon_ions-1) init_array = 1.d0 - (1.d-10 * (n_carbon_ions - 1))
                 else
                    if (ivar.eq.icarbon) init_array = 1.d0 - (1.d-10 * (n_carbon_ions - 1))
                    if (ivar.gt.icarbon.and.ivar.lt.icarbon+n_carbon_ions) init_array = 1.d-10
                 end if
              endif
              if (magnesium_ions) then
                 if (init_metal_ions_all_ionized) then
                    if (ivar.ge.imagnesium.and.ivar.lt.imagnesium+n_magnesium_ions) init_array = 1.d-10
                    if (ivar.eq.imagnesium+n_magnesium_ions-1) init_array = 1.d0 - (1.d-10 * (n_magnesium_ions - 1))
                 else
                    if (ivar.eq.imagnesium) init_array = 1.d0 - (1.d-10 * (n_magnesium_ions - 1))
                    if (ivar.gt.imagnesium.and.ivar.lt.imagnesium+n_magnesium_ions) init_array = 1.d-10
                 end if
              endif
              if (silicon_ions) then
                 if (init_metal_ions_all_ionized) then
                    if (ivar.ge.isilicon.and.ivar.lt.isilicon+n_silicon_ions) init_array = 1.d-10
                    if (ivar.eq.isilicon+n_silicon_ions-1) init_array = 1.d0 - (1.d-10 * (n_silicon_ions - 1))
                 else
                    if (ivar.eq.isilicon) init_array = 1.d0 - (1.d-10 * (n_silicon_ions - 1))
                    if (ivar.gt.isilicon.and.ivar.lt.isilicon+n_silicon_ions) init_array = 1.d-10
                 end if
              endif
              if (sulfur_ions) then
                 if (init_metal_ions_all_ionized) then
                    if (ivar.ge.isulfur.and.ivar.lt.isulfur+n_sulfur_ions) init_array = 1.d-10
                    if (ivar.eq.isulfur+n_sulfur_ions-1) init_array = 1.d0 - (1.d-10 * (n_sulfur_ions - 1))
                 else
                    if (ivar.eq.isulfur) init_array = 1.d0 - (1.d-10 * (n_sulfur_ions - 1))
                    if (ivar.gt.isulfur.and.ivar.lt.isulfur+n_sulfur_ions) init_array = 1.d-10
                 end if
              endif
              if (iron_ions) then
                 if (init_metal_ions_all_ionized) then
                    if (ivar.ge.iiron.and.ivar.lt.iiron+n_iron_ions) init_array = 1.d-10
                    if (ivar.eq.iiron+n_iron_ions-1) init_array = 1.d0 - (1.d-10 * (n_iron_ions - 1))
                 else
                    if (ivar.eq.iiron) init_array = 1.d0 - (1.d-10 * (n_iron_ions - 1))
                    if (ivar.gt.iiron.and.ivar.lt.iiron+n_iron_ions) init_array = 1.d-10
                 end if
              endif
              if (neon_ions) then
                 if (init_metal_ions_all_ionized) then
                    if (ivar.ge.ineon.and.ivar.lt.ineon+n_neon_ions) init_array = 1.d-10
                    if (ivar.eq.ineon+n_neon_ions-1) init_array = 1.d0 - (1.d-10 * (n_neon_ions - 1))
                 else
                    if (ivar.eq.ineon) init_array = 1.d0 - (1.d-10 * (n_neon_ions - 1))
                    if (ivar.gt.ineon.and.ivar.lt.ineon+n_neon_ions) init_array = 1.d-10
                 end if
              endif
#endif
           endif
