...
module cooling_module
  ...
  real(kind=8),parameter:: T2_min_fix=2.725d0
  ...
  type cooling_table
     ...
     real(kind=8),dimension(:,:)  ,pointer::metal_heat       ! Harley/grackle
     real(kind=8),dimension(:,:)  ,pointer::metal_heat_prime ! Harley/grackle
 ...
 &                       -0.15438891584271634D-002 /), (/Norderweinberg,6/) )
  ...
  ! Harley input for metal cooling from Grackle
  integer(kind=4) :: nbin_T_cloudy, nbin_n_cloudy, nbin_z_cloudy
  real(dp),allocatable, dimension(:) :: temp_cloudy !should be uniformly sampled
  real(dp),allocatable, dimension(:) :: dens_cloudy !should be uniformly sampled
  real(dp),allocatable, dimension(:) :: zred_cloudy !irregular sampling ok 
  real(dp),allocatable, dimension(:,:,:) :: met_cool_tab
  real(dp),allocatable, dimension(:,:,:) :: met_heat_tab
  real(dp),allocatable, dimension(:,:,:) :: met_cool_prime_tab
  real(dp),allocatable, dimension(:,:,:) :: met_heat_prime_tab
  real(dp):: taux_HM_HI,taux_HM_HeI,taux_HM_HeII
  real(dp):: heat_HM_HI,heat_HM_HeI,heat_HM_HeII

  ! Harley input for ion-by-ion metal cooling
  real(dp),allocatable, dimension(:)   :: met_cool_tab_temp
  real(dp),allocatable, dimension(:,:) :: met_cool_tab_iron
  real(dp),allocatable, dimension(:,:) :: met_cool_tab_oxygen
  real(dp),allocatable, dimension(:,:) :: met_cool_tab_nitrogen
  real(dp),allocatable, dimension(:,:) :: met_cool_tab_carbon
  real(dp),allocatable, dimension(:,:) :: met_cool_tab_neon
  real(dp),allocatable, dimension(:,:) :: met_cool_tab_calcium
  real(dp),allocatable, dimension(:,:) :: met_cool_tab_silicon
  real(dp),allocatable, dimension(:,:) :: met_cool_tab_sulfur
  real(dp),allocatable, dimension(:,:) :: met_cool_tab_magnesium
...
!=======================================================================
  ...
  if (cloudy_metal_cooling)then
     write(10)table%metal_heat
     write(10)table%metal_heat_prime
  endif
  close(10)
...
!=======================================================================
  ...
  real(kind=8)::metal_heat,metal_heat_prime
  ...
  do while(n>0)
     ...
     do i=1,n
        ...
        if(facT.le.logT2max)then
           ...
           if(cloudy_metal_cooling)then
              ! Metal heating 
              fa=table%metal_heat(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table%metal_heat(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
              fb=table%metal_heat(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table%metal_heat(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
              fprimea=table%metal_heat_prime(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table%metal_heat_prime(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
              fprimeb=table%metal_heat_prime(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table%metal_heat_prime(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
              alpha=fprimea
              beta=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h
              gamma=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
              metal_heat=10d0**(fa+alpha*yy+beta*yy2+gamma*yy3)
              metal_heat_prime=metal_heat/tau(ind(i))*(alpha+2d0*beta*yy+3d0*gamma*yy2)
           else
              metal_heat=0d0
           ...
           ! Total net cooling
           lambda=cool+zzz(ind(i))*(metal-metal_heat)-heat+(cool_com-heat_com)/nH(ind(i))
           lambda_prime=cool_prime+zzz(ind(i))*(metal_prime-metal_heat_prime)-heat_prime+(cool_com_prime-heat_com_prime)/nH(ind(i))
...
!=======================================================================
  ...
  if(.not.first)then
     ...
     if (cloudy_metal_cooling) then ! Harley/grackle
        deallocate(table%metal_heat)       
        deallocate(table%metal_heat_prime)
     endif
  else
     first=.false.
  endif
  ...
  if (cloudy_metal_cooling) then ! Harley/grackle
     allocate(table%metal_heat(nbin_n,nbin_T)) 
     allocate(table%metal_heat_prime(nbin_n,nbin_T))
  endif
#ifndef WITHOUTMPI
  ...
  if (cloudy_metal_cooling) then ! Harley/grackle
     allocate(table2%metal_heat(nbin_n,nbin_T)) 
     allocate(table2%metal_heat_prime(nbin_n,nbin_T)) 
  endif
#endif  
  do i_n=1,nbin_n
  ...
  if (cloudy_metal_cooling) then ! Harley/grackle
     table%metal_heat=0.0 
     table%metal_heat_prime=0.0
  endif
  ...
  if (cloudy_metal_cooling)then ! Harley/grackle
     call MPI_ALLREDUCE(table%metal_heat,table2%metal_heat,nbin_n*nbin_T,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)            
     call MPI_ALLREDUCE(table%metal_heat_prime,table2%metal_heat_prime,nbin_n*nbin_T,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
  endif
  ...
  if (cloudy_metal_cooling)then ! Harley/grackle
     table%metal_heat = table2%metal_heat
     table%metal_heat_prime = table2%metal_heat_prime
  endif
#endif
  ...
  if (cloudy_metal_cooling)then ! Harley/grackle
     deallocate(table2%metal_heat)
     deallocate(table2%metal_heat_prime)
  endif
#endif
...
!=======================================================================
  ...
  elseif (HM12) then ! Harley/grackle
     call taux_rad_HM12(z)
     call heat_rad_HM12(z)
     t_rad_spec(HI  ) = taux_HM_HI
     t_rad_spec(HEI ) = taux_HM_HeI
     t_rad_spec(HEII) = taux_HM_HeII
     h_rad_spec(HI  ) = heat_HM_HI
     h_rad_spec(HEI ) = heat_HM_HeI
     h_rad_spec(HEII) = heat_HM_HeII
  endif  
...
!=======================================================================
  ...
  real(kind=8) ::diff,metal_heat,metal_heat_prime
  integer::niter
  ...
  do i_T = 1,nbin_T
     ...
     if(cloudy_metal_cooling)then ! Harley/grackle
        ...
        table%metal_heat(i_n,i_T)=log10(metal_heat)
        table%metal_heat_prime(i_n,i_T)=metal_heat_prime
     else
...
!=======================================================================
  ...
  ! Compton heating
  if(cloudy_metal_cooling)then
     coh = heat_compton_HM12(T,aexp)*n_E/nH
  else
...
!=======================================================================
subroutine init_OS13_metal_cooling
!=======================================================================
   ! Interface to the ion-by-ion cooling from Oppenheimer & Schaye 2013
   ! The idea is that RAMSES-RTZ follows only a certain number of ions 
   ! For the neq ions that are followed, we use the neq tables
   ! However, for the ions that we don't follow, we would either have 
   ! to use the neq tables and compute ionisation equilibrium at every 
   ! time step which is expensive, or we subtract off the ions that we 
   ! follow from the equilibrium tables first and then use these.
   ! Unfortunately, this means that we have to recompute the eq tables
   ! for every new simulation if we change the ions that we follow. I 
   ! have supplied python code for doing this but one will first have to 
   ! download the equilibrium tables from https://noneq.strw.leidenuniv.nl/#otherlinks 
   ! as they are too large to include in the git.
   use hydro_parameters, only:nmetals
   implicit none
   integer::i,ntemp=176
   logical::ok
   character(LEN=256):: temp_file_neq, cie_file
   character(LEN=256),dimension(1:nmetals)::metal_cooling_neq
   
   ! Non equilibrium cooling files
   temp_file_neq       = trim(os13_cooling_dir)//'temperatures.dat'
   ! Metal Enrichment: Hardcoded. 1=Fe, 2=O, 3=N, 4=Mg, 5=Ne, 6=Si, 7=Ca, 8=C, 9=S
   metal_cooling_neq(1) = trim(os13_cooling_dir)//'iron_cooling.dat'
   metal_cooling_neq(2) = trim(os13_cooling_dir)//'oxygen_cooling.dat'
   metal_cooling_neq(3) = trim(os13_cooling_dir)//'nitrogen_cooling.dat'
   metal_cooling_neq(4) = trim(os13_cooling_dir)//'magnesium_cooling.dat'
   metal_cooling_neq(5) = trim(os13_cooling_dir)//'neon_cooling.dat'
   metal_cooling_neq(6) = trim(os13_cooling_dir)//'silicon_cooling.dat'
   metal_cooling_neq(7) = trim(os13_cooling_dir)//'calcium_cooling.dat'
   metal_cooling_neq(8) = trim(os13_cooling_dir)//'carbon_cooling.dat'
   metal_cooling_neq(9) = trim(os13_cooling_dir)//'sulfur_cooling.dat'
   ! CIE  files
   cie_file             = trim(os13_cooling_dir)//'CIE.dat'

   ! Check that all files exist
   inquire(FILE=temp_file_neq,exist=ok)
   if(.not.ok)then
      write(*,*) 'Cannot access OS13 neq temp file '//TRIM(temp_file_neq)
      call clean_stop
   endif
   do i = 1,nmetals
      inquire(FILE=metal_cooling_neq(i),exist=ok)
      if(.not.ok)then
         write(*,*) 'Cannot access OS13 neq metal file '//TRIM(metal_cooling_neq(i))
         call clean_stop
      endif
   end do
   ...
   ! Allocate the arrays
   allocate(met_cool_tab_temp(1:ntemp))
   allocate(met_cool_tab_iron(1:27,1:ntemp))
   allocate(met_cool_tab_oxygen(1:9,1:ntemp))
   allocate(met_cool_tab_nitrogen(1:8,1:ntemp))
   allocate(met_cool_tab_magnesium(1:13,1:ntemp))
   allocate(met_cool_tab_neon(1:11,1:ntemp))
   allocate(met_cool_tab_silicon(1:15,1:ntemp))
   allocate(met_cool_tab_calcium(1:21,1:ntemp))
   allocate(met_cool_tab_carbon(1:7,1:ntemp))
   allocate(met_cool_tab_sulfur(1:17,1:ntemp))
   allocate(met_cool_tab_cie(1:201,1:113))


   ! Read in the temperature
   open(unit=880,file=temp_file_neq,form='formatted')
   read(880,*) met_cool_tab_temp(:)
   close(880)

   ! Read in the metals
   open(unit=880,file=metal_cooling_neq(1),form='formatted')
   do i = 1,27 ! Iron
      read(880,*) met_cool_tab_iron(i,:)
   end do
   close(880)

   open(unit=880,file=metal_cooling_neq(2),form='formatted')
   do i = 1,9 ! Oxygen
      read(880,*) met_cool_tab_oxygen(i,:)
   end do
   close(880)   

   open(unit=880,file=metal_cooling_neq(3),form='formatted')
   do i = 1,8 ! Nitrogen
      read(880,*) met_cool_tab_nitrogen(i,:)
   end do
   close(880)

   open(unit=880,file=metal_cooling_neq(4),form='formatted')
   do i = 1,13 ! Magnesium
      read(880,*) met_cool_tab_magnesium(i,:)
   end do
   close(880)

   open(unit=880,file=metal_cooling_neq(5),form='formatted')
   do i = 1,11 ! Neon
      read(880,*) met_cool_tab_neon(i,:)
   end do
   close(880)

   open(unit=880,file=metal_cooling_neq(6),form='formatted')
   do i = 1,15 ! Silicon
      read(880,*) met_cool_tab_silicon(i,:)
   end do
   close(880)

   open(unit=880,file=metal_cooling_neq(7),form='formatted')
   do i = 1,21 ! Calcium
      read(880,*) met_cool_tab_calcium(i,:)
   end do
   close(880)

   open(unit=880,file=metal_cooling_neq(8),form='formatted')
   do i = 1,7 ! Carbon
      read(880,*) met_cool_tab_carbon(i,:)
   end do
   close(880)

   open(unit=880,file=metal_cooling_neq(9),form='formatted')
   do i = 1,17 ! Sulfur
      read(880,*) met_cool_tab_sulfur(i,:)
   end do
   close(880)
...
end subroutine init_OS13_metal_cooling
!=======================================================================
subroutine cmp_metals_os13(TK,nH,ne,nFe,nO,nN,nMg,nNe,nSi,nC,nS,xFe,xO,xN,xMg,xNe,xSi,xC,xS,cool_tot)
!=======================================================================
   ...
   ! INPUT
   real(dp)::TK,nH,ne,nFe,nO,nN,nMg,nNe,nSi,nC,nS,cool_tot
   ...
   if (log10T.le.met_cool_tab_temp(1)) then
      IT_os13 = 1
   else if (log10T.ge.met_cool_tab_temp(ntemp-1)) then
      IT_os13 = ntemp-1 ! always 1 less than max
   else
      IT_os13 = 1 + INT( float(ntemp) * (log10T - met_cool_tab_temp(1)) / (met_cool_tab_temp(ntemp) - met_cool_tab_temp(1)))
   endif
   ...
   if (log10T.le.met_cool_tab_cie(1,1)) then
      ...
      IT_CIE = 1 + INT( 201.d0 * (log10T - met_cool_tab_cie(1,1)) / (met_cool_tab_cie(201,1) - met_cool_tab_cie(1,1)))
   endif
   
   ! Prepare the interpolation (1D)
   deltaT  = met_cool_tab_temp(IT_os13 +1)-met_cool_tab_temp(IT_os13)
   frac_low = MIN(MAX((met_cool_tab_temp(IT_os13 +1) - log10T) / deltaT,0.d0),1.d0)
   frac_high = 1.d0 - frac_low

   deltaT  = met_cool_tab_cie(IT_CIE +1,1)-met_cool_tab_cie(IT_CIE,1)
   frac_low_cie = MIN(MAX((met_cool_tab_cie(IT_CIE +1,1) - log10T) / deltaT,0.d0),1.d0)
   ...
   if (iron_ions) then
      do i = 1,n_iron_ions-1
         log_cool = (met_cool_tab_iron(i,IT_os13)*frac_low) + (met_cool_tab_iron(i,IT_os13+1)*frac_high)
         cool_tot_Fe = cool_tot_Fe + ((10.d0**log_cool) * ne * nFe * xFe(i)) ! Now in erg / s / cm^3
      end do
   endif
   if (oxygen_ions) then
      do i = 1,n_oxygen_ions-1
         log_cool = (met_cool_tab_oxygen(i,IT_os13)*frac_low) + (met_cool_tab_oxygen(i,IT_os13+1)*frac_high)
         cool_tot_O = cool_tot_O + ((10.d0**log_cool) * ne * nO * xO(i)) ! Now in erg / s / cm^3
      end do
   endif
   if (nitrogen_ions) then
      do i = 1,n_nitrogen_ions-1
         log_cool = (met_cool_tab_nitrogen(i,IT_os13)*frac_low) + (met_cool_tab_nitrogen(i,IT_os13+1)*frac_high)
         cool_tot_N = cool_tot_N + ((10.d0**log_cool) * ne * nN * xN(i)) ! Now in erg / s / cm^3
      end do
   endif
   if (carbon_ions) then
      do i = 1,n_carbon_ions-1
         log_cool = (met_cool_tab_carbon(i,IT_os13)*frac_low) + (met_cool_tab_carbon(i,IT_os13+1)*frac_high)
         cool_tot_C = cool_tot_C + ((10.d0**log_cool) * ne * nC * xC(i)) ! Now in erg / s / cm^3
      end do
   endif
   if (magnesium_ions) then
      do i = 1,n_magnesium_ions-1
         log_cool = (met_cool_tab_magnesium(i,IT_os13)*frac_low) + (met_cool_tab_magnesium(i,IT_os13+1)*frac_high)
         cool_tot_Mg = cool_tot_Mg + ((10.d0**log_cool) * ne * nMg * xMg(i)) ! Now in erg / s / cm^3
      end do
   endif
   if (neon_ions) then
      do i = 1,n_neon_ions-1
         log_cool = (met_cool_tab_neon(i,IT_os13)*frac_low) + (met_cool_tab_neon(i,IT_os13+1)*frac_high)
         cool_tot_Ne = cool_tot_Ne + ((10.d0**log_cool) * ne * nNe * xNe(i)) ! Now in erg / s / cm^3
      end do
   endif
   if (sulfur_ions) then
      do i = 1,n_sulfur_ions-1
         log_cool = (met_cool_tab_sulfur(i,IT_os13)*frac_low) + (met_cool_tab_sulfur(i,IT_os13+1)*frac_high)
         cool_tot_S = cool_tot_S + ((10.d0**log_cool) * ne * nS * xS(i)) ! Now in erg / s / cm^3
      end do
   endif
   if (silicon_ions) then
      do i = 1,n_silicon_ions-1
         log_cool = (met_cool_tab_silicon(i,IT_os13)*frac_low) + (met_cool_tab_silicon(i,IT_os13+1)*frac_high)
         cool_tot_Si = cool_tot_Si + ((10.d0**log_cool) * ne * nSi * xSi(i)) ! Now in erg / s / cm^3
      end do
   endif
   
   ! Now handle the ions that we need CIE
   if (iron_ions) then
      if (n_iron_ions.lt.27) then ! Only compute if there are states that we don't account for
         ...
         if (cie_tot.gt.min_frac_cie) then
            do i = n_iron_ions,27
               ...
               cool_tot_Fe = cool_tot_Fe + ((10.d0**log_cool) * ne * nFe * xFe(n_iron_ions) * cie_loc) ! Now in erg / s / cm^3
            
            end do
         end if

      else if (n_iron_ions.eq.27) then

         log_cool = (met_cool_tab_iron(27,IT_os13)*frac_low) + (met_cool_tab_iron(27,IT_os13+1)*frac_high)
         cool_tot_Fe = cool_tot_Fe + ((10.d0**log_cool) * ne * nFe * xFe(n_iron_ions))

      endif
   endif

   if (oxygen_ions) then
      if (n_oxygen_ions.lt.9) then ! Only compute if there are states that we don't account for
         ...
         if (cie_tot.gt.min_frac_cie) then
            do i = n_oxygen_ions,9
               ...
               log_cool = (met_cool_tab_oxygen(i,IT_os13)*frac_low) + (met_cool_tab_oxygen(i,IT_os13+1)*frac_high)
               cool_tot_O = cool_tot_O + ((10.d0**log_cool) * ne * nO * xO(n_oxygen_ions) * cie_loc) ! Now in erg / s / cm^3
            end do
         end if

      else if (n_oxygen_ions.eq.9) then

         log_cool = (met_cool_tab_oxygen(9,IT_os13)*frac_low) + (met_cool_tab_oxygen(9,IT_os13+1)*frac_high)
         cool_tot_O = cool_tot_O + ((10.d0**log_cool) * ne * nO * xO(n_oxygen_ions))

      endif
   endif

   if (nitrogen_ions) then
      if (n_nitrogen_ions.lt.9) then ! Only compute if there are states that we don't account for
         ...
         if (cie_tot.gt.min_frac_cie) then
            do i = n_nitrogen_ions,8
               ...
               log_cool = (met_cool_tab_nitrogen(i,IT_os13)*frac_low) + (met_cool_tab_nitrogen(i,IT_os13+1)*frac_high)
               cool_tot_N = cool_tot_N + ((10.d0**log_cool) * ne * nN * xN(n_nitrogen_ions) * cie_loc) ! Now in erg / s / cm^3
            end do
         end if

      else if (n_nitrogen_ions.eq.8) then

         log_cool = (met_cool_tab_nitrogen(8,IT_os13)*frac_low) + (met_cool_tab_nitrogen(8,IT_os13+1)*frac_high)
         cool_tot_N = cool_tot_N + ((10.d0**log_cool) * ne * nN * xN(n_nitrogen_ions))

      endif
   endif

   if (carbon_ions) then
      if (n_carbon_ions.lt.7) then ! Only compute if there are states that we don't account for
         ...
         if (cie_tot.gt.min_frac_cie) then
            do i = n_carbon_ions,7
               ...
               log_cool = (met_cool_tab_carbon(i,IT_os13)*frac_low) + (met_cool_tab_carbon(i,IT_os13+1)*frac_high)
               cool_tot_C = cool_tot_C + ((10.d0**log_cool) * ne * nC * xC(n_carbon_ions) * cie_loc) ! Now in erg / s / cm^3
            end do
         end if

      else if (n_carbon_ions.eq.7) then

         log_cool = (met_cool_tab_carbon(7,IT_os13)*frac_low) + (met_cool_tab_carbon(7,IT_os13+1)*frac_high)
         cool_tot_C = cool_tot_C + ((10.d0**log_cool) * ne * nC * xC(n_carbon_ions))

      endif
   endif

   if (magnesium_ions) then
      if (n_magnesium_ions.lt.13) then ! Only compute if there are states that we don't account for
         ...
         if (cie_tot.gt.min_frac_cie) then
            do i = n_magnesium_ions,13
               ...
               log_cool = (met_cool_tab_magnesium(i,IT_os13)*frac_low) + (met_cool_tab_magnesium(i,IT_os13+1)*frac_high)
               cool_tot_Mg = cool_tot_Mg + ((10.d0**log_cool) * ne * nMg * xMg(n_magnesium_ions) * cie_loc) ! Now in erg / s / cm^3
            end do
         end if

      else if (n_magnesium_ions.eq.13) then

         log_cool = (met_cool_tab_magnesium(13,IT_os13)*frac_low) + (met_cool_tab_magnesium(13,IT_os13+1)*frac_high)
         cool_tot_Mg = cool_tot_Mg + ((10.d0**log_cool) * ne * nMg * xMg(n_magnesium_ions))

      endif
   endif

   if (neon_ions) then
      if (n_neon_ions.lt.11) then ! Only compute if there are states that we don't account for
         ...
         if (cie_tot.gt.min_frac_cie) then
            do i = n_neon_ions,11
               ...
               log_cool = (met_cool_tab_neon(i,IT_os13)*frac_low) + (met_cool_tab_neon(i,IT_os13+1)*frac_high)
               cool_tot_Ne = cool_tot_Ne + ((10.d0**log_cool) * ne * nNe * xNe(n_neon_ions) * cie_loc) ! Now in erg / s / cm^3
            end do
         end if

      else if (n_neon_ions.eq.11) then

         log_cool = (met_cool_tab_neon(11,IT_os13)*frac_low) + (met_cool_tab_neon(11,IT_os13+1)*frac_high)
         cool_tot_Ne = cool_tot_Ne + ((10.d0**log_cool) * ne * nNe * xNe(n_neon_ions))

      endif
   endif

   if (silicon_ions) then
      if (n_silicon_ions.lt.15) then ! Only compute if there are states that we don't account for
         ...
         if (cie_tot.gt.min_frac_cie) then
            do i = n_silicon_ions,15
               ...
               log_cool = (met_cool_tab_silicon(i,IT_os13)*frac_low) + (met_cool_tab_silicon(i,IT_os13+1)*frac_high)
               cool_tot_Si = cool_tot_Si + ((10.d0**log_cool) * ne * nSi * xSi(n_silicon_ions) * cie_loc) ! Now in erg / s / cm^3
            end do
         end if

      else if (n_silicon_ions.eq.15) then

         log_cool = (met_cool_tab_silicon(15,IT_os13)*frac_low) + (met_cool_tab_silicon(15,IT_os13+1)*frac_high)
         cool_tot_Si = cool_tot_Si + ((10.d0**log_cool) * ne * nSi * xSi(n_silicon_ions))

      endif
   endif

   if (sulfur_ions) then
      if (n_sulfur_ions.lt.17) then ! Only compute if there are states that we don't account for
         ...
         if (cie_tot.gt.min_frac_cie) then
            do i = n_sulfur_ions,17
               ...
               log_cool = (met_cool_tab_sulfur(i,IT_os13)*frac_low) + (met_cool_tab_sulfur(i,IT_os13+1)*frac_high)
               cool_tot_S = cool_tot_S + ((10.d0**log_cool) * ne * nS * xS(n_sulfur_ions) * cie_loc) ! Now in erg / s / cm^3
            end do
         end if

      else if (n_sulfur_ions.eq.17) then

         log_cool = (met_cool_tab_sulfur(17,IT_os13)*frac_low) + (met_cool_tab_sulfur(17,IT_os13+1)*frac_high)
         cool_tot_S = cool_tot_S + ((10.d0**log_cool) * ne * nS * xS(n_sulfur_ions))
   ...
   if(isnan(cool_tot))then !if NaN 
      write(*,*) ' Nan error in OS13_cooling'
...
end subroutine cmp_metals_os13
!=======================================================================
subroutine init_cloudy_metal_cooling
!=======================================================================
! Send email to Harley Katz for any issues
! Read a metal cooling table computed from Grackle 
! irradiated by some UV spectrum
  implicit none
  logical::ok
  integer::i,j,k
  real(kind=8), allocatable :: xx(:)
  inquire(FILE=cloudy_metal_file,exist=ok)
  if(.not.ok)then
     write(*,*) 'Cannot access Grackle metal file '//TRIM(cloudy_metal_file)
     call clean_stop
  endif

  open(unit=880,file=cloudy_metal_file,form='unformatted')

  ! read the number of elements in each array
  read(880) nbin_T_cloudy, nbin_n_cloudy, nbin_z_cloudy

  allocate(xx(1:nbin_T_cloudy))
  allocate(temp_cloudy(1:nbin_T_cloudy))
  allocate(dens_cloudy(1:nbin_n_cloudy))
  ...
  met_cool_tab=0.0d0
  met_heat_tab=0.0d0
  met_cool_prime_tab=0.0d0
  met_heat_prime_tab=0.0d0

  ! read temperature grid
  read(880) temp_cloudy

  ! read density grid
  read(880) dens_cloudy
  ...
  ! read metal cooling prime
  do j=1,nbin_z_cloudy
     do i=1,nbin_n_cloudy
        read(880) xx
        met_cool_prime_tab(:,i,j)=xx
     end do
  end do 
  ...
  ! read metal heating prime
  do j=1,nbin_z_cloudy
     do i=1,nbin_n_cloudy
        read(880) xx
        met_heat_prime_tab(:,i,j)=xx
     end do
  end do 
...
!=======================================================================
subroutine cmp_metals_cloudy(T2,nH,mu,metal_tot,metal_prime,metal_heat,metal_heat_prime,aexp)
!=======================================================================
  ! Compute cooling enhancement due to metals
  ! Cloudy at solar metalicity
  ! Harley borrowed this from enzo GRACKLE cooling because it takes into account
  ! the effect of the haardt madau UV background on the metal cooling
  implicit none
  real(kind=8)::T2,nH,mu,metal_tot,metal_prime,aexp,lnH
  real(kind=8)::metal_heat,metal_heat_prime
  real(kind=8)::TT,lTT,deltaT,deltanH,ZZ,deltaZ
  real(kind=8)::lcool1,lcool1_prime,lheat1,lheat1_prime
  real(kind=8)::lcool2,lcool2_prime,lheat2,lheat2_prime
  real(kind=8):: x2x1,y2y1,x2x,xx1,y2y,yy1,fQ11,fQ21,fQ12,fQ22
  real(kind=8):: fracZ,lnHmax
  integer::iT,inH,iZ
  logical::ok

  ZZ=max(1d0/aexp-1d0,0d0)
  TT=T2*mu
  lTT=log10(TT)
  lnH=log10(nH)
  ...
  ! find the index for redshift
  iZ = nbin_z_cloudy - 1
  do while (ZZ .le. zred_cloudy(iZ))
     iZ = iZ - 1
     if (iZ==1) exit
  end do
  fracZ = (zred_cloudy(iZ+1)-ZZ)/(zred_cloudy(iZ+1)-zred_cloudy(iZ))
  fracZ = min(1d0,max(0d0,fracZ)) ! do not allow extrapolation

  
  ! No extrapolation for Temperature
  ok = .true.
  if (lTT.lt.temp_cloudy(1)) ok=.false.
  if (lTT.gt.temp_cloudy(nbin_T_cloudy)) ok=.false.

  ! No extrapolation for density (this was necessary because metal_heat_tab varies a lot around boundaries)
  if (lnH.gt.dens_cloudy(nbin_n_cloudy)) lnH = dens_cloudy(nbin_n_cloudy)

  if(ok)then
     iT = 1+int((lTT-temp_cloudy(1))/(temp_cloudy(nbin_T_cloudy)-temp_cloudy(1))*(nbin_T_cloudy-1))
     inH= 1+int((lnH-dens_cloudy(1))/(dens_cloudy(nbin_n_cloudy)-dens_cloudy(1))*(nbin_n_cloudy-1)) 
     iT = max(1,min(iT, nbin_T_cloudy-1))
     inH= max(1,min(inH,nbin_n_cloudy-1))
 
     deltaT  = temp_cloudy(iT +1)-temp_cloudy(iT )
     deltanH = dens_cloudy(inH+1)-dens_cloudy(inH)
 
     ! here we are performing a trilinear interpolation over the 3d grid
     x2x1 = deltaT
     y2y1 = deltanH
     x2x = temp_cloudy(iT+1) - lTT
     xx1 = lTT - temp_cloudy(iT)
     y2y = dens_cloudy(inH+1) - lnH
     yy1 = lnH - dens_cloudy(inH)
     
     ! cooling 
     fQ11 =  met_cool_tab (iT  ,inH  ,iZ  )
     fQ12 =  met_cool_tab (iT  ,inH+1,iZ  ) 
     fQ21 =  met_cool_tab (iT+1,inH  ,iZ  )
     fQ22 =  met_cool_tab (iT+1,inH+1,iZ  )
     lcool1 = (1.0d0/(x2x1*y2y1)) * ((fQ11*x2x*y2y) + (fQ21*xx1*y2y) + (fQ12*x2x*yy1) + (fQ22*xx1*yy1))

     fQ11 =  met_cool_tab (iT  ,inH  ,iZ+1)
     fQ12 =  met_cool_tab (iT  ,inH+1,iZ+1) 
     fQ21 =  met_cool_tab (iT+1,inH  ,iZ+1)
     fQ22 =  met_cool_tab (iT+1,inH+1,iZ+1)
     lcool2 = (1.0d0/(x2x1*y2y1)) * ((fQ11*x2x*y2y) + (fQ21*xx1*y2y) + (fQ12*x2x*yy1) + (fQ22*xx1*yy1))

     ! cooling derivative
     fQ11 =  met_cool_prime_tab (iT  ,inH  ,iZ  )
     fQ12 =  met_cool_prime_tab (iT  ,inH+1,iZ  ) 
     fQ21 =  met_cool_prime_tab (iT+1,inH  ,iZ  )
     fQ22 =  met_cool_prime_tab (iT+1,inH+1,iZ  )
     lcool1_prime = (1.0d0/(x2x1*y2y1)) * ((fQ11*x2x*y2y) + (fQ21*xx1*y2y) + (fQ12*x2x*yy1) + (fQ22*xx1*yy1))

     fQ11 =  met_cool_prime_tab (iT  ,inH  ,iZ+1)
     fQ12 =  met_cool_prime_tab (iT  ,inH+1,iZ+1) 
     fQ21 =  met_cool_prime_tab (iT+1,inH  ,iZ+1)
     fQ22 =  met_cool_prime_tab (iT+1,inH+1,iZ+1)
     lcool2_prime = (1.0d0/(x2x1*y2y1)) * ((fQ11*x2x*y2y) + (fQ21*xx1*y2y) + (fQ12*x2x*yy1) + (fQ22*xx1*yy1))

     ! heating
     fQ11 =  met_heat_tab (iT  ,inH  ,iZ  )
     fQ12 =  met_heat_tab (iT  ,inH+1,iZ  ) 
     fQ21 =  met_heat_tab (iT+1,inH  ,iZ  )
     fQ22 =  met_heat_tab (iT+1,inH+1,iZ  )
     lheat1 = (1.0d0/(x2x1*y2y1)) * ((fQ11*x2x*y2y) + (fQ21*xx1*y2y) + (fQ12*x2x*yy1) + (fQ22*xx1*yy1))

     fQ11 =  met_heat_tab (iT  ,inH  ,iZ+1)
     fQ12 =  met_heat_tab (iT  ,inH+1,iZ+1) 
     fQ21 =  met_heat_tab (iT+1,inH  ,iZ+1)
     fQ22 =  met_heat_tab (iT+1,inH+1,iZ+1)
     lheat2 = (1.0d0/(x2x1*y2y1)) * ((fQ11*x2x*y2y) + (fQ21*xx1*y2y) + (fQ12*x2x*yy1) + (fQ22*xx1*yy1))

     ! heating derivative
     fQ11 =  met_heat_prime_tab (iT  ,inH  ,iZ  )
     fQ12 =  met_heat_prime_tab (iT  ,inH+1,iZ  ) 
     fQ21 =  met_heat_prime_tab (iT+1,inH  ,iZ  )
     fQ22 =  met_heat_prime_tab (iT+1,inH+1,iZ  )
     lheat1_prime = (1.0d0/(x2x1*y2y1)) * ((fQ11*x2x*y2y) + (fQ21*xx1*y2y) + (fQ12*x2x*yy1) + (fQ22*xx1*yy1))

     fQ11 =  met_heat_prime_tab (iT  ,inH  ,iZ+1)
     fQ12 =  met_heat_prime_tab (iT  ,inH+1,iZ+1) 
     fQ21 =  met_heat_prime_tab (iT+1,inH  ,iZ+1)
     fQ22 =  met_heat_prime_tab (iT+1,inH+1,iZ+1)
     lheat2_prime = (1.0d0/(x2x1*y2y1)) * ((fQ11*x2x*y2y) + (fQ21*xx1*y2y) + (fQ12*x2x*yy1) + (fQ22*xx1*yy1))

     ! Total metal cooling and temperature derivative 
     metal_tot = lcool1*fracZ + lcool2*(1d0-fracZ)
     metal_heat= lheat1*fracZ + lheat2*(1d0-fracZ)
     metal_prime= lcool1_prime*fracZ + lcool2_prime*(1d0-fracZ)
     metal_heat_prime=lheat1_prime*fracZ + lheat2_prime*(1d0-fracZ)

     metal_tot = 10d0**metal_tot
     metal_heat= 10d0**metal_heat 
    
     if(metal_tot.ne.metal_tot)     ok=.false. 
     if(metal_heat.ne.metal_heat)   ok=.false. 
     if(metal_prime.ne.metal_prime) ok=.false. 
     if(metal_heat_prime.ne.metal_heat_prime) ok=.false. 


     if(.not.ok)then !if NaN or Inf
        write(*,*) ' Interpolation error in cmp_metals_cloudy'
        write(*,*) ' metal_tot,metal_prime=',metal_tot,metal_prime
        write(*,*) ' metal_heat,metal_heat_prime=',metal_heat,metal_heat_prime
        write(*,*) ' log T  = ', lTT, T2, mu
        write(*,*) ' log nH = ', lnH, nH
        write(*,*) ' zred   = ', ZZ
        call clean_stop
     endif
  endif
end subroutine cmp_metals_cloudy
!=======================================================================
subroutine taux_rad_HM12(z)
!=======================================================================
  implicit none
  integer :: i
  real(kind=8) :: z,tt,frac
  ! Harley's Addition For Haardt + Madau 2012.  
  ! Downloaded from CUBA website (P. Madau)
  real(kind=8),dimension(1:59) :: zred_HM12 = (/ &
        & 0.00000, 0.04912, 0.10060, 0.15470, 0.21140, 0.27090, 0.33330, &
        & 0.39880, 0.46750, 0.53960, 0.61520, 0.69450, 0.77780, 0.86510, &
        & 0.95670, 1.05300, 1.15400, 1.25900, 1.37000, 1.48700, 1.60900, &
        & 1.73700, 1.87100, 2.01300, 2.16000, 2.31600, 2.47900, 2.64900, &
        & 2.82900, 3.01700, 3.21400, 3.42100, 3.63800, 3.86600, 4.10500, &
        & 4.35600, 4.61900, 4.89500, 5.18400, 5.48800, 5.80700, 6.14100, &
        & 6.49200, 6.85900, 7.24600, 7.65000, 8.07500, 8.52100, 8.98900, &
        & 9.47900, 9.99400, 10.5300, 11.1000, 11.6900, 12.3200, 12.9700, &
        & 13.6600, 14.3800, 15.1300 /)
  real(kind=8),dimension(1:59) :: hi_tau = (/ &
        & 2.28d-14, 2.84d-14, 3.54d-14, 4.40d-14, 5.46d-14, &
        & 6.74d-14, 8.31d-14, 1.02d-13, 1.25d-13, 1.52d-13, &
        & 1.85d-13, 2.23d-13, 2.67d-13, 3.18d-13, 3.76d-13, &
        & 4.40d-13, 5.10d-13, 5.85d-13, 6.60d-13, 7.32d-13, &
        & 7.99d-13, 8.59d-13, 9.09d-13, 9.44d-13, 9.63d-13, &
        & 9.65d-13, 9.50d-13, 9.19d-13, 8.75d-13, 8.22d-13, &
        & 7.65d-13, 7.05d-13, 6.47d-13, 5.94d-13, 5.46d-13, &
        & 5.04d-13, 4.69d-13, 4.41d-13, 4.12d-13, 3.60d-13, &
        & 2.93d-13, 2.30d-13, 1.75d-13, 1.29d-13, 9.28d-14, &
        & 6.55d-14, 4.56d-14, 3.12d-14, 2.12d-14, 1.43d-14, &
        & 9.59d-15, 6.40d-15, 4.27d-15, 2.92d-15, 1.73d-15, &
        & 1.02d-15, 5.92d-16, 3.41d-16, 1.94d-16 /)
 real(kind=8),dimension(1:59) :: hei_tau = (/ &
        & 1.24d-14, 1.57d-14, 1.96d-14, 2.46d-14, 3.07d-14, &
        & 3.83d-14, 4.75d-14, 5.87d-14, 7.22d-14, 8.84d-14, &
        & 1.08d-13, 1.30d-13, 1.57d-13, 1.87d-13, 2.22d-13, &
        & 2.61d-13, 3.02d-13, 3.46d-13, 3.91d-13, 4.34d-13, &
        & 4.74d-13, 5.09d-13, 5.38d-13, 5.57d-13, 5.67d-13, &
        & 5.66d-13, 5.55d-13, 5.35d-13, 5.08d-13, 4.76d-13, &
        & 4.41d-13, 4.06d-13, 3.72d-13, 3.41d-13, 3.14d-13, &
        & 2.91d-13, 2.71d-13, 2.53d-13, 2.37d-13, 2.14d-13, &
        & 1.84d-13, 1.54d-13, 1.25d-13, 9.92d-14, 7.61d-14, &
        & 5.68d-14, 4.14d-14, 2.96d-14, 2.07d-14, 1.44d-14, &
        & 9.82d-15, 6.67d-15, 4.53d-15, 3.24d-15, 2.02d-15, &
        & 1.23d-15, 7.46d-16, 4.46d-16, 2.62d-16 /)
  real(kind=8),dimension(1:59) :: heii_tau = (/ &
        & 5.55d-16, 6.76d-16, 8.23d-16, 1.00d-15, 1.22d-15, &
        & 1.48d-15, 1.80d-15, 2.18d-15, 2.63d-15, 3.17d-15, &
        & 3.80d-15, 4.54d-15, 5.38d-15, 6.33d-15, 7.38d-15, &
        & 8.52d-15, 9.70d-15, 1.09d-14, 1.19d-14, 1.27d-14, &
        & 1.32d-14, 1.34d-14, 1.33d-14, 1.28d-14, 1.19d-14, &
        & 1.06d-14, 9.04d-15, 7.22d-15, 5.30d-15, 3.51d-15, &
        & 2.08d-15, 1.14d-15, 5.91d-16, 3.02d-16, 1.52d-16, &
        & 7.60d-17, 3.75d-17, 1.82d-17, 8.57d-18, 3.23d-18, &
        & 1.17d-18, 4.42d-19, 1.73d-19, 7.01d-20, 2.92d-20, &
        & 1.25d-20, 5.67d-21, 2.74d-21, 1.44d-21, 8.19d-22, &
        & 4.99d-22, 3.25d-22, 2.12d-22, 1.43d-22, 9.84d-23, &
        & 6.81d-23, 4.73d-23, 3.30d-23, 1.92d-23 /)
  i = 2 ! initializing the index of the array
  taux_HM_HI=1.0d-40
  taux_HM_HeI=1.0d-40
  taux_HM_HeII=1.0d-40
  if (z < 15.d0) then
     ! Interpolating functions based on Haardt + Madau 2012
     ! finding the index where the redshift in the table is greater than the redshift of the simulation
     do while (z .GT. zred_HM12(i))
          i = i + 1
     end do
     !interpolation in log10 redshift
     frac = (zred_HM12(i)-z) / (zred_HM12(i) - zred_HM12(i-1)) 
     taux_HM_HI = (frac*hi_tau(i-1)) + ((1.0d0-frac)*hi_tau(i))
     taux_HM_HeI = (frac*hei_tau(i-1)) + ((1.0d0-frac)*hei_tau(i))
     taux_HM_HeII = (frac*heii_tau(i-1)) + ((1.0d0-frac)*heii_tau(i))
  end if
end subroutine taux_rad_HM12
!=======================================================================
subroutine heat_rad_HM12(z)
!=======================================================================
  implicit none
  integer :: i
  real(kind=8) :: z,tt,frac
  ...
  ! Harley's Addition For Haardt + Madau 2012.  These are simply the tables
  real(kind=8),dimension(1:59) :: zred_HM12 = (/ &
        & 0.00000, 0.04912, 0.10060, 0.15470, 0.21140, 0.27090, 0.33330, &
        & 0.39880, 0.46750, 0.53960, 0.61520, 0.69450, 0.77780, 0.86510, &
        & 0.95670, 1.05300, 1.15400, 1.25900, 1.37000, 1.48700, 1.60900, &
        ...
        & 9.47900, 9.99400, 10.5300, 11.1000, 11.6900, 12.3200, 12.9700, &
        & 13.6600, 14.3800, 15.1300 /)
  real(kind=8),dimension(1:59) :: hi_heat = (/ &
        & 8.89d-14, 1.11d-13, 1.39d-13, 1.73d-13, 2.15d-13, &
        & 2.66d-13, 3.29d-13, 4.05d-13, 4.96d-13, 6.05d-13, &
        & 7.34d-13, 8.85d-13, 1.06d-12, 1.26d-12, 1.49d-12, &
        & 1.75d-12, 2.03d-12, 2.32d-12, 2.62d-12, 2.90d-12, &
        & 3.17d-12, 3.41d-12, 3.60d-12, 3.74d-12, 3.81d-12, &
        & 3.82d-12, 3.75d-12, 3.63d-12, 3.46d-12, 3.25d-12, &
        & 3.02d-12, 2.79d-12, 2.57d-12, 2.36d-12, 2.18d-12, &
        & 2.02d-12, 1.89d-12, 1.78d-12, 1.67d-12, 1.48d-12, &
        & 1.23d-12, 9.89d-13, 7.71d-13, 5.83d-13, 4.30d-13, &
        & 3.10d-13, 2.19d-13, 1.53d-13, 1.05d-13, 7.13d-14, &
        & 4.81d-14, 3.23d-14, 2.17d-14, 1.51d-14, 9.15d-15, &
        & 5.46d-15, 3.23d-15, 1.89d-15, 1.10d-15 /)
  real(kind=8),dimension(1:59) :: hei_heat = (/ &
        & 1.12d-13, 1.40d-13, 1.74d-13, 2.16d-13, 2.67d-13, &
        & 3.31d-13, 4.08d-13, 5.02d-13, 6.15d-13, 7.51d-13, &
        & 9.11d-13, 1.10d-12, 1.32d-12, 1.57d-12, 1.86d-12, &
        & 2.17d-12, 2.51d-12, 2.87d-12, 3.23d-12, 3.57d-12, &
        & 3.87d-12, 4.13d-12, 4.32d-12, 4.44d-12, 4.46d-12, &
        & 4.38d-12, 4.22d-12, 3.98d-12, 3.68d-12, 3.36d-12, &
        & 3.04d-12, 2.74d-12, 2.49d-12, 2.27d-12, 2.09d-12, &
        & 1.94d-12, 1.81d-12, 1.70d-12, 1.60d-12, 1.46d-12, &
        & 1.30d-12, 1.12d-12, 9.52d-13, 7.83d-13, 6.25d-13, &
        & 4.83d-13, 3.63d-13, 2.66d-13, 1.91d-13, 1.34d-13, &
        & 9.27d-14, 6.36d-14, 4.35d-14, 3.14d-14, 1.98d-14, &
        & 1.22d-14, 7.49d-15, 4.55d-15, 2.70d-15 /)
  real(kind=8),dimension(1:59) :: heii_heat = (/ &
        & 1.14d-14, 1.38d-14, 1.68d-14, 2.03d-14, 2.45d-14, &
        & 2.96d-14, 3.57d-14, 4.29d-14, 5.14d-14, 6.15d-14, &
        & 7.32d-14, 8.67d-14, 1.02d-13, 1.19d-13, 1.39d-13, &
        & 1.59d-13, 1.81d-13, 2.02d-13, 2.21d-13, 2.37d-13, &
        & 2.47d-13, 2.53d-13, 2.52d-13, 2.44d-13, 2.29d-13, &
        & 2.07d-13, 1.78d-13, 1.45d-13, 1.11d-13, 7.75d-14, &
        & 4.97d-14, 2.96d-14, 1.68d-14, 9.25d-15, 5.01d-15, &
        & 2.67d-15, 1.41d-15, 7.27d-16, 3.65d-16, 1.56d-16, &
        & 6.24d-17, 2.69d-17, 1.28d-17, 6.74d-18, 3.88d-18, &
        & 2.40d-18, 1.55d-18, 1.03d-18, 6.98d-19, 4.76d-19, &
        & 3.26d-19, 2.24d-19, 1.53d-19, 1.06d-19, 7.52d-20, &
        & 5.31d-20, 3.73d-20, 2.57d-20, 1.54d-20 /)
  ! END HARLEY TABLES

  i = 2 ! initializing the index of the array
  heat_HM_HI=1.0d-40
  heat_HM_HeI=1.0d-40
  heat_HM_HeII=1.0d-40
  if (z < 15.d0) then
     !! Interpolating functions based on Haardt + Madau 2012
     ...
     frac = (zred_HM12(i) - z) / (zred_HM12(i) - zred_HM12(i-1))  !interpolation in linear redshift
     heat_HM_HI = eV*((frac*hi_heat(i-1)) + ((1.0d0-frac)*hi_heat(i)))
     heat_HM_HeI = eV*((frac*hei_heat(i-1)) + ((1.0d0-frac)*hei_heat(i)))
     heat_HM_HeII = eV*((frac*heii_heat(i-1)) + ((1.0d0-frac)*heii_heat(i))) 
  end if

end subroutine heat_rad_HM12
!=======================================================================
function heat_compton_HM12(T,aexp)
!=======================================================================
  ! Harley edit for madau compton heating
  implicit none
  real(kind=8) ::T,aexp,zz,frac,heat_compton_HM12
  integer :: i
  real(kind=8),parameter ::eV      = 1.6022000d-12
  real(kind=8),dimension(1:59) :: zred_HM12 = (/ &
        & 0.00000, 0.04912, 0.10060, 0.15470, 0.21140, 0.27090, 0.33330, &
        ...
        & 9.47900, 9.99400, 10.5300, 11.1000, 11.6900, 12.3200, 12.9700, &
        & 13.6600, 14.3800, 15.1300 /)
  real(kind=8),dimension(1:59) :: ccHM12 = (/ &
        & 6.90d-20, 8.27d-20, 9.85d-20, 1.17d-19, 1.37d-19, &
        & 1.59d-19, 1.84d-19, 2.11d-19, 2.40d-19, 2.70d-19, &
        & 3.02d-19, 3.34d-19, 3.66d-19, 3.97d-19, 4.26d-19, &
        & 4.52d-19, 4.75d-19, 4.93d-19, 5.05d-19, 5.10d-19, &
        & 5.07d-19, 4.93d-19, 4.67d-19, 4.33d-19, 4.00d-19, &
        & 3.69d-19, 3.40d-19, 3.13d-19, 2.88d-19, 2.65d-19, &
        & 2.44d-19, 2.24d-19, 2.06d-19, 1.89d-19, 1.73d-19, &
        & 1.59d-19, 1.46d-19, 1.34d-19, 1.23d-19, 1.13d-19, &
        & 1.04d-19, 9.51d-20, 8.72d-20, 7.99d-20, 7.31d-20, &
        & 6.69d-20, 6.10d-20, 5.56d-20, 5.05d-20, 4.57d-20, &
        & 4.12d-20, 3.68d-20, 3.26d-20, 2.84d-20, 2.43d-20, &
        & 2.01d-20, 1.56d-20, 1.09d-20, 5.77d-21 /) 
  i = 2 ! initializing the index of the array
  zz = (1.0d0/aexp) - 1.0d0
  heat_compton_HM12=1.0d-100
  if (zz < 15.d0) then
     !! Interpolating functions based on Haardt + Madau 2012
     ! finding the index where the redshift in the table is greater than the redshift of the simulation
     do while (zz .GT. zred_HM12(i))
          i = i + 1
     end do
     frac = (zred_HM12(i)-zz) / (zred_HM12(i) - zred_HM12(i-1)) !interpolation in linear redshift
     heat_compton_HM12= eV*((frac*ccHM12(i-1)) + ((1.0d0-frac)*ccHM12(i)))
  else
     heat_compton_HM12=5.406D-36*2.726D0/aexp**5
  end if
  return
end function heat_compton_HM12
