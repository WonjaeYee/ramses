...
module mechanical_commons
   ...
   use hydro_parameters, ONLY:nmetals,nco
   integer(kind=4),parameter::nvarSN=13+nchem+nmetals+nco
  ...
  ! common lists for SNe across different cpus
   ...
   real(dp),dimension(1:ncomm_max,1:nmetals+nco)::mZSN_comm         ! metal mass of SNe 
   ...
   real(dp),dimension(1:ncomm_max,1:nmetals+nco)::mZloadSN_comm     ! metals ejected + entrained
...
subroutine init_mechanical
  ...
  !if (snIa_mech) call init_snIa_yield
  if (snIa_mech) call init_snIa_yield_harley

  ! This is not to double count SN feedback
  if (mechanical_feedback) f_w = -1

  ! Binary stellar evolution
  if (mechanical_bpass) call init_bpass_v2

end subroutine init_mechanical
!####################################################################
!####################################################################
!####################################################################
subroutine init_snIa_yield_harley
   use metal_yields, only: SNIaFe, SNIaO, SNIaC, SNIaN, SNIaMg, SNIaAl, SNIaSi, SNIaCa, SNIaEu, SNIaNe, SNIaS
   use mechanical_commons, ONLY: mejecta_Ia
   implicit none
   mejecta_Ia = SNIaFe + SNIaO + SNIaC + SNIaN  + SNIaMg + SNIaAl + SNIaSi + SNIaCa + SNIaEu + SNIaNe + SNIaS
end subroutine init_snIa_yield_harley
!####################################################################
!####################################################################
!####################################################################
