...
subroutine hydro_refine(ug,um,ud,ok,nn,ilevel)
  ...
  ! added by Taysun Kimm
  if(mISM_refine.ge.0.and.aexp.ge.aISM_refine)then
      do k=1,nn
         dm=um(k,1)
         !Zcell=um(k,imetal)/0.02
         Zcell=(2.09d0*um(k,imetal+1)+1.06d0*um(k,imetal))/0.02 ! metal enrichment
         if(dm*scale_nH.ge.nISM_refine.and.zcell.ge.ZsunISM_refine)then
            cell_mass = dm*vol_loc*(scale_l**3*scale_d/2d33)
            ok(k) = ok(k) .or. (cell_mass>mISM_refine)
         endif
      end do
  endif
