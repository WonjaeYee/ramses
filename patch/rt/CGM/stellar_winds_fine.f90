...
subroutine stellar_winds_dump(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  real(dp),dimension(1:nvector,1:nmetals)::mzloss
  ...
  real(dp)::msun2g=2d33
  ! stellar library
  real(dp)::mejecta,dfmloss,log_deloss_erg
  real(dp),dimension(1:nmetals)::dfmzloss
  ...
  do j=1,np
     ...
     zstar = 2.09d0*zp(ind_part(j),2)+1.06d0*zp(ind_part(j),1) !metal enrichment
     ...
     if(metal)then
        do imet=1,nmetals
           mzloss(j,imet)=mzloss(j,imet)+mstar_ini*dfmzloss(imet)/vol_loc(j)
        enddo
     endif
     ! Chemical species
     if(nchem>0)then
     ...
     if (MC_tracer) &
        call mark_yielding_particle(ind_part(j), mejecta / mp(ind_part(j)))
  ...
  if(metal)then
     do j=1,np
        do imet=1,nmetals
           unew(indp(j),imetal+imet-1)=unew(indp(j),imetal+imet-1)+mzloss(j,imet)
        enddo
     end do
  endif
...
subroutine cmp_stellar_wind_props (birth_time,dteff, zstar,dfmloss, log_deloss_erg, dfmzloss, dfmloss_spec)
   ...
   real(dp)::dfmloss,log_deloss_erg
   real(dp),dimension(1:nmetals)::dfmzloss
   ...
   dfmzloss(1) = 10d0**cmz2 - 10d0**cmz1 !TODO(code): HARLEY THIS IS A HACK FOR NOW
