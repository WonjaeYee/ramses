subroutine newdt_fine(ilevel)
  ...
  ! added by Harley Katz so that if star formation is on
  ! we don't let the time step to go above some predefined number
  ! of years
  star_min_timestep_loc = star_min_timestep * 365.25d0 * 24.d0 * 60.d0 * 60.d0
  if (star.and.star_min_timestep_loc.gt.0.d0.and.nstar_tot.gt.0) then
     ! determine if on highest active level...
     if (ilevel==nlevelmax)then
        highest_level=.true.
     else if (numbtot(1,ilevel+1)==0)then
     ...
     if (highest_level) then
        ...
        dtnew(ilevel) = MIN(dtnew(ilevel),star_min_timestep_loc/scale_t)
     end if
  end if
