...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  !-------------------------------------------------------------------
  !-------------------------------------------------------------------
  integer::i,im,ind,iskip,idim,nleaf,nx_loc,ix,iy,iz
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  real(kind=8)::dtcool,nISM,nCOM,damp_factor,cooling_switch,t_blast
  real(dp)::polytropic_constant
  integer,dimension(1:nvector),save::ind_cell,ind_leaf
  real(kind=8),dimension(1:nvector),save::nH,T2,delta_T2,ekk,err,temp_cool_time
  real(kind=8),dimension(1:nvector),save::total_rho
  real(kind=8),dimension(1:nvector),save::temperature_save
  real(kind=8),dimension(ncoolheatratesave,1:nvector),save::cooling_save
  ...
  real(kind=8),dimension(1:nvector),save::nco_mol
  ...
  do ind=1,twotondim
     ...
     if(metal)then
        do i=1,nleaf
           Zsolar(i)=(2.09d0*uold(ind_leaf(i),imetal+1)+1.06d0*uold(ind_leaf(i),imetal))/nH(i)/0.02 ! metal enrichment
        end do
     else
        do i=1,nleaf
           Zsolar(i)=z_ave
     ...
     do i=1,nleaf
        ...
        ! First subtract off all of the metals
        ! we subtract regardless of whether we follow the ions
        total_rho(i) = MAX(uold(ind_leaf(i),1),smallr) * scale_d ! g/cm^3
        do im=1,nmetals
           nH(i) = nH(i) - uold(ind_leaf(i),imetal+im-1)
        end do
        ...
        if (nco.gt.0) then 
           ...
           nco_mol(i) = uold(ind_leaf(i),ico) * scale_d / ((mC_NIST_amu + mO_NIST_amu) * amu_to_g) ! CO number density
           nH(i) = nH(i) - uold(ind_leaf(i),ico)
        endif

        !We need to subtract the metals off before doing this multiplication
        !Otherwise we get the wrong He abundance
        nH(i)=MAX(nH(i)*scale_nH,1.d-8) ! Don't let density drop below 1e-8
        Zsolar(i) = (nOxygen(i)/nH(i)) / 4.90d-4 ! Update metallicity measure
     end do
	...
	enddo
     ...
     if(neq_chem) then
        ...
        call rt_solve_cooling(T2_new, xion, Np, Fp, p_gas, dNpdt, dFpdt  &
                         , nH, total_rho, cooling_on, Zsolar, dtcool, aexp_loc, nleaf, dx_SS &
                         , xO, xN, xC, xMg, xSi, xS, xFe, xNe, nOxygen, nNitrogen, nCarbon, nMagnesium, nSilicon &
                         , nSulfur, nIron, nNeon, nco_mol, dx_loc*scale_l, temp_cool_time, temperature_save, cooling_save)
...
#if NVARNOADVECT>0
          !Update first non-advected variable to store the cooling time
          ! Grab cooling time from cooling module and rescale to internal units
#if NCOOLLENGTHSAVE>0
          if (cooling_time_ivar.gt.0) unoadvect(ind_leaf(i),cooling_time_ivar) = temp_cool_time(i) / scale_t
#endif
#if NTEMPERATURESAVE>0
          if (temperature_save_ivar.gt.0)  unoadvect(ind_leaf(i),temperature_save_ivar) = temperature_save(i) / scale_T2
#endif
#if NCOOLHEATRATESAVE>0
          do idim=1,ncoolheatratesave
             unoadvect(ind_leaf(i),coolheat_save_ivar+idim-1) = cooling_save(idim,i)
          end do
#endif
#endif
        end do
...
#ifdef RT
     ! nH(i) below is actually total density in code units, see line 656.
     if(neq_chem) then
        ...
        if (oxygen_ions) then
         do ii=0,n_oxygen_ions-1
            do i=1,nleaf
               uold(ind_leaf(i),ioxygen+ii) = xO(1+ii,i)*nH(i)
            end do
         end do
        endif
        ...
        if (nitrogen_ions) then
         do ii=0,n_nitrogen_ions-1
            do i=1,nleaf
               uold(ind_leaf(i),initrogen+ii) = xN(1+ii,i)*nH(i)
            end do
         end do
        endif
        ...
        if (carbon_ions) then
         do ii=0,n_carbon_ions-1
            do i=1,nleaf
               uold(ind_leaf(i),icarbon+ii) = xC(1+ii,i)*nH(i)
            end do
         end do
        endif
        ...
        if (magnesium_ions) then
         do ii=0,n_magnesium_ions-1
            do i=1,nleaf
               uold(ind_leaf(i),imagnesium+ii) = xMg(1+ii,i)*nH(i)
            end do
         end do
        endif
        ...
        if (silicon_ions) then
         do ii=0,n_silicon_ions-1
            do i=1,nleaf
               uold(ind_leaf(i),isilicon+ii) = xSi(1+ii,i)*nH(i)
            end do
         end do
        endif
        ...
        if (sulfur_ions) then
         do ii=0,n_sulfur_ions-1
            do i=1,nleaf
               uold(ind_leaf(i),isulfur+ii) = xS(1+ii,i)*nH(i)
            end do
         end do
        endif
        ...
        if (iron_ions) then
         do ii=0,n_iron_ions-1
            do i=1,nleaf
               uold(ind_leaf(i),iiron+ii) = xFe(1+ii,i)*nH(i)
            end do
         end do
        endif
        ...
        if (neon_ions) then
         do ii=0,n_neon_ions-1
            do i=1,nleaf
               uold(ind_leaf(i),ineon+ii) = xNe(1+ii,i)*nH(i)
            end do
         end do
        endif
        ...
        if (isCO) then
         do i=1,nleaf
            ! Save the mass fraction of CO with respect to total density
            uold(ind_leaf(i),ico) = nco_mol(i) * (mO_NIST_amu + mC_NIST_amu) * amu_to_g / scale_d
            ! Update CMassFrac
            uold(ind_leaf(i),imetal+7) = nCarbon(i) * (mC_NIST_amu * amu_to_g) / scale_d
            ! Update OMassFrac
            uold(ind_leaf(i),imetal+1) = nOxygen(i) * (mO_NIST_amu * amu_to_g) / scale_d
         end do
        endif
