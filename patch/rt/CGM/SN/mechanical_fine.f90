...
subroutine mechanical_feedback_fine(ilevel, icount)
   ...
   real(dp), dimension(1:twotondim, 1:nmetals), save::mz8  ! metal enrichment
   real(dp), dimension(1:twotondim, 1:3), save:: p8  ! SNe
   real(dp), dimension(1:nvector), save::nSNe, mSNe, nphSNe
   real(dp), dimension(1:nvector, 1:nmetals), save::mZSNe ! metal enrichment
   ...
   real(dp), dimension(1:nmetals)::Zejecta
...
if(myid==1)then 
   print *, '## dtnew =', ilevel, myid, sngl(dtnew(levelmin:nlevelmax))
   print *, '## dtold =', ilevel, myid, sngl(dtold(levelmin:nlevelmax))
endif
      ...
      do jgrid = 1, numbl(icpu, ilevel)
         ...
         if (npart1 > 0) then
            ...
            do jpart = 1, npart1
              ...
              !  if (pop3 .and. (mett .lt. Zcrit_pop3)) then
                  ...
                  if (ok) then
                     ...
                     if (mechanical_bpass) then
                        ...
                        call max_age_SN_raiteri(mett, 8.0d0, t_delay) ! Get the max SN age in Myr
                        ...
                        if (age2 .lt. t_delay*1d-3) then  ! if younger than t_delay
                           ...
                           if (mp(ipart)*scale_msun .lt. meanmassM*nsnII_star) then
                              ok = .false.
                            write (*, *) "SN prevented due to negative mass", nsnII_star*meanmassM, mp(ipart)*scale_msun, nsnII_star
                           end if
                        else
                           if (log_mfb) then
                              write (*, *) "8Msun stars evolved off main sequence", idp(ipart), t_delay, age2*1d3
                           end if
                           idp(ipart) = abs(idp(ipart)) ! Flip the ID if the star is too old
               ...
               if (ok) then
                  ...
                  if (son(ind_cell) == 0) then  ! leaf cell
                     ...
                     if (metal) then
                        ...
                        Zejecta(1) = nsnII_star*SNII_Fe_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari, yields_lc18)/scale_msun !1=Fe metal enrichment
                        Zejecta(2) = nsnII_star*SNII_O_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari, yields_lc18)/scale_msun  !2=O  metal enrichment
                        Zejecta(3) = nsnII_star*SNII_N_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari, yields_lc18)/scale_msun  !3=N  metal enrichment
                        Zejecta(4) = nsnII_star*SNII_Mg_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari, yields_lc18)/scale_msun !4=Mg metal enrichment
                        Zejecta(5) = nsnII_star*SNII_Ne_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari, yields_lc18)/scale_msun !5=Ne metal enrichment
                        Zejecta(6) = nsnII_star*SNII_Si_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari, yields_lc18)/scale_msun !6=Si metal enrichment
                        Zejecta(7) = nsnII_star*SNII_Ca_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari, yields_lc18)/scale_msun !7=Ca metal enrichment
                        Zejecta(8) = nsnII_star*SNII_C_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari, yields_lc18)/scale_msun  !8=C  metal enrichment
                        Zejecta(9) = nsnII_star*SNII_S_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari, yields_lc18)/scale_msun  !9=S  metal enrichment
                     end if
                     ...
                     if (log_mfb) then
399                     format('Ejected mass (Msun):',e9.2,' Fe=',e9.2,' O=',e9.2,' N=',e9.2,' Mg=',e9.2,' Ne=',e9.2,' Si=',e9.2,' Ca=',e9.2,' C=',e9.2,' S=',e9.2)
                        write(*,399) scale_msun*mejecta,scale_msun*Zejecta(1),scale_msun*Zejecta(2),scale_msun*Zejecta(3),scale_msun*Zejecta(4),scale_msun*Zejecta(5),scale_msun*Zejecta(6),scale_msun*Zejecta(7),scale_msun*Zejecta(8),scale_msun*Zejecta(9)
                     end if
                     ...
                     if (variable_energy_SN) then
                        ...
                        call get_effective_num_sn(mett,meanmassM,nsnII_star,nsnII_star_variable_resample)
                     end if
...
subroutine mech_fine(ind_grid, ind_pos_cell, np, ilevel, dteff, nSN, mSN, pSN, mZSN, nphSN, mchSN)
   ...
   real(dp), dimension(1:nvector, 1:nmetals)::mZSN, mZloadSN ! metal enrichment
   ...
   real(dp), dimension(1:nmetals)::Zload = 0d0
...
subroutine mech_fine_mpi(ilevel)
   ...
   real(dp), dimension(1:nmetals)::zloadSN_i
...
!  logical,dimension(1:nvector,1:nSNnei),save ::snowplough
      ...
      do icpu = 1, ncpu_send
         ...
         do i = 1, ncell_send
            ...
            if (icpuSN_comm_mpi(j, 2) .eq. cpu2send) then
               ...
               if (mechanical_geen .and. rt) SNsend(12 + nmetals, ncc) = rSt_comm(i)
               do ich = 1, nchem
   ...
   do i = 1, np
      ...
      if (mechanical_geen .and. rt) rSt_i = SNrecv(12 + nmetals, i) ! Stromgren sphere in pc
...
!---------------------------------------
subroutine get_effective_num_sn(mett,smass,num_sn_eff_in,num_sn_eff_out)
!---------------------------------------
...
! stellar metallicity following recommendations from chiaki
   ...
   implicit none
   REAL(kind=8), intent(out) :: num_sn_eff_out
   ...
   do i=1,NINT(num_sn_eff_in)
      ...
      if (loc_rand.le.hn_fraction) then 
         ...
         num_sn_eff_out = num_sn_eff_out + 1.d0
      end if
   end do
