...
subroutine userflag_fine(ilevel)
  ...
  real(dp)::agg_x,agg_y,agg_z,agg_r,agg_H,rho_code,z_code
  real(dp)::xx1,xx2,xx3,rr1,dxmin2,rcl_code
  ...
  ! if ivar_refine_reset_disable_lxyz is set, turn off lx_refine and resort to ivar_refine 
  ! this will not affect simulations even if we restart sims with ivar_refine_reset_disable_lxyz=.true.
  if(nstep>1.and.ivar_refine_reset_disable_lxyz)then
     if(lx_refine(ilevel)>=0) lx_refine(ilevel) = -1d0
     if(ly_refine(ilevel)>=0) ly_refine(ilevel) = -1d0
     if(lz_refine(ilevel)>=0) lz_refine(ilevel) = -1d0
     ivar_refine_reset_disable_lxyz = .false.
  endif
...
!     if(d0*(dx_loc/2.0)**ndim.lt.msnk/2d0)prevent_refine=.true.
  ...
  ! Taysun------------------------------------------ 
  ! Assuming that there is just one star particle
  ! ------------------------------------------------
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        if(m_refine(ilevel)>-1.0d0)then
           ...
           if(r_refine(ilevel)>-1.0.or.lx_refine(ilevel)>-1.0)then
           ...
           ! Aggressive Refinement for turbulence
           if(agg_geo_refine)then
 
              agg_x = agg_geo_refine_x
              agg_y = agg_geo_refine_y
              agg_z = agg_geo_refine_z
              agg_r = agg_geo_refine_rho_kpc / (scale_l/kpc2cm*boxlen) 
              agg_H = agg_geo_refine_z_kpc / (scale_l/kpc2cm*boxlen) 
              ! Compute cell center in code units
              do idim=1,ndim
                 do i=1,ngrid
                    xx(i,idim)=xg(ind_grid(i),idim)+xc(ind,idim)
              ...
              do i=1,ngrid
                 rho_code = sqrt( (xx(i,1)-agg_x)**2.+(xx(i,2)-agg_y)**2. )
                 z_code   = abs( xx(i,3)-agg_z)
                 if( (rho_code.le.agg_r).and.(z_code.le.agg_H)) then
                    ok(i)=.true. !aggressive refinement
                 end if
              end do
        ...
        !--------------------------------------------------------------
        ! Taysun wants the cells around the particle maximally refined
        do idim=1,ndim
           do i=1,ngrid
              xx(i,idim)=xg(ind_grid(i),idim)+xc(ind,idim)
           end do
        ...
        do idim=1,ndim
           do i=1,ngrid
              xx(i,idim)=(xx(i,idim)-skip_loc(idim))
           end do
        end do
     
        do i=1,ngrid
           rr1 = (xx(i,1)-(xx1+dxmin2))**2+(xx(i,2)-(xx2+dxmin2))**2+(xx(i,3)-(xx3+dxmin2))**2
           rr1 = sqrt(rr1)
           if(rr1.le.(2d0*dx)) ok(i)=.true. ! central region
        end do
        !--------------------------------------------------------------
  ...
  ! Do the same for SN particles
  !if(feedback_refine) call feedback_flag(ilevel)
...
subroutine geometry_refine(xx,ind_cell,ok,ncell,ilevel)
  ...
  ! user-defined physical criterion for refinement.
  !-------------------------------------------------
  real(dp)::er,xr,yr,zr,rr,xn,yn,zn,r,aa,bb,cc
  integer ::i

  ! Authorize refinement if cell lies within region,
  ! otherwise unmark cell (no refinement outside region)
  if(lx_refine(ilevel)>-1.0)then
     xr=x_refine  (ilevel) ! Region centre
     yr=y_refine  (ilevel)
     zr=z_refine  (ilevel)
     aa=lx_refine (ilevel)/2d0  ! Length (beware !)
     bb=ly_refine (ilevel)/2d0  ! Length (beware !)
     cc=lz_refine (ilevel)/2d0  ! Length (beware !)
     do i=1,ncell
        xn=0.0d0; yn=0.0d0; zn=0.0d0
        ok_geo = (abs(xx(i,1)-xr)<=aa)
#if NDIM > 1
        ok_geo = ok_geo.and.(abs(xx(i,2)-yr)<=bb)
#endif
#if NDIM >2
        ok_geo = ok_geo.and.(abs(xx(i,3)-zr)<=cc)
#endif
        ok(i)=ok(i).and.ok_geo
        if( (ivar_refine_reset_disable_lxyz).and.(ivar_refine>0).and.(.not.ok_geo))then ! reset ivar_refine to 0 if outside
           uold(ind_cell(i),ivar_refine) = 0
        endif 
     end do
  else if(r_refine(ilevel)>-1.0)then
