subroutine read_params
  ...
  use mpi_mod
  use amr_parameters, only: MC_tracer
  implicit none
  !--------------------------------------------------
  ! Local variables
  !--------------------------------------------------
  integer::i,narg,iargc,ierr,levelmax,levelmax_TK=-1
  ...
  namelist/run_params/clumpfind,cosmo,pic,sink,lightcone,poisson,hydro,rt,verbose,debug &
       ...
       & ,convert_birth_times,use_proper_time,single_precision_comm,tracer
  namelist/output_params/noutput,foutput,fbackup,aout,tout,output_mode &
       & ,tend,delta_tout,aend,delta_aout,gadget_output,cpu_time_limit
  namelist/amr_params/levelmin,levelmax,ngridmax,ngridtot &
       & ,npartmax,nparttot,nexpand,boxlen,levelmax_TK
  ...
  namelist/movie_params/levelmax_frame,nw_frame,nh_frame,ivar_frame &
       ...
       & ,tend_theta_camera,tend_phi_camera,el_movie_vars,rt_movie_vars,noadvect_movie_vars &
...
#endif
  !--------------------------------------------------
  ! Advertise RAMSES-RTZ
  !--------------------------------------------------
  if(myid==1)then
  write(*,'(A)')'_/_/_/       _/_/     _/    _/    _/_/_/   _/_/_/_/    _/_/_/         _/_/_/     _/_/_/_/_/   _/_/_/_/_/'
  write(*,'(A)')'_/    _/    _/  _/    _/_/_/_/   _/    _/  _/         _/    _/        _/    _/       _/               _/'
  write(*,'(A)')'_/    _/   _/    _/   _/ _/ _/   _/        _/         _/              _/    _/       _/             _/  '
  write(*,'(A)')'_/_/_/     _/_/_/_/   _/    _/     _/_/    _/_/_/       _/_/   _____/ _/_/_/         _/           _/    '
  write(*,'(A)')'_/    _/   _/    _/   _/    _/         _/  _/               _/        _/    _/       _/         _/      '
  write(*,'(A)')'_/    _/   _/    _/   _/    _/   _/    _/  _/         _/    _/        _/    _/       _/       _/        '
  write(*,'(A)')'_/    _/   _/    _/   _/    _/    _/_/_/   _/_/_/_/    _/_/_/         _/    _/       _/       _/_/_/_/_/'
  write(*,'(A)')'                                              Version 3.0                                               '
  write(*,'(A)')'                            written by Romain Teyssier (University of Zurich)                           ' 
  write(*,'(A)')'                                   (c) CEA 1999-2007, UZH 2008-2014                                     '
  write(*,'(A)')' '
...
81 continue
  ...
  if(imovout>0) then
     ...
     if(tendmov>0)then
        do i=1,imovout
           tmovout(i)=(tendmov-tstartmov)*dble(i-1)/dble(imovout-1)+tstartmov
        enddo
     endif
     if(aendmov>0)then
        do i=1,imovout
           amovout(i)=(aendmov-astartmov)*dble(i-1)/dble(imovout-1)+astartmov
        enddo
     endif
  ...
  nlevelmax=levelmax
  !if(levelmax_TK>0) then
  !   nlevelmax_TK=levelmax_TK
  !else
  !   nlevelmax_TK=nlevelmax
  !endif
  ...
  if(SFdiagnostics)then
   ...
   if(log_exist)then
      ...
      write(SFunit_out,*)"# 'nstep'   'index'   'ilevel'  'rho [H/cc]'   'x [kpc]'   'y [kpc]'   'z [kpc]'   'mstar [Msun]'   'tform [s]'    'aexp'   'metallicity'"
   endif
  endif
  ...
  do i=nlevelmax,levelmin,-1
     ...
     lx_refine (i)=lx_refine (i-levelmin+1)
     ly_refine (i)=ly_refine (i-levelmin+1)
     lz_refine (i)=lz_refine (i-levelmin+1)
  end do
  do i=1,levelmin-1
     nexpand   (i)= 1
     nsubcycle (i)= 1
