...
subroutine mechanical_feedback_snIa_fine(ilevel, icount)
   ...
   use mpi_mod
   use metal_yields, only: SNIaFe, SNIaO, SNIaC, SNIaN, SNIaMg, SNIaAl, SNIaSi, SNIaCa, SNIaEu, SNIaNe, SNIaS
   use mpi_mod
#ifdef RT
   ...
   real(dp), dimension(1:twotondim, 1:nmetals+nco), save::mz8
   real(dp), dimension(1:twotondim, 1:3), save:: p8  ! SNe
   real(dp), dimension(1:nvector), save::nSNe, mSNe, nphSNe
   real(dp), dimension(1:nvector, 1:nmetals+nco), save::mZSNe
      ...
      do jgrid = 1, numbl(icpu, ilevel)
         ...
         if (npart1 > 0) then
            ...
            do jpart = 1, npart1
               ...
               ! Pop II stars
               ok_star = is_pop_II(typep(ipart)) .and. is_post_SN(typep(ipart))

               ! compute the number of snIa
               nsnIa_star = 0d0
               if (snIa_mech .and. ok_star) then
                  if (use_initial_mass) then
                     mass0 = mp0(ipart)*scale_msun
                  else
                     mass0 = mp(ipart)*scale_msun
                     if (is_post_SN(typep(ipart))) mass0 = mass0/(1d0 - eta_sn)
                  end if
               ...
               if (nsnIa_star > 0) then
                  ...
                  if (son(ind_cell) == 0) then  ! leaf cell
                     ...
                     if (metal) then
                        mz8(ind_son, 1) = mz8(ind_son, 1) + nsnIa_star*SNIaFe/scale_msun
                        mz8(ind_son, 2) = mz8(ind_son, 2) + nsnIa_star*SNIaO/scale_msun
                        mz8(ind_son, 3) = mz8(ind_son, 3) + nsnIa_star*SNIaN/scale_msun
                        mz8(ind_son, 4) = mz8(ind_son, 4) + nsnIa_star*SNIaMg/scale_msun
                        mz8(ind_son, 5) = mz8(ind_son, 5) + nsnIa_star*SNIaNe/scale_msun
                        mz8(ind_son, 6) = mz8(ind_son, 6) + nsnIa_star*SNIaSi/scale_msun
                        mz8(ind_son, 7) = mz8(ind_son, 7) + nsnIa_star*SNIaCa/scale_msun
                        mz8(ind_son, 8) = mz8(ind_son, 8) + nsnIa_star*SNIaC/scale_msun
                        mz8(ind_son, 9) = mz8(ind_son, 9) + nsnIa_star*SNIaS/scale_msun
                     ...
                     if (log_mfb) then
399                     format('SNIa -- Ejected mass (Msun):',f10.3,' Fe=',f10.3,' O=',f10.3,' N=',f10.3,' Mg=',f10.3,' Ne=',f10.3,' Si=',f10.3,' Ca=',f10.3,' C=',f10.3,' S=',f10.3)
                        write(*,399) scale_msun*mejecta,nsnIa_star*SNIaFe,nsnIa_star*SNIaO,nsnIa_star*SNIaN,nsnIa_star*SNIaMg,nsnIa_star*SNIaNe,nsnIa_star*SNIaSi,nsnIa_star*SNIaCa,nsnIa_star*SNIaC,nsnIa_star*SNIaS
                     end if
                     ...
                     if (mejecta.ge.mp(ipart)) write(*,*) "WARNING TOO MANY SNIa --> NEGATIVE MASS PREVENTED"
                     mp(ipart) = mp(ipart) - MIN(mejecta,0.999d0*mp(ipart))
...
subroutine mech_fine_snIa(ind_grid, ind_pos_cell, np, ilevel, dteff, nSN, mSN, pSN, mZSN, nphSN, mchSN)
   ...
   real(dp), dimension(1:nvector, 1:nmetals+nco)::mZSN
   real(dp), dimension(1:nvector)::mloadSN, eloadSN
   real(dp), dimension(1:nvector, 1:nmetals+nco)::mZloadSN
   ...
   do i = 1, np
      ...
      z = (2.09d0*uold(icell, imetal + 1) + 1.06d0*uold(icell, imetal))/d ! metal enrichment
...
!  logical, dimension(1:nvector,1:nSNnei),save ::snowplough
      ...
      do icpu = 1, ncpu_send
         ...
         do i = 1, ncell_send
            ...
            if (icpuSN_comm_mpi(j, 2) .eq. cpu2send) then
               ...
               do ich = 1, nchem
                  SNsend(13 + ich, ncc) = mchloadSN_comm(i, ich) ! --> do not use...broke
               end do
            end if
         end do ! i
   ...
   do i = 1, np
      ...
      if (metal) then
         ...
         !TODO(code): generalization
         if (nco.gt.0) ZloadSN_i(nmetals+1) = SNrecv(12 + nmetals, i)/mloadSN_i
      end if
      do ich = 1, nchem
