   ...
   contains
      ...
      !-----------------------------------------------------------
         ...
         do while (not_found)
            ...
            x0(1:ndim) = xg(igrid,1:ndim)-dx_here-skip_loc(1:ndim) !!left corner of *this* grid [0-1], not cell
            ind  = 1
            do idim = 1,ndim
      ...
      !-----------------------------------------------------------
         ...
         do ileaf=1,nleaf
            do ibor=1,6
               ...
               Zsol_nbor(ileaf,ibor) = (2.09d0*pvar(imetal+1)+1.06d0*pvar(imetal))/pvar(1)/0.02 ! metal enrichment
      ...
      !-----------------------------------------------------------
         ...
         fescLya = 1d0/(cosh(sqrt(3.)/pi**(5./12.)/xi_new*sqrt( (av_Lya*tau0_Lya)**(1./3.)*tau_Lya_da))) 
...
subroutine Lya_fine1 (ind_grid,ngrid,ilevel)
  ...
  do ind=1,twotondim
     ...
     do ileaf=1,nleaf 
        ...
        Zsol_cen = (2.09d0*pvar(imetal+1)+1.06d0*pvar(imetal))/pvar(1)/0.02 ! metal enrichment
...
subroutine inject_momentum_Lya_neigh (nH_cen,Tk_cen,Zsol_cen,xHI_cen,pvar_nei,level_diff,i_nei,Nph50)
   ...
   Zsol_nei = (2.09d0*pvar_nei(imetal+1)+1.06d0*pvar_nei(imetal))/d/0.02 ! metal enrichment
