...
!_________________________________________________________________________
  ...
  integer::SED_nA, SED_nZ=8, SED_nIMF ! Number of age bins and Z bins
  ! Age and z logarithmic intervals and lowest values:
  real(dp),parameter::SED_dlgA=0.02d0
  real(dp)::SED_dlgZ, SED_dIMF
  real(dp)::SED_lgA0, SED_lgZ0, SED_IMF0
  real(dp),allocatable,dimension(:)::SED_ages,SED_zeds,SED_IMFs![Gyr],[m_met/m_gas]
  ! SED_table: iAges, imetallicities, iimfs, igroups, properties
...
!-------------------------------------------------------------------------
  ...
  integer:: nAges, nzs, nIMFs, nLs              ! # of bins of age, z, imf, wavelength
  real(kind=8),allocatable::ages(:), imfs(:), Zs(:), Ls(:), rebAges(:)
  real(kind=8),allocatable::SEDs(:,:,:,:)           ! SEDs f(lambda,age,met,imf)
  ...
  real(kind=8),allocatable::tmp_tbl(:,:,:)
  real(kind=8),allocatable::tmp_tbl_oxygen(:,:,:)
  real(kind=8),allocatable::tmp_tbl_nitrogen(:,:,:)
  real(kind=8),allocatable::tmp_tbl_carbon(:,:,:)
  real(kind=8),allocatable::tmp_tbl_magnesium(:,:,:)
  real(kind=8),allocatable::tmp_tbl_silicon(:,:,:)
  real(kind=8),allocatable::tmp_tbl_sulfur(:,:,:)
  real(kind=8),allocatable::tmp_tbl_iron(:,:,:)
  real(kind=8),allocatable::tmp_tbl_neon(:,:,:)
  real(kind=8),allocatable::tmp_tbl_dust(:,:,:)
  integer::i,ia,iz,ip,ii,il,dum
  character(len=128)::fZs, fAges, fSEDs, fIMFs                ! Filenames
  logical::ok,okAge,okZ,okimf
...
!-------------------------------------------------------------------------
  ...
  ! Set up the IMF bins
  SED_nIMF = 1 ! default to 1 and will be updated later 
  if (imf_varies_complex) then
     if(.not. okimf) then
        if(myid.eq.1) then
           write(*,*) 'Cannot read SED IMF files...'
           write(*,*) 'Check if SED-directory contains the file imf_bins.dat'
        endif
        call clean_stop
     end if
  end if
  ...
  ! READ IMF BINS---------------------------------------------------------
  if (imf_varies_complex) then
     open(unit=10,file=fIMFs,status='old',form='formatted')
     read(10,'(i8)') SED_nIMF
     allocate(SED_IMFs(SED_nIMF))
     do i = 1, SED_nIMF
        read(10,'(e14.6)') SED_IMFs(i)
     end do
     close(10)
  end if
  ! READ SEDS-------------------------------------------------------------
  if(KatzSED)then
     ...
     allocate(SEDs(nLs,nAges,nzs,SED_nIMF))
     do iz = 1, nzs
        do ia = 1, nAges
           do il = 1, nLs
              do ii = 1, SED_nIMF
                 read(10,'(e14.6)') SEDs(il,ia,iz,ii)
              end do
           end do
        end do
     end do
  ...
  allocate(tbl(nAges,nZs,SED_nIMF,nv))
  allocate(tmp_tbl(nAges-1,nZs,nv))

  if (oxygen_ions) then
     allocate(tbl_oxygen(nAges,nZs,SED_nIMF,2*n_oxygen_ions))
     allocate(tmp_tbl_oxygen(nAges-1,nZs,2*n_oxygen_ions))
  endif
  if (nitrogen_ions) then
     allocate(tbl_nitrogen(nAges,nZs,SED_nIMF,2*n_nitrogen_ions))
     allocate(tmp_tbl_nitrogen(nAges-1,nZs,2*n_nitrogen_ions))
  endif
  if (carbon_ions) then
     allocate(tbl_carbon(nAges,nZs,SED_nIMF,2*n_carbon_ions))
     allocate(tmp_tbl_carbon(nAges-1,nZs,2*n_carbon_ions))
  endif
  if (magnesium_ions) then
     allocate(tbl_magnesium(nAges,nZs,SED_nIMF,2*n_magnesium_ions))
     allocate(tmp_tbl_magnesium(nAges-1,nZs,2*n_magnesium_ions))
  endif
  if (silicon_ions) then
     allocate(tbl_silicon(nAges,nZs,SED_nIMF,2*n_silicon_ions))
     allocate(tmp_tbl_silicon(nAges-1,nZs,2*n_silicon_ions))
  endif
  if (sulfur_ions) then
     allocate(tbl_sulfur(nAges,nZs,SED_nIMF,2*n_sulfur_ions))
     allocate(tmp_tbl_sulfur(nAges-1,nZs,2*n_sulfur_ions))
  endif
  if (iron_ions) then
     allocate(tbl_iron(nAges,nZs,SED_nIMF,2*n_iron_ions))
     allocate(tmp_tbl_iron(nAges-1,nZs,2*n_iron_ions))
  endif
  if (neon_ions) then
     allocate(tbl_neon(nAges,nZs,SED_nIMF,2*n_neon_ions))
     allocate(tmp_tbl_neon(nAges-1,nZs,2*n_neon_ions))
  endif

  ! Dust table
  allocate(tbl_dust(nAges,nZs,SED_nIMF,2))
  allocate(tmp_tbl_dust(nAges-1,nZs,2))
  ...
  do ip = 1,nSEDgroups                                ! Loop photon groups
     ...
     do il = 1, SED_nIMF                                           ! Loop IMF
        tbl(ia,iz,il,1) = getSEDLuminosity(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1)
        tbl(ia,iz,il,3) = getSEDEgy(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1)
        do ii = 1,nIons                                     ! Loop species
           tbl(ia,iz,il,2+ii*2) = getSEDcsn(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,ii)
           tbl(ia,iz,il,3+ii*2) = getSEDcse(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,ii)
        end do ! End species loop

         ! LOOP OVER METALS
         if (oxygen_ions) then
            do ii=0,n_oxygen_ions-1 ! Loop over ionisation states
               tbl_oxygen(ia,iz,il,ii*2+1)   = getSEDcsn_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,1,ii+1)
               tbl_oxygen(ia,iz,il,1+ii*2+1) = getSEDcse_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,1,ii+1)
            enddo
         endif
         if (nitrogen_ions) then
            do ii=0,n_nitrogen_ions-1
               tbl_nitrogen(ia,iz,il,ii*2+1)   = getSEDcsn_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,2,ii+1)
               tbl_nitrogen(ia,iz,il,1+ii*2+1) = getSEDcse_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,2,ii+1)
            enddo
         endif
         if (carbon_ions) then
            do ii=0,n_carbon_ions-1
               tbl_carbon(ia,iz,il,ii*2+1)   = getSEDcsn_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,3,ii+1)
               tbl_carbon(ia,iz,il,1+ii*2+1) = getSEDcse_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,3,ii+1)
            enddo
         endif
         if (magnesium_ions) then
            do ii=0,n_magnesium_ions-1
               tbl_magnesium(ia,iz,il,ii*2+1)   = getSEDcsn_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,4,ii+1)
               tbl_magnesium(ia,iz,il,1+ii*2+1) = getSEDcse_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,4,ii+1)
            enddo
         endif
         if (silicon_ions) then
            do ii=0,n_silicon_ions-1
               tbl_silicon(ia,iz,il,ii*2+1)   = getSEDcsn_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,5,ii+1)
               tbl_silicon(ia,iz,il,1+ii*2+1) = getSEDcse_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,5,ii+1)
            enddo
         endif
         if (sulfur_ions) then
            do ii=0,n_sulfur_ions-1
               tbl_sulfur(ia,iz,il,ii*2+1)   = getSEDcsn_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,6,ii+1)
               tbl_sulfur(ia,iz,il,1+ii*2+1) = getSEDcse_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,6,ii+1)
            enddo
         endif
         if (iron_ions) then
            do ii=0,n_iron_ions-1
               tbl_iron(ia,iz,il,ii*2+1)   = getSEDcsn_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,7,ii+1)
               tbl_iron(ia,iz,il,1+ii*2+1) = getSEDcse_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,7,ii+1)
            enddo
         endif
         if (neon_ions) then
            do ii=0,n_neon_ions-1
               tbl_neon(ia,iz,il,ii*2+1)   = getSEDcsn_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,8,ii+1)
               tbl_neon(ia,iz,il,1+ii*2+1) = getSEDcse_metals(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,8,ii+1)
            enddo
         endif

         ! Now deal with dust
         tbl_dust(ia,iz,il,1)   = getSEDcsn_dust(Ls,SEDs(:,ia,iz,il),nLs,pL0,pL1,0)
...
#ifndef WITHOUTMPI
     allocate(tbl2(nAges,nzs,SED_nIMF,nv))
     call MPI_ALLREDUCE(tbl,tbl2,nAges*nzs*SED_nIMF*nv,&
          MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
     tbl = tbl2
     deallocate(tbl2)

     if (oxygen_ions) then
      allocate(tbl_oxygen2(nAges,nzs,SED_nIMF,2*n_oxygen_ions))
      call MPI_ALLREDUCE(tbl_oxygen,tbl_oxygen2,nAges*nzs*SED_nIMF*2*n_oxygen_ions,&
           MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
     ...
     if (nitrogen_ions) then
      allocate(tbl_nitrogen2(nAges,nzs,SED_nIMF,2*n_nitrogen_ions))
      call MPI_ALLREDUCE(tbl_nitrogen,tbl_nitrogen2,nAges*nzs*SED_nIMF*2*n_nitrogen_ions,&
           MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
     ...
     if (carbon_ions) then
      allocate(tbl_carbon2(nAges,nzs,SED_nIMF,2*n_carbon_ions))
      call MPI_ALLREDUCE(tbl_carbon,tbl_carbon2,nAges*nzs*SED_nIMF*2*n_carbon_ions,&
           MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
     ...
     if (magnesium_ions) then
      allocate(tbl_magnesium2(nAges,nzs,SED_nIMF,2*n_magnesium_ions))
      call MPI_ALLREDUCE(tbl_magnesium,tbl_magnesium2,nAges*nzs*SED_nIMF*2*n_magnesium_ions,&
           MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
     ...
     if (silicon_ions) then
      allocate(tbl_silicon2(nAges,nzs,SED_nIMF,2*n_silicon_ions))
      call MPI_ALLREDUCE(tbl_silicon,tbl_silicon2,nAges*nzs*SED_nIMF*2*n_silicon_ions,&
           MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
     ...
     if (sulfur_ions) then
      allocate(tbl_sulfur2(nAges,nzs,SED_nIMF,2*n_sulfur_ions))
      call MPI_ALLREDUCE(tbl_sulfur,tbl_sulfur2,nAges*nzs*SED_nIMF*2*n_sulfur_ions,&
           MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
     ...
     if (iron_ions) then
      allocate(tbl_iron2(nAges,nzs,SED_nIMF,2*n_iron_ions))
      call MPI_ALLREDUCE(tbl_iron,tbl_iron2,nAges*nzs*SED_nIMF*2*n_iron_ions,&
           MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
      tbl_iron = tbl_iron2
     ...
     if (neon_ions) then
      allocate(tbl_neon2(nAges,nzs,SED_nIMF,2*n_neon_ions))
      call MPI_ALLREDUCE(tbl_neon,tbl_neon2,nAges*nzs*SED_nIMF*2*n_neon_ions,&
           MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
      tbl_neon = tbl_neon2
      deallocate(tbl_neon2)
     endif

     allocate(tbl2_dust(nAges,nzs,SED_nIMF,2))
     call MPI_ALLREDUCE(tbl_dust,tbl2_dust,nAges*nzs*SED_nIMF*2,&
          MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
     tbl_dust = tbl2_dust
     ...
     do iz = 1, nzs ! Loop metallicity
        do il = 1,SED_nIMF ! Loop IMF
           tmp = trapz1( ages, tbl(:,iz,il,1), nAges, tbl(:,iz,il,2) )
        end do
     end do
     ...
     ! Loop over IMFs --> these will be interpolated later
     do il = 1,SED_nIMF
        if (myid.eq.1) write(*,*) "Working with IMF",il,"and group",ip

        tmp_tbl(:,:,:) = tbl(2:nAges,:,il,:) ! Select the table at a fixed IMF
        call rebin_log(dlgA, SED_dlgZ                                 &
             , tmp_tbl, nAges-1, nZs, ages(2:nAges), zs, nv        &
             , reb_tbl, SED_nA, SED_nZ, rebAges, SED_Zeds)

        if (oxygen_ions) then
           tmp_tbl_oxygen(:,:,:) = tbl_oxygen(2:nAges,:,il,:) ! Select the table at a fixed IMF
           call rebin_log(dlgA, SED_dlgZ                                  &
           , tmp_tbl_oxygen, nAges-1, nZs, ages(2:nAges), zs, 2*n_oxygen_ions   &
           , reb_tbl_oxygen, SED_nA, SED_nZ, rebAges, SED_Zeds)
        endif

        if (nitrogen_ions) then
           tmp_tbl_nitrogen(:,:,:) = tbl_nitrogen(2:nAges,:,il,:) ! Select the table at a fixed IMF
           call rebin_log(dlgA, SED_dlgZ                                  &
           , tmp_tbl_nitrogen, nAges-1, nZs, ages(2:nAges), zs, 2*n_nitrogen_ions   &
           , reb_tbl_nitrogen, SED_nA, SED_nZ, rebAges, SED_Zeds)
        endif

        if (carbon_ions) then
           tmp_tbl_carbon(:,:,:) = tbl_carbon(2:nAges,:,il,:) ! Select the table at a fixed IMF
           call rebin_log(dlgA, SED_dlgZ                                  &
           , tmp_tbl_carbon, nAges-1, nZs, ages(2:nAges), zs, 2*n_carbon_ions   &
           , reb_tbl_carbon, SED_nA, SED_nZ, rebAges, SED_Zeds)
        endif

        if (magnesium_ions) then
           tmp_tbl_magnesium(:,:,:) = tbl_magnesium(2:nAges,:,il,:) ! Select the table at a fixed IMF
           call rebin_log(dlgA, SED_dlgZ                                  &
           , tmp_tbl_magnesium, nAges-1, nZs, ages(2:nAges), zs, 2*n_magnesium_ions   &
           , reb_tbl_magnesium, SED_nA, SED_nZ, rebAges, SED_Zeds)
        endif

        if (silicon_ions) then
           tmp_tbl_silicon(:,:,:) = tbl_silicon(2:nAges,:,il,:) ! Select the table at a fixed IMF
           call rebin_log(dlgA, SED_dlgZ                                  &
           , tmp_tbl_silicon, nAges-1, nZs, ages(2:nAges), zs, 2*n_silicon_ions   &
           , reb_tbl_silicon, SED_nA, SED_nZ, rebAges, SED_Zeds)
        endif

        if (sulfur_ions) then
           tmp_tbl_sulfur(:,:,:) = tbl_sulfur(2:nAges,:,il,:) ! Select the table at a fixed IMF
           call rebin_log(dlgA, SED_dlgZ                                  &
           , tmp_tbl_sulfur, nAges-1, nZs, ages(2:nAges), zs, 2*n_sulfur_ions   &
           , reb_tbl_sulfur, SED_nA, SED_nZ, rebAges, SED_Zeds)
        endif

        if (iron_ions) then
           tmp_tbl_iron(:,:,:) = tbl_iron(2:nAges,:,il,:) ! Select the table at a fixed IMF
           call rebin_log(dlgA, SED_dlgZ                                  &
           , tmp_tbl_iron, nAges-1, nZs, ages(2:nAges), zs, 2*n_iron_ions   &
           , reb_tbl_iron, SED_nA, SED_nZ, rebAges, SED_Zeds)
        endif

        if (neon_ions) then
           tmp_tbl_neon(:,:,:) = tbl_neon(2:nAges,:,il,:) ! Select the table at a fixed IMF
           call rebin_log(dlgA, SED_dlgZ                                  &
           , tmp_tbl_neon, nAges-1, nZs, ages(2:nAges), zs, 2*n_neon_ions   &
           , reb_tbl_neon, SED_nA, SED_nZ, rebAges, SED_Zeds)
        endif

        tmp_tbl_dust(:,:,:) = tbl_dust(2:nAges,:,il,:) ! Select the table at a fixed IMF
        call rebin_log(dlgA, SED_dlgZ                                  &
        , tmp_tbl_dust, nAges-1, nZs, ages(2:nAges), zs, 2   &
        , reb_tbl_dust, SED_nA, SED_nZ, rebAges, SED_Zeds)

        SED_nA=SED_nA+1                              ! Make room for zero age
        if(ip .eq. 1 .and. il .eq. 1) then ! only do this on first group and IMF
           allocate(SED_table(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, nv))

           if (oxygen_ions)    allocate(SED_table_oxygen(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, 2*n_oxygen_ions))
           if (nitrogen_ions)  allocate(SED_table_nitrogen(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, 2*n_nitrogen_ions))
           if (carbon_ions)    allocate(SED_table_carbon(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, 2*n_carbon_ions))
           if (magnesium_ions) allocate(SED_table_magnesium(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, 2*n_magnesium_ions))
           if (silicon_ions)   allocate(SED_table_silicon(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, 2*n_silicon_ions))
           if (sulfur_ions)    allocate(SED_table_sulfur(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, 2*n_sulfur_ions))
           if (iron_ions)      allocate(SED_table_iron(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, 2*n_iron_ions))
           if (neon_ions)      allocate(SED_table_neon(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, 2*n_neon_ions))

           allocate(SED_table_dust(SED_nA, SED_nZ, SED_nIMF, nSEDgroups, 2))

        endif

        SED_table(1, :, il, ip, :) = reb_tbl(1,:,:)          ! Zero age properties
        SED_table(1, :, il, ip, 2) = 0.                        !  Lacc=0 at zero age
        SED_table(2:,:, il, ip, :) = reb_tbl

        if (oxygen_ions) then
           SED_table_oxygen(1, :, il, ip, :) = reb_tbl_oxygen(1,:,:)          ! Zero age properties
           SED_table_oxygen(2:,:, il, ip, :) = reb_tbl_oxygen
        endif
        if (nitrogen_ions) then
           SED_table_nitrogen(1, :, il, ip, :) = reb_tbl_nitrogen(1,:,:)      ! Zero age properties
           SED_table_nitrogen(2:,:, il, ip, :) = reb_tbl_nitrogen
        endif
        if (carbon_ions) then
           SED_table_carbon(1, :, il, ip, :) = reb_tbl_carbon(1,:,:)          ! Zero age properties
           SED_table_carbon(2:,:, il, ip, :) = reb_tbl_carbon
        endif
        if (magnesium_ions) then
           SED_table_magnesium(1, :, il, ip, :) = reb_tbl_magnesium(1,:,:)    ! Zero age properties
           SED_table_magnesium(2:,:, il, ip, :) = reb_tbl_magnesium
        endif
        if (silicon_ions) then
           SED_table_silicon(1, :, il, ip, :) = reb_tbl_silicon(1,:,:)        ! Zero age properties
           SED_table_silicon(2:,:, il, ip, :) = reb_tbl_silicon
        endif
        if (sulfur_ions) then
           SED_table_sulfur(1, :, il, ip, :) = reb_tbl_sulfur(1,:,:)          ! Zero age properties
           SED_table_sulfur(2:,:, il, ip, :) = reb_tbl_sulfur
        endif
        if (iron_ions) then
           SED_table_iron(1, :, il, ip, :) = reb_tbl_iron(1,:,:)              ! Zero age properties
           SED_table_iron(2:,:, il, ip, :) = reb_tbl_iron
        endif
        if (neon_ions) then
           SED_table_neon(1, :, il, ip, :) = reb_tbl_neon(1,:,:)              ! Zero age properties
           SED_table_neon(2:,:, il, ip, :) = reb_tbl_neon
        endif

        SED_table_dust(1, :, il, ip, :) = reb_tbl_dust(1,:,:)                   ! Zero age properties
        SED_table_dust(2:,:, il, ip, :) = reb_tbl_dust

     end do ! End loop over IMFs
  ...
  if (oxygen_ions) then
     deallocate(reb_tbl_oxygen) 
     deallocate(tmp_tbl_oxygen)
  end if
  if (nitrogen_ions) then  
     deallocate(reb_tbl_nitrogen) 
     deallocate(tmp_tbl_nitrogen)
  end if
  if (carbon_ions) then    
     deallocate(reb_tbl_carbon) 
     deallocate(tmp_tbl_carbon)
  end if
  if (magnesium_ions) then 
     deallocate(reb_tbl_magnesium) 
     deallocate(tmp_tbl_magnesium)
  end if
  if (silicon_ions) then
     deallocate(reb_tbl_silicon) 
     deallocate(tmp_tbl_silicon)
  end if
  if (sulfur_ions) then
     deallocate(reb_tbl_sulfur) 
     deallocate(tmp_tbl_sulfur)
  end if
  if (iron_ions) then      
     deallocate(reb_tbl_iron) 
     deallocate(tmp_tbl_iron)
  end if
  if (neon_ions) then     
     deallocate(reb_tbl_neon) 
     deallocate(tmp_tbl_neon)
  end if

  deallocate(reb_tbl_dust) 
  deallocate(tmp_tbl_dust)

  SED_lgZ0 = log10(SED_Zeds(1))                  ! Interpolation intervals
  SED_lgA0 = log10(rebAges(1))
  if (SED_nIMF .gt. 1) then
     SED_IMF0 = SED_IMFs(1)
     SED_dIMF = SED_IMFs(2) - SED_IMFs(1) ! NOTE: THIS ASSUMES EQUALLY SPACED IMFS
  else
     SED_dIMF = 1.d0 ! Set to a dummy value if not needed  
  end if
...
!-------------------------------------------------------------------------
  ...
  real(dp):: mass, age, Z, loc_imf_slope
...
!-------------------------------------------------------------------------
  ...
  do i=1,npartmax
     ...
     if(imf_varies_complex) loc_imf_slope = part_imf_slope(i)
     ...
     if(pop3 .and. is_pop_III(typep(i)))then
        ...
        call inp_SED_table(age, Z, loc_imf_slope, 1, .false., L_star)     !  [# s-1 m_sun-1]
        call inp_SED_table(age, Z, loc_imf_slope, 3, .true., egy_star(:)) !             [ev]
        do ii=1,nIons
           call inp_SED_table(age, Z, loc_imf_slope, 2+2*ii, .true., csn_star(:,ii))! [cm^2]
           call inp_SED_table(age, Z, loc_imf_slope, 3+2*ii, .true., cse_star(:,ii))! [cm^2]
        end do

        if (oxygen_ions) then
         do ii=1,n_oxygen_ions
            call inp_SED_table_metals(age, Z, loc_imf_slope,   2*ii-1, .false., 1, ii, csn_star_oxygen(:,ii))! [cm^2]
            call inp_SED_table_metals(age, Z, loc_imf_slope, 1+2*ii-1, .true., 1, ii, cse_star_oxygen(:,ii))! [cm^2]
         enddo
        endif

        if (nitrogen_ions) then
         do ii=1,n_nitrogen_ions
            call inp_SED_table_metals(age, Z, loc_imf_slope,   2*ii-1, .false., 2, ii, csn_star_nitrogen(:,ii))! [cm^2]
            call inp_SED_table_metals(age, Z, loc_imf_slope, 1+2*ii-1, .true., 2, ii, cse_star_nitrogen(:,ii))! [cm^2]
         enddo
        endif

        if (carbon_ions) then
         do ii=1,n_carbon_ions
            call inp_SED_table_metals(age, Z, loc_imf_slope,   2*ii-1, .false., 3, ii, csn_star_carbon(:,ii))! [cm^2]
            call inp_SED_table_metals(age, Z, loc_imf_slope, 1+2*ii-1, .true., 3, ii, cse_star_carbon(:,ii))! [cm^2]
         enddo
        endif

        if (magnesium_ions) then
         do ii=1,n_magnesium_ions
            call inp_SED_table_metals(age, Z, loc_imf_slope,   2*ii-1, .false., 4, ii, csn_star_magnesium(:,ii))! [cm^2]
            call inp_SED_table_metals(age, Z, loc_imf_slope, 1+2*ii-1, .true., 4, ii, cse_star_magnesium(:,ii))! [cm^2]
         enddo
        endif

        if (silicon_ions) then
         do ii=1,n_silicon_ions
            call inp_SED_table_metals(age, Z, loc_imf_slope,   2*ii-1, .false., 5, ii, csn_star_silicon(:,ii))! [cm^2]
            call inp_SED_table_metals(age, Z, loc_imf_slope, 1+2*ii-1, .true., 5, ii, cse_star_silicon(:,ii))! [cm^2]
         enddo
        endif

        if (sulfur_ions) then
         do ii=1,n_sulfur_ions
            call inp_SED_table_metals(age, Z, loc_imf_slope,   2*ii-1, .false., 6, ii, csn_star_sulfur(:,ii))! [cm^2]
            call inp_SED_table_metals(age, Z, loc_imf_slope, 1+2*ii-1, .true., 6, ii, cse_star_sulfur(:,ii))! [cm^2]
         enddo
        endif

        if (iron_ions) then
         do ii=1,n_iron_ions
            call inp_SED_table_metals(age, Z, loc_imf_slope,   2*ii-1, .false., 7, ii, csn_star_iron(:,ii))! [cm^2]
            call inp_SED_table_metals(age, Z, loc_imf_slope, 1+2*ii-1, .true., 7, ii, cse_star_iron(:,ii))! [cm^2]
         enddo
        endif

        if (neon_ions) then
         do ii=1,n_neon_ions
            call inp_SED_table_metals(age, Z, loc_imf_slope,   2*ii-1, .false., 8, ii, csn_star_neon(:,ii))! [cm^2]
            call inp_SED_table_metals(age, Z, loc_imf_slope, 1+2*ii-1, .true., 8, ii, cse_star_neon(:,ii))! [cm^2]
         enddo
        endif

        call inp_SED_table_dust(age, Z, loc_imf_slope, 1, .false., csn_star_dust(:,1))! [cm^2]
  ...
  do ip=1,nSEDgroups
     ...
     if(sum_L_all(ip) .gt. 0.) then
        ...
        group_egy(ip)       = SED_table(1,1,1,ip,3)
        do ii=1,nIons
           group_csn(ip,ii) = SED_table(1,1,1,ip,2+2*ii)
           group_cse(ip,ii) = SED_table(1,1,1,ip,3+2*ii)
        enddo

        if (oxygen_ions) then
         do ii=1,n_oxygen_ions
            group_csn_oxygen(ip,ii) = SED_table_oxygen(1,1,1,ip,  2*ii-1)
            group_cse_oxygen(ip,ii) = SED_table_oxygen(1,1,1,ip,1+2*ii-1)
         enddo
        endif

        if (nitrogen_ions) then
         do ii=1,n_nitrogen_ions
            group_csn_nitrogen(ip,ii) = SED_table_nitrogen(1,1,1,ip,  2*ii-1)
            group_cse_nitrogen(ip,ii) = SED_table_nitrogen(1,1,1,ip,1+2*ii-1)
         enddo
        endif

        if (carbon_ions) then
         do ii=1,n_carbon_ions
            group_csn_carbon(ip,ii) = SED_table_carbon(1,1,1,ip,  2*ii-1)
            group_cse_carbon(ip,ii) = SED_table_carbon(1,1,1,ip,1+2*ii-1)
         enddo
        endif

        if (magnesium_ions) then
         do ii=1,n_magnesium_ions
            group_csn_magnesium(ip,ii) = SED_table_magnesium(1,1,1,ip,  2*ii-1)
            group_cse_magnesium(ip,ii) = SED_table_magnesium(1,1,1,ip,1+2*ii-1)
         enddo
        endif

        if (silicon_ions) then
         do ii=1,n_silicon_ions
            group_csn_silicon(ip,ii) = SED_table_silicon(1,1,1,ip,  2*ii-1)
            group_cse_silicon(ip,ii) = SED_table_silicon(1,1,1,ip,1+2*ii-1)
         enddo
        endif

        if (sulfur_ions) then
         do ii=1,n_sulfur_ions
            group_csn_sulfur(ip,ii) = SED_table_sulfur(1,1,1,ip,  2*ii-1)
            group_cse_sulfur(ip,ii) = SED_table_sulfur(1,1,1,ip,1+2*ii-1)
         enddo
        endif

        if (iron_ions) then
         do ii=1,n_iron_ions
            group_csn_iron(ip,ii) = SED_table_iron(1,1,1,ip,  2*ii-1)
            group_cse_iron(ip,ii) = SED_table_iron(1,1,1,ip,1+2*ii-1)
         enddo
        endif

        if (neon_ions) then
         do ii=1,n_neon_ions
            group_csn_neon(ip,ii) = SED_table_neon(1,1,1,ip,  2*ii-1)
            group_cse_neon(ip,ii) = SED_table_neon(1,1,1,ip,1+2*ii-1)
         enddo
        endif

        group_csn_dust(ip,1) = SED_table_dust(1,1,1,ip,  1)
...
!-------------------------------------------------------------------------
  do ip=1,nSEDgroups
     ...
     write(10,*) SED_nA, SED_nZ, SED_nIMF

     do k = 1,SED_nIMF
        do j = 1,SED_nz
           do i = 1,SED_nA
              tmp_imf = -999.d0
              if (SED_nIMF.gt.1) tmp_imf = SED_IMFs(k) 
              write(10,900)                                              &
                   SED_ages(i)        ,    SED_zeds(j)        ,          &
                   tmp_imf            ,                                  &
                   SED_table(i,j,k,ip,1),    SED_table(i,j,k,ip,2),      &
                   SED_table(i,j,k,ip,3),    SED_table(i,j,k,ip,4),      &
                   SED_table(i,j,k,ip,5),    SED_table(i,j,k,ip,6),      &
                   SED_table(i,j,k,ip,7),    SED_table(i,j,k,ip,8),      &
                   SED_table(i,j,k,ip,9)
           end do
        end do
     end do
     close(10)
  end do
900 format (ES15.4, ES15.4, ES15.4, ES15.4, ES15.4, f15.4                &
        ...
        do j = 1,SED_nz
           do i = 1,SED_nA
              do k=1,2*n_oxygen_ions-1
                 write(10,900,advance="no") SED_table_oxygen(i,j,l,ip,k)
              end do
              write(10,900) SED_table_oxygen(i,j,l,ip,2*n_oxygen_ions)
           end do
        end do
     end do
     close(10)
        ...
        do j = 1,SED_nz
           do i = 1,SED_nA
              do k=1,2*n_nitrogen_ions-1
                 write(10,900,advance="no") SED_table_nitrogen(i,j,l,ip,k)
              end do
              write(10,900) SED_table_nitrogen(i,j,l,ip,2*n_nitrogen_ions)
           end do
        end do
     end do
     close(10)
        ...
        do j = 1,SED_nz
           do i = 1,SED_nA
              do k=1,2*n_carbon_ions-1
                 write(10,900,advance="no") SED_table_carbon(i,j,l,ip,k)
              end do
              write(10,900) SED_table_carbon(i,j,l,ip,2*n_carbon_ions)
           end do
        end do
     end do
     close(10)
        ...
        do j = 1,SED_nz
           do i = 1,SED_nA
              do k=1,2*n_magnesium_ions-1
                 write(10,900,advance="no") SED_table_magnesium(i,j,l,ip,k)
              end do
              write(10,900) SED_table_magnesium(i,j,l,ip,2*n_magnesium_ions)
           end do
        end do
     end do
     close(10)
        ...
        do j = 1,SED_nz
           do i = 1,SED_nA
              do k=1,2*n_silicon_ions-1
                 write(10,900,advance="no") SED_table_silicon(i,j,l,ip,k)
              end do
              write(10,900) SED_table_silicon(i,j,l,ip,2*n_silicon_ions)
           end do
        end do
     end do
     close(10)
        ...
        do j = 1,SED_nz
           do i = 1,SED_nA
              do k=1,2*n_sulfur_ions-1
                 write(10,900,advance="no") SED_table_sulfur(i,j,l,ip,k)
              end do
              write(10,900) SED_table_sulfur(i,j,l,ip,2*n_sulfur_ions-1)
           end do
        end do
     end do
     close(10)
        ...
        do j = 1,SED_nz
           do i = 1,SED_nA
              do k=1,2*n_iron_ions-1
                 write(10,900,advance="no") SED_table_iron(i,j,l,ip,k)
              end do
              write(10,900) SED_table_iron(i,j,l,ip,2*n_iron_ions)
           end do
        end do
     end do
     close(10)
         ...
         do j = 1,SED_nz
            do i = 1,SED_nA
               do k=1,2*n_neon_ions-1
                  write(10,900,advance="no") SED_table_neon(i,j,l,ip,k)
               end do
               write(10,900) SED_table_neon(i,j,l,ip,2*n_neon_ions)
            end do
         end do
      end do
      close(10)
     ...
     do ip=1,nSEDgroups
        ...
        do l = 1,SED_nIMF
           do j = 1,SED_nz
              do i = 1,SED_nA
                 tmp_imf=-999.d0
                 if (SED_nIMF.gt.1) tmp_imf = SED_IMFs(l)
                 write(10,900)                                              &
                      SED_ages(i)        ,    SED_zeds(j)        ,          &
                      tmp_imf            ,                                  &
                      SED_table_dust(i,j,l,ip,1)
              end do
           end do
        end do
        close(10)
...
!*************************************************************************
SUBROUTINE inp_SED_table(age, Z, my_imf_slope, nProp, same, ret)
...
!-------------------------------------------------------------------------
  use amr_commons
  use rt_parameters
  real(dp), intent(in):: age, Z, my_imf_slope
  real(dp):: lgAge, lgZ
  integer:: nProp
  logical:: same
  real(dp),dimension(:):: ret
  integer,save:: ia, iz, ii
  real(dp),save:: da, da0, da1, dz, dz0, dz1, di, di0, di1
!-------------------------------------------------------------------------
  ...
  if(.not. same) then
     ...
     if (SED_nIMF .gt.1) then
        ii = min(max(floor((my_imf_slope-SED_IMF0)/SED_dIMF ) + 1,   1  ),  SED_nIMF-1 )
        di = SED_IMFs(ii+1)-SED_IMFs(ii)
        di0= min( max(   (my_imf_slope-SED_IMFs(ii)) /di,         0. ),  1.         )
        di1= min( max(  (SED_IMFs(ii+1)-my_imf_slope)/di,         0. ),  1.         )
     else
        ii = 1
        di0 = 0.d0
        di1 = 1.d0
     ...
     if (abs(da0+da1-1.0d0) > 1.0d-5 .or. abs(dz0+dz1-1.0d0) > 1.0d-5 .or. abs(di0+di1-1.0d0) > 1.0d-5) then
        write(*,*) 'Screwed up the sed interpolation ... '
        write(*,*) da0+da1,dz0+dz1,di0+di1
        call clean_stop
     end if
  endif

  ret = da0 * dz0 * di1 * SED_table(ia+1, iz+1, ii, :, nProp) + &
        da1 * dz0 * di1 * SED_table(ia,   iz+1, ii, :, nProp) + &
        da0 * dz1 * di1 * SED_table(ia+1, iz,   ii, :, nProp) + &
        da1 * dz1 * di1 * SED_table(ia,   iz,   ii, :, nProp)  
  if (SED_nIMF .gt.1) then
      ret = ret +                                                     & 
            da0 * dz0 * di0 * SED_table(ia+1, iz+1, ii+1, :, nProp) + &
            da1 * dz0 * di0 * SED_table(ia,   iz+1, ii+1, :, nProp) + &
            da0 * dz1 * di0 * SED_table(ia+1, iz,   ii+1, :, nProp) + &
            da1 * dz1 * di0 * SED_table(ia,   iz,   ii+1, :, nProp)
  end if

END SUBROUTINE inp_SED_table

!*************************************************************************
SUBROUTINE inp_SED_table_metals(age, Z, my_imf_slope, nProp, same, species, ion_state, ret)
   ...
   !-------------------------------------------------------------------------
     ...
     if (species.eq.1) then ! oxygen
        ret = da0 * dz0 * di1 * SED_table_oxygen(ia+1, iz+1, ii  , :, nProp) + &
              da1 * dz0 * di1 * SED_table_oxygen(ia,   iz+1, ii  , :, nProp) + &
              da0 * dz1 * di1 * SED_table_oxygen(ia+1, iz,   ii  , :, nProp) + &
              da1 * dz1 * di1 * SED_table_oxygen(ia,   iz,   ii  , :, nProp)  
        if (SED_nIMF .gt.1) then
           ret = ret +                                                            & 
                 da0 * dz0 * di0 * SED_table_oxygen(ia+1, iz+1, ii+1, :, nProp) + &
                 da1 * dz0 * di0 * SED_table_oxygen(ia,   iz+1, ii+1, :, nProp) + &
                 da0 * dz1 * di0 * SED_table_oxygen(ia+1, iz,   ii+1, :, nProp) + &
                 da1 * dz1 * di0 * SED_table_oxygen(ia,   iz,   ii+1, :, nProp)
        end if
     else if (species.eq.2) then ! nitrogen
        ret = da0 * dz0 * di1 * SED_table_nitrogen(ia+1, iz+1, ii  , :, nProp) + &
              da1 * dz0 * di1 * SED_table_nitrogen(ia,   iz+1, ii  , :, nProp) + &
              da0 * dz1 * di1 * SED_table_nitrogen(ia+1, iz,   ii  , :, nProp) + &
              da1 * dz1 * di1 * SED_table_nitrogen(ia,   iz,   ii  , :, nProp)  
        if (SED_nIMF .gt.1) then
           ret = ret +                                                              & 
                 da0 * dz0 * di0 * SED_table_nitrogen(ia+1, iz+1, ii+1, :, nProp) + &
                 da1 * dz0 * di0 * SED_table_nitrogen(ia,   iz+1, ii+1, :, nProp) + &
                 da0 * dz1 * di0 * SED_table_nitrogen(ia+1, iz,   ii+1, :, nProp) + &
                 da1 * dz1 * di0 * SED_table_nitrogen(ia,   iz,   ii+1, :, nProp)
        end if  
     else if (species.eq.3) then ! carbon
        ret = da0 * dz0 * di1 * SED_table_carbon(ia+1, iz+1, ii  , :, nProp) + &
              da1 * dz0 * di1 * SED_table_carbon(ia,   iz+1, ii  , :, nProp) + &
              da0 * dz1 * di1 * SED_table_carbon(ia+1, iz,   ii  , :, nProp) + &
              da1 * dz1 * di1 * SED_table_carbon(ia,   iz,   ii  , :, nProp)  
        if (SED_nIMF .gt.1) then
           ret = ret +                                                            & 
                 da0 * dz0 * di0 * SED_table_carbon(ia+1, iz+1, ii+1, :, nProp) + &
                 da1 * dz0 * di0 * SED_table_carbon(ia,   iz+1, ii+1, :, nProp) + &
                 da0 * dz1 * di0 * SED_table_carbon(ia+1, iz,   ii+1, :, nProp) + &
                 da1 * dz1 * di0 * SED_table_carbon(ia,   iz,   ii+1, :, nProp)
        end if   
     else if (species.eq.4) then ! magnesium
        ret = da0 * dz0 * di1 * SED_table_magnesium(ia+1, iz+1, ii  , :, nProp) + &
              da1 * dz0 * di1 * SED_table_magnesium(ia,   iz+1, ii  , :, nProp) + &
              da0 * dz1 * di1 * SED_table_magnesium(ia+1, iz,   ii  , :, nProp) + &
              da1 * dz1 * di1 * SED_table_magnesium(ia,   iz,   ii  , :, nProp)  
        if (SED_nIMF .gt.1) then
           ret = ret +                                                               & 
                 da0 * dz0 * di0 * SED_table_magnesium(ia+1, iz+1, ii+1, :, nProp) + &
                 da1 * dz0 * di0 * SED_table_magnesium(ia,   iz+1, ii+1, :, nProp) + &
                 da0 * dz1 * di0 * SED_table_magnesium(ia+1, iz,   ii+1, :, nProp) + &
                 da1 * dz1 * di0 * SED_table_magnesium(ia,   iz,   ii+1, :, nProp)
        end if    
     else if (species.eq.5) then ! silicon
        ret = da0 * dz0 * di1 * SED_table_silicon(ia+1, iz+1, ii  , :, nProp) + &
              da1 * dz0 * di1 * SED_table_silicon(ia,   iz+1, ii  , :, nProp) + &
              da0 * dz1 * di1 * SED_table_silicon(ia+1, iz,   ii  , :, nProp) + &
              da1 * dz1 * di1 * SED_table_silicon(ia,   iz,   ii  , :, nProp)  
        if (SED_nIMF .gt.1) then
           ret = ret +                                                             & 
                 da0 * dz0 * di0 * SED_table_silicon(ia+1, iz+1, ii+1, :, nProp) + &
                 da1 * dz0 * di0 * SED_table_silicon(ia,   iz+1, ii+1, :, nProp) + &
                 da0 * dz1 * di0 * SED_table_silicon(ia+1, iz,   ii+1, :, nProp) + &
                 da1 * dz1 * di0 * SED_table_silicon(ia,   iz,   ii+1, :, nProp)
        end if   
     else if (species.eq.6) then ! sulfur
        ret = da0 * dz0 * di1 * SED_table_sulfur(ia+1, iz+1, ii  , :, nProp) + &
              da1 * dz0 * di1 * SED_table_sulfur(ia,   iz+1, ii  , :, nProp) + &
              da0 * dz1 * di1 * SED_table_sulfur(ia+1, iz,   ii  , :, nProp) + &
              da1 * dz1 * di1 * SED_table_sulfur(ia,   iz,   ii  , :, nProp)  
        if (SED_nIMF .gt.1) then
           ret = ret +                                                            & 
                 da0 * dz0 * di0 * SED_table_sulfur(ia+1, iz+1, ii+1, :, nProp) + &
                 da1 * dz0 * di0 * SED_table_sulfur(ia,   iz+1, ii+1, :, nProp) + &
                 da0 * dz1 * di0 * SED_table_sulfur(ia+1, iz,   ii+1, :, nProp) + &
                 da1 * dz1 * di0 * SED_table_sulfur(ia,   iz,   ii+1, :, nProp)
        end if   
     else if (species.eq.7) then ! iron
        ret = da0 * dz0 * di1 * SED_table_iron(ia+1, iz+1, ii  , :, nProp) + &
              da1 * dz0 * di1 * SED_table_iron(ia,   iz+1, ii  , :, nProp) + &
              da0 * dz1 * di1 * SED_table_iron(ia+1, iz,   ii  , :, nProp) + &
              da1 * dz1 * di1 * SED_table_iron(ia,   iz,   ii  , :, nProp)  
        if (SED_nIMF .gt.1) then
           ret = ret +                                                          & 
                 da0 * dz0 * di0 * SED_table_iron(ia+1, iz+1, ii+1, :, nProp) + &
                 da1 * dz0 * di0 * SED_table_iron(ia,   iz+1, ii+1, :, nProp) + &
                 da0 * dz1 * di0 * SED_table_iron(ia+1, iz,   ii+1, :, nProp) + &
                 da1 * dz1 * di0 * SED_table_iron(ia,   iz,   ii+1, :, nProp)
        end if    
     else if (species.eq.8) then ! neon
        ret = da0 * dz0 * di1 * SED_table_neon(ia+1, iz+1, ii  , :, nProp) + &
              da1 * dz0 * di1 * SED_table_neon(ia,   iz+1, ii  , :, nProp) + &
              da0 * dz1 * di1 * SED_table_neon(ia+1, iz,   ii  , :, nProp) + &
              da1 * dz1 * di1 * SED_table_neon(ia,   iz,   ii  , :, nProp)  
        if (SED_nIMF .gt.1) then
           ret = ret +                                                          & 
                 da0 * dz0 * di0 * SED_table_neon(ia+1, iz+1, ii+1, :, nProp) + &
                 da1 * dz0 * di0 * SED_table_neon(ia,   iz+1, ii+1, :, nProp) + &
                 da0 * dz1 * di0 * SED_table_neon(ia+1, iz,   ii+1, :, nProp) + &
                 da1 * dz1 * di0 * SED_table_neon(ia,   iz,   ii+1, :, nProp)
        end if   
     endif

END SUBROUTINE inp_SED_table_metals

SUBROUTINE inp_SED_table_dust(age, Z, my_imf_slope, nProp, same, ret)
   ...
   !-------------------------------------------------------------------------
     ...
     ret = da0 * dz0 * di1 * SED_table_dust(ia+1, iz+1, ii  , :, nProp) + &
           da1 * dz0 * di1 * SED_table_dust(ia,   iz+1, ii  , :, nProp) + &
           da0 * dz1 * di1 * SED_table_dust(ia+1, iz,   ii  , :, nProp) + &
           da1 * dz1 * di1 * SED_table_dust(ia,   iz,   ii  , :, nProp)  
     if (SED_nIMF .gt.1) then
        ret = ret +                                                          & 
              da0 * dz0 * di0 * SED_table_dust(ia+1, iz+1, ii+1, :, nProp) + &
              da1 * dz0 * di0 * SED_table_dust(ia,   iz+1, ii+1, :, nProp) + &
              da0 * dz1 * di0 * SED_table_dust(ia+1, iz,   ii+1, :, nProp) + &
              da1 * dz1 * di0 * SED_table_dust(ia,   iz,   ii+1, :, nProp)
   end if  
   END SUBROUTINE inp_SED_table_dust

!*************************************************************************
SUBROUTINE getNPhotonsEmitted(age1_Gyr, dt_Gyr, Z, my_imf_slope, ret)
...
!-------------------------------------------------------------------------
  use rt_parameters
  real(dp),intent(in):: age1_Gyr, dt_Gyr, Z, my_imf_slope
  real(dp),dimension(nSEDgroups):: ret, Lc0, Lc1, SEDegy
!-------------------------------------------------------------------------
  ! Lc0 = cumulative emitted photons at the start of the timestep
  call inp_SED_table(age1_Gyr-dt_Gyr, Z, my_imf_slope, 2, .false., Lc0)
  ! Lc1 = cumulative emitted photons at the end of the timestep
  call inp_SED_table(age1_Gyr, Z, my_imf_slope, 2, .false., Lc1)
  ret = max(Lc1-Lc0,0.)
...
!-------------------------------------------------------------------------
  ...
  use rt_parameters
  use metal_yields, only: table_Solar_Fe
  ...
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v, scale_Np  &
            ...
            , dt_loc_Gyr, scale_msun, mass, loc_imf_slope
...
!-------------------------------------------------------------------------
  ...
  do j = 1, np
     ...
     if(metal) then
        ...
        if(imf_varies_complex) z = (zp(ind_part(j), 1) / 0.76d0 )/table_Solar_Fe 
     else
        z=z_ave*0.02
     endif
     ...
     loc_imf_slope = 2.3d0 ! initialization
     if(imf_varies_complex) loc_imf_slope = part_imf_slope(ind_part(j))
     ...
     if(pop3.and.is_pop_III(typep(ind_part(j)))) then
        ...
        z= max(z, 10.d-5)                             !      [m_metals/m_tot]
        call getNPhotonsEmitted(age,dt_loc_Gyr,z,loc_imf_slope,part_NpInp(j,1:nSEDgroups))
