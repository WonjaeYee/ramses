...
!   delz=deltaz_frame(1)+deltaz_frame(2)/aexp !+deltaz_frame(3)*aexp**2+deltaz_frame(4)*aexp**3
  ...
  allocate(el_data_frame(1:nw_frame,1:nh_frame,1:NEMISSIONLINES))
  el_data_frame(:,:,:) = 0d0
#endif
                    ...
                    do jj=jmin,jmax
                       ...
                       if(rt) then
                          ...
                          if (SUM(el_movie_vars).ge.1) then 
                             ...
                             if(el_movie_vars(1).eq.1) el_data_frame(ii,jj,1) = el_data_frame(ii,jj,1) + get_halpha_lum(temp,ne,nHII,nHI,cv)
                             if(el_movie_vars(2).eq.1) el_data_frame(ii,jj,2) = el_data_frame(ii,jj,2) + get_hbeta_lum(temp,ne,nHII,nHI,cv)
                             if (oxygen_ions) then 
                                nO =  uold(ind_cell(i),imetal+1) * scale_d / (mO_NIST_amu * amu_to_g)

                                if (temp.lt.1.d5 .and. simple_dust_depletion .and. nO.gt.0.d0) then 
                                   d_mass = 0.0d0
                                ...
                                if(el_movie_vars(3).eq.1) el_data_frame(ii,jj,3) = el_data_frame(ii,jj,3) + get_OIII_5007_lum(temp,ne,nO3,cv)
                                if(el_movie_vars(4).eq.1) el_data_frame(ii,jj,4) = el_data_frame(ii,jj,4) + get_OIII_4959_lum(temp,ne,nO3,cv)
                                if(el_movie_vars(5).eq.1) el_data_frame(ii,jj,5) = el_data_frame(ii,jj,5) + get_OIII_4363_lum(temp,ne,nO3,cv)
                                if(el_movie_vars(6).eq.1) el_data_frame(ii,jj,6) = el_data_frame(ii,jj,6) + get_OII_3726_lum(temp,ne,nO2,cv)
                                if(el_movie_vars(7).eq.1) el_data_frame(ii,jj,7) = el_data_frame(ii,jj,7) + get_OII_3728_lum(temp,ne,nO2,cv)
                             endif
                             if (nitrogen_ions) then
                                ...
                                if(el_movie_vars(8).eq.1) el_data_frame(ii,jj,8) = el_data_frame(ii,jj,8) + get_NII_6583_lum(temp,ne,nN2,cv)
                             endif
                          endif
                       endif
#endif
...
!  end do
  ...
  allocate(el_data_frame_all(1:nw_frame,1:nh_frame,1:NEMISSIONLINES))
  el_data_frame_all(:,:,:)=0d0
  call MPI_ALLREDUCE(el_data_frame,el_data_frame_all        &
      ,nw_frame*nh_frame*NEMISSIONLINES,MPI_DOUBLE_PRECISION      &
      ,MPI_SUM,MPI_COMM_WORLD,info)
...
!     endif
      ...
      do kk=1, NEMISSIONLINES
         if (el_movie_vars(kk).eq.1) then
            ...
            data_single(:,:)=0.
            data_single=log10(MAX(el_data_frame(:,:,kk),el_min))
            rewind(ilun)  
            if(tendmov>0)then
              write(ilun)t,delx,dely,delz
            else
