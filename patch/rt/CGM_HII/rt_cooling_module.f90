...
module rt_cooling_module
   ...
   use cooling_module, only: X, Y, cmp_metals_cloudy
   use rt_parameters
...
!XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
   SUBROUTINE rt_solve_cooling(T2, xion, Np, Fp, p_gas, dNpdt, dFpdt        &
                            ...
                            &  , nSilicon, nSulfur, nIron, nNeon, dx_loc )
...
!-------------------------------------------------------------------------
      ...
      real(dp), dimension(1:nvector):: nH, Zsolar, dx_SS
      ...
      real(dp), dimension(1:nvector)::nSilicon, nSulfur, nIron, nNeon ! number densities of each species
   ...
   contains
      ...
      SUBROUTINE cool_step(icell, loopcnt)
         ...
         real(dp), save:: kUV, HrateLW, Dlw, Flw, sigUVthin, sigUV, ss_H2
         real(dp), save:: Ebpump, EUV, fpump, f_shd
         real(dp), save:: f_dg, F_pe, G_0, G_conv, eps_PE, HratePE
         ...
         ! for dust depletion
         real(dp)::d_mass,g_mass
         real(dp),dimension(1:nmetals)::depletion_factors

         ! DVODE parameters
         INTEGER, PARAMETER :: NEQ_O = n_oxygen_ions
         INTEGER, PARAMETER :: NEQ_N = n_nitrogen_ions
         INTEGER, PARAMETER :: NEQ_C = n_carbon_ions
         INTEGER, PARAMETER :: NEQ_MG = n_magnesium_ions
         INTEGER, PARAMETER :: NEQ_SI = n_silicon_ions
         INTEGER, PARAMETER :: NEQ_S = n_sulfur_ions
         INTEGER, PARAMETER :: NEQ_FE = n_iron_ions
         INTEGER, PARAMETER :: NEQ_NE = n_neon_ions
         INTEGER ITASK, ISTATE, JROOTS, ISTATS, FOUND, NOUT, IRVAL, MATFILE, IERROR
         DOUBLE PRECISION ATOL, RTOL, RSTATS, TIN, TOUT_CHEM, TSETTLE, CON1, R
         DOUBLE PRECISION YV_O, YV_N, YV_C, YV_MG, YV_SI, YV_S, YV_FE, YV_NE
         DIMENSION RSTATS(22), ISTATS(31)
         DIMENSION YV_O(NEQ_O), YV_N(NEQ_N), YV_C(NEQ_C), YV_MG(NEQ_MG) 
         DIMENSION YV_SI(NEQ_SI), YV_S(NEQ_S), YV_FE(NEQ_FE), YV_NE(NEQ_NE) 
         TYPE (VODE_OPTS) :: OPTIONS

         ATOL = metal_chem_atol
         RTOL = metal_chem_rtol
         ...
         if (isH2) then
            ...
            if (self_shielding) then
               ss_H2 = f_shd*comp_Sd(nN(ixHII), nN(ixHI), dx_SS(icell), Zsolar(icell))
            end if
         end if
         ...
         rho = nH(icell)/X*mH ! g/cm^3 total
         ! Need to add contribution from metals as it was subtracted earlier
         if (oxygen_ions)    rho = rho + (nOxygen(icell)    * mO_NIST_amu  * amu_to_g)
         if (nitrogen_ions)  rho = rho + (nNitrogen(icell)  * mN_NIST_amu  * amu_to_g)
         if (carbon_ions)    rho = rho + (nCarbon(icell)    * mc_NIST_amu  * amu_to_g)
         if (magnesium_ions) rho = rho + (nMagnesium(icell) * mMg_NIST_amu * amu_to_g)
         if (silicon_ions)   rho = rho + (nSilicon(icell)   * mSi_NIST_amu * amu_to_g)
         if (sulfur_ions)    rho = rho + (nSulfur(icell)    * mS_NIST_amu  * amu_to_g)
         if (iron_ions)      rho = rho + (nIron(icell)      * mFe_NIST_amu * amu_to_g)
         if (neon_ions)      rho = rho + (nNeon(icell)      * mNe_NIST_amu * amu_to_g)
#if NGROUPS>0
         ! Set dust opacities--------------------------------------------------
         if (rt .and. nGroups .gt. 0) then
            kAbs_loc = kappaAbs
            ...
            if (T2(icell) .le. T_sputter) then ! dust is assumed to be destroyed if too hot
               dustAbs(:) = kAbs_loc(:)*rho*Zsolar(icell)*rt_c_cgs  ! [cm2/g * g/cm^3 * cm/s]
               ...
               if (Zsolar(icell).gt.min_z_neq) then
                  if (oxygen_ions) then
                     do im=1,n_oxygen_ions-1
                        phAbs(igroup) = phAbs(igroup) + (signc_oxygen(igroup, im)*nOxygen(icell)*xO(im,icell)) ! cross section * c * number_density_ion
                     enddo
                  endif
                  if (nitrogen_ions) then
                     do im=1,n_nitrogen_ions-1
                        phAbs(igroup) = phAbs(igroup) + (signc_nitrogen(igroup, im)*nNitrogen(icell)*xN(im,icell)) ! cross section * c * number_density_ion
                     enddo
                  endif
                  if (carbon_ions) then
                     do im=1,n_carbon_ions-1
                        phAbs(igroup) = phAbs(igroup) + (signc_carbon(igroup, im)*nCarbon(icell)*xC(im,icell)) ! cross section * c * number_density_ion
                     enddo
                  endif
                  if (magnesium_ions) then
                     do im=1,n_magnesium_ions-1
                        phAbs(igroup) = phAbs(igroup) + (signc_magnesium(igroup, im)*nMagnesium(icell)*xMg(im,icell)) ! cross section * c * number_density_ion
                     enddo
                  endif
                  if (silicon_ions) then
                     do im=1,n_silicon_ions-1
                        phAbs(igroup) = phAbs(igroup) + (signc_silicon(igroup, im)*nSilicon(icell)*xSi(im,icell)) ! cross section * c * number_density_ion
                     enddo
                  endif
                  if (sulfur_ions) then
                     do im=1,n_sulfur_ions-1
                        phAbs(igroup) = phAbs(igroup) + (signc_sulfur(igroup, im)*nSulfur(icell)*xS(im,icell)) ! cross section * c * number_density_ion
                     enddo
                  endif
                  if (iron_ions) then
                     do im=1,n_iron_ions-1
                        phAbs(igroup) = phAbs(igroup) + (signc_iron(igroup, im)*nIron(icell)*xFe(im,icell)) ! cross section * c * number_density_ion
                     enddo
                  endif
                  if (neon_ions) then
                     do im=1,n_neon_ions-1
                        phAbs(igroup) = phAbs(igroup) + (signc_neon(igroup, im)*nNeon(icell)*xNe(im,icell)) ! cross section * c * number_density_ion
                     enddo
                  endif
               endif
            end do
            ...
            if (rt_isIR) then
               do igroup = iIR + 1, nGroups
                  ...
                  if (PEH .and. group_egy(igroup) > 5.4 .and. group_egy(igroup) < 13.6 .and. TK .le. T_sputter) then
                     f_dg = (dust2metal_solar)*Zsolar(icell)
                     ...
                     HratePE = 1.3d-24*eps_PE*G_0*f_dg*nH(icell) ! [erg/cm3/s] energy used to heat up the gas through PEH
         ...
         if (c_switch(icell) .and. cooling .and. .not. rt_T_rad) then
               ...
               do igroup = 1, nGroups                              !  Photoheating
                  if (igroup .eq. LWgroup) then
                     if(isH2Katz) then
                        ...
                        HrateLW = ((kUV*fpump*Ebpump) + (kUV*EUV))*nN(ixHI) ! [erg/s/cm3] = [#/s]*[erg/#]*[cm-3]
                        Hrate = Hrate + max(HrateLW, 0.0d0)
                     endif
                  else
                     Hrate = Hrate + dNp(igroup)*SUM(nN(:)*PHrate(igroup, :))

                     ! photoheating from metals
                     if (oxygen_ions)    Hrate = Hrate + ( dNp(igroup) * nOxygen(icell)   * SUM( xO(:,icell)  * PHrate_oxygen(igroup,:)   ) )
                     if (nitrogen_ions)  Hrate = Hrate + ( dNp(igroup) * nNitrogen(icell) * SUM( xN(:,icell)  * PHrate_nitrogen(igroup,:) ) )
                     if (carbon_ions)    Hrate = Hrate + ( dNp(igroup) * nCarbon(icell)   * SUM( xC(:,icell)  * PHrate_carbon(igroup,:)   ) )
                     if (magnesium_ions) Hrate = Hrate + ( dNp(igroup) * nMagnesium(icell)* SUM( xMg(:,icell) * PHrate_magnesium(igroup,:)) )
                     if (silicon_ions)   Hrate = Hrate + ( dNp(igroup) * nSilicon(icell)  * SUM( xSi(:,icell) * PHrate_silicon(igroup,:)  ) )
                     if (sulfur_ions)    Hrate = Hrate + ( dNp(igroup) * nSulfur(icell)   * SUM( xS(:,icell)  * PHrate_sulfur(igroup,:)   ) )
                     if (iron_ions)      Hrate = Hrate + ( dNp(igroup) * nIron(icell)     * SUM( xFe(:,icell) * PHrate_iron(igroup,:)     ) )
                     if (neon_ions)      Hrate = Hrate + ( dNp(igroup) * nNeon(icell)     * SUM( xNe(:,icell) * PHrate_neon(igroup,:)     ) )
                  end if
                  ...
                  if (PEH) then
                     if (group_egy(igroup) > 5.4 .and. group_egy(igroup) < 13.6 .and. TK .le. T_sputter) then
                        ...
                        G_0 = F_pe/G_conv
                        eps_PE = PE_efficiency(G_0, TK, ne)
                        HratePE = 1.3d-24*eps_PE*G_0*f_dg*nH(icell) ! [erg/cm3/s]
                        Hrate = Hrate + HratePE
                     end if
                  end if
            ...
            Hrate = Hrate + (nN(2)*UV_background_hydrogen_heating*ss_factor) !HI
            if (isHe) then 
               Hrate = Hrate + (nN(3)*UV_background_helium_heating(1)*ss_factor) !HeI
               Hrate = Hrate + (nN(4)*UV_background_helium_heating(2)*ss_factor) !HeII
            endif
            if (isH2Katz) Hrate = Hrate + (nN(1)*UV_background_H2_heating*ss_factor) !H2

            ! Add heating from UV background
            if (oxygen_ions) then
               do im=1,n_oxygen_ions-1
                  Hrate = Hrate + (nOxygen(icell)*xO(im,icell)*UV_background_oxygen_heating(im)*ss_factor)
               enddo
            endif
            if (nitrogen_ions) then
               do im=1,n_nitrogen_ions-1
                  Hrate = Hrate + (nNitrogen(icell)*xN(im,icell)*UV_background_nitrogen_heating(im)*ss_factor)
               enddo
            endif
            if (carbon_ions) then
               do im=1,n_carbon_ions-1
                  Hrate = Hrate + (nCarbon(icell)*xC(im,icell)*UV_background_carbon_heating(im)*ss_factor)
               enddo
            endif
            if (magnesium_ions) then
               do im=1,n_magnesium_ions-1
                  Hrate = Hrate + (nMagnesium(icell)*xMg(im,icell)*UV_background_magnesium_heating(im)*ss_factor)
               enddo
            endif
            if (silicon_ions) then
               do im=1,n_silicon_ions-1
                  Hrate = Hrate + (nSilicon(icell)*xSi(im,icell)*UV_background_silicon_heating(im)*ss_factor)
               enddo
            endif
            if (sulfur_ions) then
               do im=1,n_sulfur_ions-1
                  Hrate = Hrate + (nSulfur(icell)*xS(im,icell)*UV_background_sulfur_heating(im)*ss_factor)
               enddo
            endif
            if (iron_ions) then
               do im=1,n_iron_ions-1
                  Hrate = Hrate + (nIron(icell)*xFe(im,icell)*UV_background_iron_heating(im)*ss_factor)
               enddo
            endif
            if (neon_ions) then
               do im=1,n_neon_ions-1
                  Hrate = Hrate + (nNeon(icell)*xNe(im,icell)*UV_background_neon_heating(im)*ss_factor)
               enddo
            endif
            ...
            fire_tot = 0.d0; fire_prime = 0.d0             ! Hopkins cooling (metals,dust,cosmic rays, photo-electric
            fine_structure = 0.d0; fine_structure_prime = 0.d0 ! non-equilibrium fine-structure cooling
            ...
            if (low_t_noneq_cooling.and.Zsolar(icell).gt.min_z_neq) then
               ...
               if (T2(icell) .le. T_sputter .and. simple_dust_depletion) then ! no dust at T>10^5 K
                  d_mass = 0.d0
                  g_mass = rho * (dx_loc**3.d0) ! gas mass in grams
               ...
               if (TK.le.1.d4) then
                  ...
                  if (oxygen_ions) then
                     if ((nOxygen(icell)*xO(1,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (OI_fine_structure(TK,nOxygen(icell)*xO(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(2))
                        fine_structure_prime = fine_structure_prime + (OI_fine_structure(TK*1.001,nOxygen(icell)*xO(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(2))
                     endif
#if NOXYGENIONS>1
                     if ((nOxygen(icell)*xO(2,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (OII_fine_structure(TK,nOxygen(icell)*xO(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(2))
                        fine_structure_prime = fine_structure_prime + (OII_fine_structure(TK*1.001,nOxygen(icell)*xO(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(2))
                     endif
#endif
#if NOXYGENIONS>2
                     if ((nOxygen(icell)*xO(3,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (OIII_fine_structure(TK,nOxygen(icell)*xO(3,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(2))
                        fine_structure_prime = fine_structure_prime + (OIII_fine_structure(TK*1.001,nOxygen(icell)*xO(3,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(2))
                     endif
#endif
                  endif
                  ...
                  if (nitrogen_ions) then
#if NNITROGENIONS>1
                     if ((nNitrogen(icell)*xN(2,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (NII_fine_structure(TK,nNitrogen(icell)*xN(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(3))
                        fine_structure_prime = fine_structure_prime + (NII_fine_structure(TK*1.001,nNitrogen(icell)*xN(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(3))
                     endif
#endif
                  endif
                  ...
                  if (carbon_ions) then
                     if ((nCarbon(icell)*xC(1,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (CI_fine_structure(TK,nCarbon(icell)*xC(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(8))
                        fine_structure_prime = fine_structure_prime + (CI_fine_structure(TK*1.001,nCarbon(icell)*xC(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(8))
                     endif
#if NCARBONIONS>1
                     if ((nCarbon(icell)*xC(2,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (CII_fine_structure(TK,nCarbon(icell)*xC(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(8))
                        fine_structure_prime = fine_structure_prime + (CII_fine_structure(TK*1.001,nCarbon(icell)*xC(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(8))
                     endif
#endif
                  endif
                  ...
                  if (silicon_ions) then
                     if ((nSilicon(icell)*xSi(1,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (SiI_fine_structure(TK,nSilicon(icell)*xSi(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(6))
                        fine_structure_prime = fine_structure_prime + (SiI_fine_structure(TK*1.001,nSilicon(icell)*xSi(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(6))
                     endif
#if NSILICONIONS>1
                     if ((nSilicon(icell)*xSi(2,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (SiII_fine_structure(TK,nSilicon(icell)*xSi(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(6))
                        fine_structure_prime = fine_structure_prime + (SiII_fine_structure(TK*1.001,nSilicon(icell)*xSi(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(6))
                     endif
#endif
                  endif
                  ...
                  if (iron_ions) then
                     if ((nIron(icell)*xFe(1,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (FeI_fine_structure(TK,nIron(icell)*xFe(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(1))
                        fine_structure_prime = fine_structure_prime + (FeI_fine_structure(TK*1.001,nIron(icell)*xFe(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(1))
                     endif
#if NIRONIONS>1
                     if ((nIron(icell)*xFe(2,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (FeII_fine_structure(TK,nIron(icell)*xFe(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(1))
                        fine_structure_prime = fine_structure_prime + (FeII_fine_structure(TK*1.001,nIron(icell)*xFe(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(1))
                     endif
#endif
                  endif
                  ...
                  if (sulfur_ions) then
                     if ((nSulfur(icell)*xS(1,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                        fine_structure = fine_structure + (SI_fine_structure(TK,nSulfur(icell)*xS(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(9))
                        fine_structure_prime = fine_structure_prime + (SI_fine_structure(TK*1.001,nSulfur(icell)*xS(1,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(9))
                     endif
                  endif

                  if (neon_ions) then
#if NNEONIONS>1
                    if ((nNeon(icell)*xNe(2,icell)).gt.min_cool_ion) then ! No reason to call in very low densities
                       fine_structure = fine_structure + (NeII_fine_structure(TK,nNeon(icell)*xNe(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(5))
                       fine_structure_prime = fine_structure_prime + (NeII_fine_structure(TK*1.001,nNeon(icell)*xNe(2,icell),nN(ixHII),nI(ixHII),ne,nH2_cool,(1.d0/a_exp)-1.d0)*depletion_factors(5))
                    endif
#endif
                  endif
                  ...
                  if (metal_cooling_catastrophe_fix) then 
                     fine_structure = fine_structure * exp(metal_cooling_catastrophe_rho/(-rho*X/mH))
                     fine_structure_prime = fine_structure_prime * exp(metal_cooling_catastrophe_rho/(-rho*X/mH))
                  endif
                  fine_structure_prime = (fine_structure - fine_structure_prime) / (TK - (TK*1.001))
               endif
            ...
            X_nHkb = 1.0 / (1.5 * (rho/mH) * kB)     ! Harley update to use full rho
            rate = X_nHkb*(Hrate - Crate - Zsolar(icell)*metal_tot - fire_tot - fine_structure)
            dRate = -X_nHkb*(dCdT2 + Zsolar(icell)*metal_prime + fire_prime + fine_structure_prime)     ! dRate/dT2
            ! 1st order dt constr
         ...
         if (isH2) then
            ! H2 creation rates
            xe = ne/nH(icell)
            alpha(ixHI) = comp_Alpha_H2(TK, Zsolar(icell), xe, xHII) ! [cm3 s-1]
...
!           write(777,778) xH2, nH(icell), cr, de, photoRate, beta(ixHI),alpha(ixHI),xHI,xe,ddt(icell)
         ...
         if (include_charge_transfer) then
            if (isHe) then
               cr = cr + HEIIon(Tk)*xHII*nHE*xHeII ! He + H+ => He+ + H
            endif
            if (Zsolar(icell).gt.min_z_neq) then
               if (oxygen_ions) then
                  do im=1,n_oxygen_ions-1 ! No ionization of final ion
                     cr = cr + HCTIon(im,8,Tk)*xHII*dxO(im)*nOxygen(icell) ! Example:  O + H+ => O+ + H
                  enddo
               endif
               if (nitrogen_ions) then
                  do im=1,n_nitrogen_ions-1 ! No ionization of final ion
                     cr = cr + HCTIon(im,7,Tk)*xHII*dxN(im)*nNitrogen(icell) ! Example:  N + H+ => N+ + H
                  enddo
               endif
               if (carbon_ions) then
                  do im=1,n_carbon_ions-1 ! No ionization of final ion
                     cr = cr + HCTIon(im,6,Tk)*xHII*dxC(im)*nCarbon(icell) ! Example:  C + H+ => C+ + H
                  enddo
               endif
               if (magnesium_ions) then
                  do im=1,n_magnesium_ions-1 ! No ionization of final ion
                     cr = cr + HCTIon(im,12,Tk)*xHII*dxMg(im)*nMagnesium(icell) ! Example:  Mg + H+ => Mg+ + H
                  enddo
               endif
               if (silicon_ions) then
                  do im=1,n_silicon_ions-1 ! No ionization of final ion
                     cr = cr + HCTIon(im,14,Tk)*xHII*dxSi(im)*nSilicon(icell) ! Example:  Si + H+ => Si+ + H
                  enddo
               endif
               if (sulfur_ions) then
                  do im=1,n_sulfur_ions-1 ! No ionization of final ion
                     cr = cr + HCTIon(im,16,Tk)*xHII*dxS(im)*nSulfur(icell) ! Example:  S + H+ => S+ + H
                  enddo
               endif
               if (iron_ions) then
                  do im=1,n_iron_ions-1 ! No ionization of final ion
                     cr = cr + HCTIon(im,26,Tk)*xHII*dxFe(im)*nIron(icell) ! Example:  Fe + H+ => Fe+ + H
                  enddo
               endif
               if (neon_ions) then
                  do im=1,n_neon_ions-1 ! No ionization of final ion
                     cr = cr + HCTIon(im,10,Tk)*xHII*dxNe(im)*nNeon(icell) ! Example:  Ne + H+ => Ne+ + H
                  enddo
               endif
            endif
         endif
            ...
            if (Zsolar(icell).gt.min_z_neq) then
               if (oxygen_ions) then
                  do im=2,n_oxygen_ions ! No recombination of groundstate
                     de = de + HCTRecom(im,8,Tk)*dxO(im)*nOxygen(icell) ! Example:  O+ + H => O + H+
                  enddo
               endif
               if (nitrogen_ions) then
                  do im=2,n_nitrogen_ions ! No recombination of groundstate
                     de = de + HCTRecom(im,7,Tk)*dxN(im)*nNitrogen(icell) ! Example:  N+ + H => N + H+
                  enddo
               endif
               if (carbon_ions) then
                  do im=2,n_carbon_ions ! No recombination of groundstate
                     de = de + HCTRecom(im,6,Tk)*dxC(im)*nCarbon(icell) ! Example:  C+ + H => C + H+
                  enddo
               endif
               if (magnesium_ions) then
                  do im=2,n_magnesium_ions ! No recombination of groundstate
                     de = de + HCTRecom(im,12,Tk)*dxMg(im)*nMagnesium(icell) ! Example:  Mg+ + H => Mg + H+
                  enddo
               endif
               if (silicon_ions) then
                  do im=2,n_silicon_ions ! No recombination of groundstate
                     de = de + HCTRecom(im,14,Tk)*dxSi(im)*nSilicon(icell) ! Example:  Si+ + H => Si + H+
                  enddo
               endif
               if (sulfur_ions) then
                  do im=2,n_sulfur_ions ! No recombination of groundstate
                     de = de + HCTRecom(im,16,Tk)*dxS(im)*nSulfur(icell) ! Example:  S+ + H => S + H+
                  enddo
               endif
               if (iron_ions) then
                  do im=2,n_iron_ions ! No recombination of groundstate
                     de = de + HCTRecom(im,26,Tk)*dxFe(im)*nIron(icell) ! Example:  Fe+ + H => Fe + H+
                  enddo
               endif
               if (neon_ions) then
                  do im=2,n_neon_ions ! No recombination of groundstate
                     de = de + HCTRecom(im,10,Tk)*dxNe(im)*nNeon(icell) ! Example:  Ne+ + H => Ne + H+
                  enddo
               endif
            endif
         endif
         ...
         if (include_charge_transfer) then
            if (isHe) then
               cr = cr + HEIIRecomb(Tk)*nHE*xHeII     ! H + He+  -> He + H+
               cr = cr + HEIIIRecomb(Tk)*nHE*xHeIII   ! H + He++ -> He+ + H+
               de = de + HEIIon(Tk)*nHE*xHeII ! He + H+ => He+ + H
            endif
            if (Zsolar(icell).gt.min_z_neq) then
               if (oxygen_ions) then
                  do im=1,n_oxygen_ions 
                     if (im.gt.1) cr = cr + HCTRecom(im,8,Tk)*dxO(im)*nOxygen(icell) ! Example:  O+ + H => O + H+
                     if (im.lt.n_oxygen_ions) de = de + HCTIon(im,8,Tk)*dxO(im)*nOxygen(icell) ! Example:  O + H+ => O+ + H
                  enddo
               endif
               if (nitrogen_ions) then
                  do im=1,n_nitrogen_ions 
                     if (im.gt.1) cr = cr + HCTRecom(im,7,Tk)*dxN(im)*nNitrogen(icell) ! Example:  N+ + H => N + H+
                     if (im.lt.n_nitrogen_ions) de = de + HCTIon(im,7,Tk)*dxN(im)*nNitrogen(icell) ! Example:  N + H+ => N+ + H
                  enddo
               endif
               if (carbon_ions) then
                  do im=1,n_carbon_ions 
                     if (im.gt.1) cr = cr + HCTRecom(im,6,Tk)*dxC(im)*nCarbon(icell) ! Example:  C+ + H => C + H+
                     if (im.lt.n_carbon_ions) de = de + HCTIon(im,6,Tk)*dxC(im)*nCarbon(icell) ! Example:  C + H+ => C+ + H
                  enddo
               endif
               if (magnesium_ions) then
                  do im=1,n_magnesium_ions 
                     if (im.gt.1) cr = cr + HCTRecom(im,12,Tk)*dxMg(im)*nMagnesium(icell) ! Example:  Mg+ + H => Mg + H+
                     if (im.lt.n_magnesium_ions) de = de + HCTIon(im,12,Tk)*dxMg(im)*nMagnesium(icell) ! Example:  Mg + H+ => Mg+ + H
                  enddo
               endif
               if (silicon_ions) then
                  do im=1,n_silicon_ions 
                     if (im.gt.1) cr = cr + HCTRecom(im,14,Tk)*dxSi(im)*nSilicon(icell) ! Example:  Si+ + H => Si + H+
                     if (im.lt.n_silicon_ions) de = de + HCTIon(im,14,Tk)*dxSi(im)*nSilicon(icell) ! Example:  Si + H+ => Si+ + H
                  enddo
               endif
               if (sulfur_ions) then
                  do im=1,n_sulfur_ions 
                     if (im.gt.1) cr = cr + HCTRecom(im,16,Tk)*dxS(im)*nSulfur(icell) ! Example:  S+ + H => S + H+
                     if (im.lt.n_sulfur_ions) de = de + HCTIon(im,16,Tk)*dxS(im)*nSulfur(icell) ! Example:  S + H+ => S+ + H
                  enddo
               endif
               if (iron_ions) then
                  do im=1,n_iron_ions 
                     if (im.gt.1) cr = cr + HCTRecom(im,26,Tk)*dxFe(im)*nIron(icell) ! Example:  Fe+ + H => Fe + H+
                     if (im.lt.n_iron_ions) de = de + HCTIon(im,26,Tk)*dxFe(im)*nIron(icell) ! Example:  Fe + H+ => Fe+ + H
                  enddo
               endif
               if (neon_ions) then
                  do im=1,n_neon_ions 
                     if (im.gt.1) cr = cr + HCTRecom(im,10,Tk)*dxNe(im)*nNeon(icell) ! Example:  Ne+ + H => Ne + H+
                     if (im.lt.n_neon_ions) de = de + HCTIon(im,10,Tk)*dxNe(im)*nNeon(icell) ! Example:  Ne + H+ => Ne+ + H
                  enddo
               endif
            endif
         endif
         ...
         if (oxygen_ions) then
            if (simple_ode) then ! Use the simple method based on anninos
               do im=1,n_oxygen_ions
                  ! Creation
                  cr = 0.d0
               ...
               enddo
            else ! properly solve the ODE with the BDF solver
               ! Prepare the quantities for the solver
               ne_chem = ne
               TK_chem = TK 
               dNp_chem(:) = dNp(:)
               signc_oxygen_chem(:,:) = signc_oxygen(:,:)
               chem_spec = 1

               JAC_O = 0.d0
               do im=1,n_oxygen_ions
                  ! Creation
                  if (im.lt.n_oxygen_ions) JAC_O(im,im+1) = JAC_O(im,im+1) + comp_Alpha_oxygen(TK_chem, 8-im)*ne_chem ! Recombinations of the more excited ionization state
                  if (im.gt.1) then 
                     if (include_collisional_ionization) JAC_O(im,im-1) = JAC_O(im,im-1) + comp_beta_oxygen(TK_chem, im-1)*ne_chem ! Collisional ionization of the less excited state
                     if (rt.and.include_photoionisation) then 
                        JAC_O(im,im-1) = JAC_O(im,im-1) + SUM(signc_oxygen_chem(:, im-1)*dNp_chem) ! photoionization of the less excited state
                        JAC_O(im,im-1) = JAC_O(im,im-1) + UV_background_oxygen(im-1)*ss_factor ! photoionization of the less excited state (uvbg)
                     endif
                  endif

                  ! Destruction = collisional ionization + photoionization + recombination
                  if (im.lt.n_oxygen_ions.and.include_collisional_ionization) JAC_O(im,im) = JAC_O(im,im) - comp_beta_oxygen(TK_chem, im)*ne_chem ! Collisional ionization 
                  if (im.gt.1) JAC_O(im,im) = JAC_O(im,im) - comp_Alpha_oxygen(TK_chem, 9-im)*ne_chem ! Recombination (the 9 is correct)
                  
                  if (rt.and.include_photoionisation) then 
                     JAC_O(im,im) = JAC_O(im,im) - SUM(signc_oxygen_chem(:, im)*dNp_chem)
                     if (im.lt.n_oxygen_ions) JAC_O(im,im) = JAC_O(im,im) - UV_background_oxygen(im)*ss_factor
                  endif
                  !if (haardt_madau.and.include_photoionisation) JAC_O(im,im) = JAC_O(im,im) - UVrates(ixHeII, 1)*ss_factor
               enddo
               ...
               OPTIONS = SET_NORMAL_OPTS(RELERR=RTOL,ABSERR=ATOL,DENSE_J=.FALSE.,USER_SUPPLIED_JACOBIAN=.TRUE.)
               CALL DVODE_F90(FEX,NEQ_O,YV_O,TIN,TOUT_CHEM,ITASK,ISTATE,OPTIONS,J_FCN=JEX)
               CALL GET_STATS(RSTATS,ISTATS)
               if (ISTATE<0) then
                  write(*,*) "somthing broke in oxygen update",ISTATE
               endif
         ...
         if (nitrogen_ions) then
            if (simple_ode) then
               ...
               TK_chem = TK 
               dNp_chem(:) = dNp(:)
               signc_nitrogen_chem(:,:) = signc_nitrogen(:,:)
               chem_spec = 2

               JAC_N = 0.d0
               do im=1,n_nitrogen_ions
                  ! Creation
                  if (im.lt.n_nitrogen_ions) JAC_N(im,im+1) = JAC_N(im,im+1) + comp_Alpha_nitrogen(TK_chem, 7-im)*ne_chem ! Recombinations of the more excited ionization state
                  if (im.gt.1) then 
                     if (include_collisional_ionization) JAC_N(im,im-1) = JAC_N(im,im-1) + comp_beta_nitrogen(TK_chem, im-1)*ne_chem ! Collisional ionization of the less excited state
                     if (rt.and.include_photoionisation) then 
                        JAC_N(im,im-1) = JAC_N(im,im-1) + SUM(signc_nitrogen_chem(:, im-1)*dNp_chem) ! photoionization of the less excited state
                        JAC_N(im,im-1) = JAC_N(im,im-1) + UV_background_nitrogen(im-1)*ss_factor ! photoionization of the less excited state (uvbg)
                     endif
                  endif

                  ! Destruction = collisional ionization + photoionization + recombination
                  if (im.lt.n_nitrogen_ions.and.include_collisional_ionization) JAC_N(im,im) = JAC_N(im,im) - comp_beta_nitrogen(TK_chem, im)*ne_chem ! Collisional ionization 
                  if (im.gt.1) JAC_N(im,im) = JAC_N(im,im) - comp_Alpha_nitrogen(TK_chem, 8-im)*ne_chem ! Recombination (the 9 is correct)
                  
                  if (rt.and.include_photoionisation) then 
                     JAC_N(im,im) = JAC_N(im,im) - SUM(signc_nitrogen_chem(:, im)*dNp_chem)
                     if (im.lt.n_nitrogen_ions) JAC_N(im,im) = JAC_N(im,im) - UV_background_nitrogen(im)*ss_factor 
                  endif
                  !if (haardt_madau.and.include_photoionisation) JAC_N(im,im) = JAC_N(im,im) - UVrates(ixHeII, 1)*ss_factor
               enddo
               ...
               OPTIONS = SET_NORMAL_OPTS(RELERR=RTOL,ABSERR=ATOL,DENSE_J=.FALSE.,USER_SUPPLIED_JACOBIAN=.TRUE.)
               CALL DVODE_F90(FEX,NEQ_N,YV_N,TIN,TOUT_CHEM,ITASK,ISTATE,OPTIONS,J_FCN=JEX)
               CALL GET_STATS(RSTATS,ISTATS)
               if (ISTATE<0) then
                  write(*,*) "somthing broke in nitrogen update",ISTATE
               endif
         ...
         if (carbon_ions) then
            if (simple_ode) then 
               ...
               TK_chem = TK 
               dNp_chem(:) = dNp(:)
               signc_carbon_chem(:,:) = signc_carbon(:,:)
               chem_spec = 3

               JAC_C = 0.d0
               do im=1,n_carbon_ions
                  ! Creation
                  if (im.lt.n_carbon_ions) JAC_C(im,im+1) = JAC_C(im,im+1) + comp_Alpha_carbon(TK_chem, 6-im)*ne_chem ! Recombinations of the more excited ionization state
                  if (im.gt.1) then 
                     if (include_collisional_ionization) JAC_C(im,im-1) = JAC_C(im,im-1) + comp_beta_carbon(TK_chem, im-1)*ne_chem ! Collisional ionization of the less excited state
                     if (rt.and.include_photoionisation) then 
                        JAC_C(im,im-1) = JAC_C(im,im-1) + SUM(signc_carbon_chem(:, im-1)*dNp_chem) ! photoionization of the less excited state
                        JAC_C(im,im-1) = JAC_C(im,im-1) + UV_background_carbon(im-1)*ss_factor ! photoionization of the less excited state (uvbg)
                     endif
                  endif

                  ! Destruction = collisional ionization + photoionization + recombination
                  if (im.lt.n_carbon_ions.and.include_collisional_ionization) JAC_C(im,im) = JAC_C(im,im) - comp_beta_carbon(TK_chem, im)*ne_chem ! Collisional ionization 
                  if (im.gt.1) JAC_C(im,im) = JAC_C(im,im) - comp_Alpha_carbon(TK_chem, 7-im)*ne_chem ! Recombination (the 7 is correct)
                  
                  if (rt.and.include_photoionisation) then 
                     JAC_C(im,im) = JAC_C(im,im) - SUM(signc_carbon_chem(:, im)*dNp_chem)
                     if (im.lt.n_carbon_ions) JAC_C(im,im) = JAC_C(im,im) - UV_background_carbon(im)*ss_factor
                  endif
                  !if (haardt_madau.and.include_photoionisation) JAC_C(im,im) = JAC_C(im,im) - UVrates(ixHeII, 1)*ss_factor
               enddo
               ...
               OPTIONS = SET_NORMAL_OPTS(RELERR=RTOL,ABSERR=ATOL,DENSE_J=.FALSE.,USER_SUPPLIED_JACOBIAN=.TRUE.)
               CALL DVODE_F90(FEX,NEQ_C,YV_C,TIN,TOUT_CHEM,ITASK,ISTATE,OPTIONS,J_FCN=JEX)
               CALL GET_STATS(RSTATS,ISTATS)
               if (ISTATE<0) then
                  write(*,*) "somthing broke in carbon update",ISTATE
               endif
         ...
         if (magnesium_ions) then
            if (simple_ode) then 
               ...
               TK_chem = TK 
               dNp_chem(:) = dNp(:)
               signc_magnesium_chem(:,:) = signc_magnesium(:,:)
               chem_spec = 4

               JAC_MG = 0.d0
               do im=1,n_magnesium_ions
                  ! Creation
                  if (im.lt.n_magnesium_ions) JAC_MG(im,im+1) = JAC_MG(im,im+1) + comp_Alpha_magnesium(TK_chem, 12-im)*ne_chem ! Recombinations of the more excited ionization state
                  if (im.gt.1) then 
                     if (include_collisional_ionization) JAC_MG(im,im-1) = JAC_MG(im,im-1) + comp_beta_magnesium(TK_chem, im-1)*ne_chem ! Collisional ionization of the less excited state
                     if (rt.and.include_photoionisation) then 
                        JAC_MG(im,im-1) = JAC_MG(im,im-1) + SUM(signc_magnesium_chem(:, im-1)*dNp_chem) ! photoionization of the less excited state
                        JAC_MG(im,im-1) = JAC_MG(im,im-1) + UV_background_magnesium(im-1)*ss_factor ! photoionization of the less excited state (uvbg)
                     endif
                  endif

                  ! Destruction = collisional ionization + photoionization + recombination
                  if (im.lt.n_magnesium_ions.and.include_collisional_ionization) JAC_MG(im,im) = JAC_MG(im,im) - comp_beta_magnesium(TK_chem, im)*ne_chem ! Collisional ionization 
                  if (im.gt.1) JAC_MG(im,im) = JAC_MG(im,im) - comp_Alpha_magnesium(TK_chem, 13-im)*ne_chem ! Recombination (the 13 is correct)
                  
                  if (rt.and.include_photoionisation) then 
                     JAC_MG(im,im) = JAC_MG(im,im) - SUM(signc_magnesium_chem(:, im)*dNp_chem)
                     if (im.lt.n_magnesium_ions) JAC_MG(im,im) = JAC_MG(im,im) - UV_background_magnesium(im)*ss_factor
                  endif
                  !if (haardt_madau.and.include_photoionisation) JAC_MG(im,im) = JAC_MG(im,im) - UVrates(ixHeII, 1)*ss_factor
               enddo
               ...
               OPTIONS = SET_NORMAL_OPTS(RELERR=RTOL,ABSERR=ATOL,DENSE_J=.FALSE.,USER_SUPPLIED_JACOBIAN=.TRUE.)
               CALL DVODE_F90(FEX,NEQ_MG,YV_MG,TIN,TOUT_CHEM,ITASK,ISTATE,OPTIONS,J_FCN=JEX)
               CALL GET_STATS(RSTATS,ISTATS)
               if (ISTATE<0) then
                  write(*,*) "somthing broke in magnesium update",ISTATE
               endif
         ...
         if (silicon_ions) then
            if (simple_ode) then 
               ...
               TK_chem = TK 
               dNp_chem(:) = dNp(:)
               signc_silicon_chem(:,:) = signc_silicon(:,:)
               chem_spec = 5

               JAC_SI = 0.d0
               do im=1,n_silicon_ions
                  ! Creation
                  if (im.lt.n_silicon_ions) JAC_SI(im,im+1) = JAC_SI(im,im+1) + comp_Alpha_silicon(TK_chem, 14-im)*ne_chem ! Recombinations of the more excited ionization state
                  if (im.gt.1) then 
                     if (include_collisional_ionization) JAC_SI(im,im-1) = JAC_SI(im,im-1) + comp_beta_silicon(TK_chem, im-1)*ne_chem ! Collisional ionization of the less excited state
                     if (rt.and.include_photoionisation) then 
                        JAC_SI(im,im-1) = JAC_SI(im,im-1) + SUM(signc_silicon_chem(:, im-1)*dNp_chem) ! photoionization of the less excited state
                        JAC_SI(im,im-1) = JAC_SI(im,im-1) + UV_background_silicon(im-1)*ss_factor ! photoionization of the less excited state (uvbg)
                     endif
                  endif

                  ! Destruction = collisional ionization + photoionization + recombination
                  if (im.lt.n_silicon_ions.and.include_collisional_ionization) JAC_SI(im,im) = JAC_SI(im,im) - comp_beta_silicon(TK_chem, im)*ne_chem ! Collisional ionization 
                  if (im.gt.1) JAC_SI(im,im) = JAC_SI(im,im) - comp_Alpha_silicon(TK_chem, 15-im)*ne_chem ! Recombination (the 15 is correct)
                  
                  if (rt.and.include_photoionisation) then 
                     JAC_SI(im,im) = JAC_SI(im,im) - SUM(signc_silicon_chem(:, im)*dNp_chem)
                     if (im.lt.n_silicon_ions) JAC_SI(im,im) = JAC_SI(im,im) - UV_background_silicon(im)*ss_factor
                  endif
                  !if (haardt_madau.and.include_photoionisation) JAC_SI(im,im) = JAC_SI(im,im) - UVrates(ixHeII, 1)*ss_factor
               enddo
               ...
               OPTIONS = SET_NORMAL_OPTS(RELERR=RTOL,ABSERR=ATOL,DENSE_J=.FALSE.,USER_SUPPLIED_JACOBIAN=.TRUE.)
               CALL DVODE_F90(FEX,NEQ_SI,YV_SI,TIN,TOUT_CHEM,ITASK,ISTATE,OPTIONS,J_FCN=JEX)
               CALL GET_STATS(RSTATS,ISTATS)
               if (ISTATE<0) then
                  write(*,*) "somthing broke in silicon update",ISTATE
               endif
         ...
         if (sulfur_ions) then
            if (simple_ode) then 
               ...
               TK_chem = TK 
               dNp_chem(:) = dNp(:)
               signc_sulfur_chem(:,:) = signc_sulfur(:,:)
               chem_spec = 6

               JAC_S = 0.d0
               do im=1,n_sulfur_ions
                  ! Creation
                  if (im.lt.n_sulfur_ions) JAC_S(im,im+1) = JAC_S(im,im+1) + comp_Alpha_sulfur(TK_chem, 16-im)*ne_chem ! Recombinations of the more excited ionization state
                  if (im.gt.1) then 
                     if (include_collisional_ionization) JAC_S(im,im-1) = JAC_S(im,im-1) + comp_beta_sulfur(TK_chem, im-1)*ne_chem ! Collisional ionization of the less excited state
                     if (rt.and.include_photoionisation) then 
                        JAC_S(im,im-1) = JAC_S(im,im-1) + SUM(signc_sulfur_chem(:, im-1)*dNp_chem) ! photoionization of the less excited state
                        JAC_S(im,im-1) = JAC_S(im,im-1) + UV_background_sulfur(im-1)*ss_factor ! photoionization of the less excited state (uvbg)
                     endif
                  endif

                  ! Destruction = collisional ionization + photoionization + recombination
                  if (im.lt.n_sulfur_ions.and.include_collisional_ionization) JAC_S(im,im) = JAC_S(im,im) - comp_beta_sulfur(TK_chem, im)*ne_chem ! Collisional ionization 
                  if (im.gt.1) JAC_S(im,im) = JAC_S(im,im) - comp_Alpha_sulfur(TK_chem, 17-im)*ne_chem ! Recombination (the 15 is correct)
                  
                  if (rt.and.include_photoionisation) then 
                     JAC_S(im,im) = JAC_S(im,im) - SUM(signc_sulfur_chem(:, im)*dNp_chem)
                     if (im.lt.n_sulfur_ions) JAC_S(im,im) = JAC_S(im,im) - UV_background_sulfur(im)*ss_factor
                  endif
                  !if (haardt_madau.and.include_photoionisation) JAC_S(im,im) = JAC_S(im,im) - UVrates(ixHeII, 1)*ss_factor
               enddo
               ...
               OPTIONS = SET_NORMAL_OPTS(RELERR=RTOL,ABSERR=ATOL,DENSE_J=.FALSE.,USER_SUPPLIED_JACOBIAN=.TRUE.)
               CALL DVODE_F90(FEX,NEQ_S,YV_S,TIN,TOUT_CHEM,ITASK,ISTATE,OPTIONS,J_FCN=JEX)
               CALL GET_STATS(RSTATS,ISTATS)
               if (ISTATE<0) then
                  write(*,*) "somthing broke in sulfur update",ISTATE
               endif
         ...
         if (iron_ions) then
            if (simple_ode) then 
               ...
               TK_chem = TK 
               dNp_chem(:) = dNp(:)
               signc_iron_chem(:,:) = signc_iron(:,:)
               chem_spec = 7

               JAC_FE = 0.d0
               do im=1,n_iron_ions
                  ! Creation
                  if (im.lt.n_iron_ions) JAC_FE(im,im+1) = JAC_FE(im,im+1) + comp_Alpha_iron(TK_chem, 26-im)*ne_chem ! Recombinations of the more excited ionization state
                  if (im.gt.1) then 
                     if (include_collisional_ionization) JAC_FE(im,im-1) = JAC_FE(im,im-1) + comp_beta_iron(TK_chem, im-1)*ne_chem ! Collisional ionization of the less excited state
                     if (rt.and.include_photoionisation) then 
                        JAC_FE(im,im-1) = JAC_FE(im,im-1) + SUM(signc_iron_chem(:, im-1)*dNp_chem) ! photoionization of the less excited state
                        JAC_FE(im,im-1) = JAC_FE(im,im-1) + UV_background_iron(im-1)*ss_factor ! photoionization of the less excited state (uvbg)
                     endif
                  endif

                  ! Destruction = collisional ionization + photoionization + recombination
                  if (im.lt.n_iron_ions.and.include_collisional_ionization) JAC_FE(im,im) = JAC_FE(im,im) - comp_beta_iron(TK_chem, im)*ne_chem ! Collisional ionization 
                  if (im.gt.1) JAC_FE(im,im) = JAC_FE(im,im) - comp_Alpha_iron(TK_chem, 27-im)*ne_chem ! Recombination (the 15 is correct)
                  
                  if (rt.and.include_photoionisation) then 
                     JAC_FE(im,im) = JAC_FE(im,im) - SUM(signc_iron_chem(:, im)*dNp_chem)
                     if (im.lt.n_iron_ions) JAC_FE(im,im) = JAC_FE(im,im) - UV_background_iron(im)*ss_factor
                  endif
                  !if (haardt_madau.and.include_photoionisation) JAC_FE(im,im) = JAC_FE(im,im) - UVrates(ixHeII, 1)*ss_factor
               enddo
               ...
               OPTIONS = SET_NORMAL_OPTS(RELERR=RTOL,ABSERR=ATOL,DENSE_J=.FALSE.,USER_SUPPLIED_JACOBIAN=.TRUE.)
               CALL DVODE_F90(FEX,NEQ_FE,YV_FE,TIN,TOUT_CHEM,ITASK,ISTATE,OPTIONS,J_FCN=JEX)
               CALL GET_STATS(RSTATS,ISTATS)
               if (ISTATE<0) then
                  write(*,*) "somthing broke in iron update",ISTATE
               endif
         ...
         if (neon_ions) then
            if (simple_ode) then 
               ...
               TK_chem = TK 
               dNp_chem(:) = dNp(:)
               signc_neon_chem(:,:) = signc_neon(:,:)
               chem_spec = 8

               JAC_NE = 0.d0
               do im=1,n_neon_ions
                  ! Creation
                  if (im.lt.n_neon_ions) JAC_NE(im,im+1) = JAC_NE(im,im+1) + comp_Alpha_neon(TK_chem, 10-im)*ne_chem ! Recombinations of the more excited ionization state
                  if (im.gt.1) then 
                     if (include_collisional_ionization) JAC_NE(im,im-1) = JAC_NE(im,im-1) + comp_beta_neon(TK_chem, im-1)*ne_chem ! Collisional ionization of the less excited state
                     if (rt.and.include_photoionisation) then 
                        JAC_NE(im,im-1) = JAC_NE(im,im-1) + SUM(signc_neon_chem(:, im-1)*dNp_chem) ! photoionization of the less excited state
                        JAC_NE(im,im-1) = JAC_NE(im,im-1) + UV_background_neon(im-1)*ss_factor ! photoionization of the less excited state (uvbg)
                     endif
                  endif

                  ! Destruction = collisional ionization + photoionization + recombination
                  if (im.lt.n_neon_ions.and.include_collisional_ionization) JAC_NE(im,im) = JAC_NE(im,im) - comp_beta_neon(TK_chem, im)*ne_chem ! Collisional ionization 
                  if (im.gt.1) JAC_NE(im,im) = JAC_NE(im,im) - comp_Alpha_neon(TK_chem, 11-im)*ne_chem ! Recombination (the 15 is correct)
                  
                  if (rt.and.include_photoionisation) then 
                     JAC_NE(im,im) = JAC_NE(im,im) - SUM(signc_neon_chem(:, im)*dNp_chem)
                     if (im.lt.n_neon_ions) JAC_NE(im,im) = JAC_NE(im,im) - UV_background_neon(im)*ss_factor
                  endif
                  !if (haardt_madau.and.include_photoionisation) JAC_NE(im,im) = JAC_NE(im,im) - UVrates(ixHeII, 1)*ss_factor
               enddo
               ...
               OPTIONS = SET_NORMAL_OPTS(RELERR=RTOL,ABSERR=ATOL,DENSE_J=.FALSE.,USER_SUPPLIED_JACOBIAN=.TRUE.)
               CALL DVODE_F90(FEX,NEQ_NE,YV_NE,TIN,TOUT_CHEM,ITASK,ISTATE,OPTIONS,J_FCN=JEX)
               CALL GET_STATS(RSTATS,ISTATS)
               if (ISTATE<0) then
                  write(*,*) "somthing broke in neon update",ISTATE
               endif
...
!------------------------------------------------------------------------
      ...
      if (isH2) then
         ...
         a_H2 = comp_Alpha_H2(TK, Zsol, xe)     ! H2 creation rate [cm3 s-1]
      end if
...
!-------------------------------------------------------------------------
      ...
      real(dp), dimension(1:ndim, 1:nGroups, 1:nvector):: Fp, dFpdt
      real(dp), dimension(1:ndim, 1:nvector):: p_gas
      real(dp), dimension(1:nvector)::nH = 0., Zsolar = 0., dx_SS = 0.
...
!-------------------------------------------------------------------------
      ...
      do while (aexp < aend)
         ...
         call rt_solve_cooling(T2, xion, Np, Fp, p_gas, dNpdt, dFpdt, nH, c_switch &
                               ...
                               , nCarbon, nMagnesium, nSilicon, nSulfur, nIron, nNeon, 0.d0 ) ! dx_SS=0.; no self-shielding
...
SUBROUTINE get_dust_mass_and_depletion(depletion_factors,dx_loc,d_mass,g_mass,nH,nFe,nO,nN,nMg,nNe,nSi,nC,nS)
   ...
   ! C  abund = 245.9   mass frac = 0.33990096660329705
   ! O  abund = 133.2   mass frac = 0.24526381122680713
   ! Si abund = 33.3    mass frac = 0.10763461083244515
   ! mg abund = 33.3    mass frac = 0.09314625754508836
   ! fe abund = 33.3    mass frac = 0.21405435379236232
   C_mfrac  = 0.33990096660329705d0
   O_mfrac  = 0.24526381122680713d0
   Si_mfrac = 0.10763461083244515d0
   Mg_mfrac = 0.09314625754508836d0
   Fe_mfrac = 0.21405435379236232d0

   ! Initialize all depletion factors to 1
   depletion_factors = 1.d0 

   ! For solar metallicity, the depletion factors are:
   ! Fe: 0.016  O: 0.746  Mg: 0.218  Si: 0.040  C: 0.146
   ! These are in relatively good agreement with the default values in CLOUDY
   ! Fe: 0.01  O: 0.60  Mg: 0.20  Si: 0.03  C: 0.4
   ! But the carbon is perhaps a bit low
   ! Values from jenkins 2009 (for reference Table 4 column 7
   ! Fe: 0.0058, O: 0.5808, Mg: 0.0541, Si: 0.0437, C: 0.612
   ! Values from dopita 2000 (for reference)
   ! Fe: 0.01  O: 0.60  Mg: 0.20  Si: 0.10  C: 0.50 

   ! Iron
   depletion_factors(1) = 1.d0 - MIN( (d_mass * Fe_mfrac)  / (nFe * amu_to_g * (dx_loc**3) * mFe_NIST_amu ), 1.d0 )
   ! Oxygen
   depletion_factors(2) = 1.d0 - MIN( (d_mass * O_mfrac )  / (nO  * amu_to_g * (dx_loc**3) * mO_NIST_amu  ), 1.d0 )
   ! Magnesium
   depletion_factors(4) = 1.d0 - MIN( (d_mass * Mg_mfrac)  / (nMg * amu_to_g * (dx_loc**3) * mMg_NIST_amu ), 1.d0 )
   ! Silicon
   depletion_factors(6) = 1.d0 - MIN( (d_mass * Si_mfrac)  / (nSi * amu_to_g * (dx_loc**3) * mSi_NIST_amu ), 1.d0 )
   ! Carbon
   depletion_factors(8) = 1.d0 - MIN( (d_mass * C_mfrac )  / (nC  * amu_to_g * (dx_loc**3) * mC_NIST_amu  ), 1.d0 )
