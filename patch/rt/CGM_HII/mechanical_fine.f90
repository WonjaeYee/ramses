                  ...
                  if (ok) then
                     ...
                     if (mechanical_bpass) then
                        ...
                        if (age2 .lt. t_delay*1d-3) then  ! if younger than t_delay
                           ...
                           if (use_particle_type) ityp(ipart) = abs(ityp(ipart))
                        end if
               ...
               if (ok) then
                  ...
                  if (son(ind_cell) == 0) then  ! leaf cell
                     ...
                     if (metal) then
                        ...
                        Zejecta(1) = nsnII_star*SNII_Fe_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari)/scale_msun !1=Fe metal enrichment
                        Zejecta(2) = nsnII_star*SNII_O_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari)/scale_msun  !2=O  metal enrichment
                        Zejecta(3) = nsnII_star*SNII_N_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari)/scale_msun  !3=N  metal enrichment
                        Zejecta(4) = nsnII_star*SNII_Mg_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari)/scale_msun !4=Mg metal enrichment
                        Zejecta(5) = nsnII_star*SNII_Ne_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari)/scale_msun !5=Ne metal enrichment
                        Zejecta(6) = nsnII_star*SNII_Si_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari)/scale_msun !6=Si metal enrichment
                        Zejecta(7) = nsnII_star*SNII_Ca_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari)/scale_msun !7=Ca metal enrichment
                        Zejecta(8) = nsnII_star*SNII_C_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari)/scale_msun  !8=C  metal enrichment
                        Zejecta(9) = nsnII_star*SNII_S_yield(min(meanmassM, SNyieldmcap), mett, yields_portinari)/scale_msun  !9=S  metal enrichment
                     end if
...
subroutine mech_fine(ind_grid, ind_pos_cell, np, ilevel, dteff, nSN, mSN, pSN, mZSN, nphSN, mchSN)
   ...
   do i = 1, np
      ...
      if (log_mfb) then
398      format('MFB = ', f7.3, 1x, f7.3, 1x, f5.1, 1x, f5.3, 1x, f9.5, 1x, f7.3, 1x, f7.3)
         write (*, 398) log10(d*scale_nH), log10(Tk), num_sn, floadSN(i), 1./aexp - 1, log10(dx_loc*scale_l/pc2cm), log10(z/0.02)
      end if
