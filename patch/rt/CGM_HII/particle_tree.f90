...
subroutine virtual_tree_fine(ilevel)
  ...
  particle_data_width_int = 3
  if(use_particle_type) particle_data_width_int = particle_data_width_int + 1
...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  do i=1,np
     ...
     if(use_particle_type) reception(icpu,ilevel)%fp(ind_com(i),4)=ityp(ind_part(i))
  end do
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  do i=1,np
     ...
     if(use_particle_type) ityp(ind_part(i))=emission(icpu,ilevel)%fp(ind_com(i),4)
  end do
