subroutine init_part
  ...
  scale_m    = scale_l**3d0*scale_d
  scale_pc   = scale_l*boxlen/3.08d18
  ...
  if(star.or.sink)then
     ...
     tp=0.0
     if(metal)then
        allocate(zp(npartmax,nmetals)) ! metal enrichment
        zp=0.0
     end if
        ...
        do while (.not.eof)
           ...
           if(myid==1)then
              ...
              do i=1,nvector
                 ...
                 xx(i,1)=xx1*boxlen+boxlen/2.0/2**nlevelmax
                 xx(i,2)=xx2*boxlen+boxlen/2.0/2**nlevelmax
                 xx(i,3)=xx3*boxlen+boxlen/2.0/2**nlevelmax
                 vv(i,1)=vv1
                 vv(i,2)=vv2
                 vv(i,3)=vv3
                 !mm(i  )=mm1
                 mm(i  )=mm1/(scale_m/1.989d33)
                 tt(i  )=tt1
                 ii(i  )=indglob
              end do
100           continue
           ...
           do i=1,jpart
              ...
              if(cc(i)==myid)then
                 ...
                 tp(ipart)    =tt(i)
                 if(metal) then
                     zp(ipart,1) = z_ave*table_Solar_Fe ! Iron
                     zp(ipart,2) = z_ave*table_Solar_O  ! Oxygen
                     zp(ipart,3) = z_ave*table_Solar_N  ! Nitrogen
                     zp(ipart,4) = z_ave*table_Solar_Mg ! Magnesium
                     zp(ipart,5) = z_ave*table_Solar_Ne ! Neon
                     zp(ipart,6) = z_ave*table_Solar_Si ! Silicon
                     zp(ipart,7) = z_ave*table_Solar_Ca ! Calcium
                     zp(ipart,8) = z_ave*table_Solar_C  ! Carbon
                     zp(ipart,9) = z_ave*table_Solar_S  ! Sulfur
                 endif
                 if(use_initial_mass) mp0(ipart) = mm(i)
                 !if(use_particle_type) ityp(ipart) = 0 ! assume that it is a dark matter
                 if(use_particle_type) ityp(ipart) = -2 ! assume that it is a star particle
#ifndef WITHOUTMPI
              endif
#endif
           enddo
