...
!-------------------------------------------------------------------------
  use amr_commons
  use hydro_commons
  use rt_parameters,only:nIons,iIons,isH2,isHe,ixHI,ixHII,ixHeII,ixHeIII,isH2Katz
  use cooling_module,only:Y
  implicit none
  integer::ngrid
  integer,dimension(1:nvector)::ind_grid
  integer::i, ind, iskip, idim, nleaf, irad
  real(dp)::scale_nH, scale_T2, scale_l, scale_d, scale_t, scale_v
  integer,dimension(1:nvector),save::ind_cell, ind_leaf
  real(dp)::nH, T2, ekk, x, mu, err, Zsolar
  ...
  do ind=1,twotondim
     ...
     do i=1,nleaf
        ...
        if(metal) then
           ...
           Zsolar=(2.09d0*uold(ind_leaf(i),imetal+1)+1.06d0*uold(ind_leaf(i),imetal))/nH/0.02 ! metal enrichment
        endif
        ! Compute pressure from energy density
        T2 = uold(ind_leaf(i),ndim+2)          ! Energy density (kin+heat)
        ekk = 0.0d0                            !            Kinetic energy
        err = 0.0d0                            !            Non-thermal energy
        do idim=1,ndim
...
!-------------------------------------------------------------------------
  ...
  Zsolar = z_ave
  if(metal) then
   !Zsolar=vars(imetal)/nH/0.02
   Zsolar=(2.09d0*vars(imetal+1)+1.06d0*vars(imetal))/nH/0.02 ! metal enrichment
  endif

  ! Compute pressure from energy density
  T2 = vars(ndim+2)                         ! Energy dens. (kin+heat) [UU]
  ekk = 0.0d0                               !          Kinetic energy [UU]
  err = 0.0d0                               !      Non-thermal energy [UU]
  do idim=1,ndim
  ...
  if(haardt_madau) phI_rates = phI_rates + UVrates(1:nIons,1) * ss_factor
