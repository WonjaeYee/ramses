...
!            end if
      ...
      if (rt_advect) then
         do igroup=1,nGroups
            if (group_egy(igroup).gt.5.6d0 .and. group_egy(igroup).lt.13.6d0) then 
               advected_G0 = advected_G0 + (dNp(igroup) * rt_c_cgs(ilevel) * group_egy(igroup) * eV2erg / (1.6d-3))
            end if
         end do  
      end if
#endif
...
#ifdef RT
#ifdef CALIMA
      ! In the case of using CALIMA, dust optical properties are
      ! computed based on the local dust and PAH properties
      if (rt_advect) then
         ...
         call compute_dust_rad_rates(dust_helper,advected_G0+UV_background_G0,Tk,ne,&
      ...
      if(rt_advect) then
         ...
         do igroup=1,nGroups     ! ----------------- Do the update of N and F
            dNp(igroup)= MAX(smallNp,                                      &
                           (ddt(icell)*(recRad(igroup)+dNpdt(igroup,icell)) &
                                       +dNp(igroup))                        &
                           / (1d0+ddt(icell)*(phAbs(igroup)+dustAbs(igroup))))
         ...
         do igroup=1,nGroups ! -------Momentum transfer from photons to gas:
            mom_fact = ddt(icell) * (phAbs(igroup) + phSc(igroup) + dustRp(igroup)) &
      ...
      call compute_dust_precool(dust_helper, advected_G0+UV_background_G0, TK, ne, nElement_dep, &
      ...
      if (isH2_rtz) then
         ...
         if (H2ondust) then
            alpha_H2_loc = alpha_H2(TK, 0d0, xe, H2_cosmic_ray_ionization_rate, &
                           advected_G0*f_shd + UV_background_G0, dXion(1,1), dXion(1,2), nElement_dep(1)) 
...
SUBROUTINE rtz_updateRTGroups_CoolConstants(ilevel)
   ...
   !------------------------------------------------------------------------
   use rt_parameters
   use rtz_cooling_module, only: signc_dust
#ifdef CALIMA
