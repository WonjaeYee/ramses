...
module rtz_cooling_module
  ...
  ! real(dp),parameter::x_min=1d-20, x_fm=1d0, x_frac=1.0 ! temporarily increase the limit
  ...
  if (rtz_equilibrium_test.gt.0) then
      ...
      do i_interp = 1,300
         ...
         do i=1,ncell
            ...
            xion(1:n_elements,1:n_elements,i) = MIN(MAX(xion(1:n_elements,1:n_elements,i), 0d0),1d0)

            ! Loop over each element and ensure that the ionization fractions
            ! sum to 1
            ! xion(1,1, i) = 1.d0 - xion(1,2, i)
            ! do iElement=1,n_elements
            ...
            !          xion(iElement,iIon,i) = xion(iElement,iIon,i) / current_mass_frac
            !       end do
            !    end if
            ! end do
         end do
