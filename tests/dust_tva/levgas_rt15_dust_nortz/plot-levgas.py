...
# This folder can run EITHER opacity treatment, and its namelist always carries
# kappaSc/kappaAbs because the rt_kIR_RT15=.true. mode needs them -- so their
# mere presence no longer distinguishes the two. Key off the switch itself, and
# fall back to the old test when the switch is absent (folders levgas_rt15 and
# levgas_rt15_dust, where presence of kappaSc is still the right discriminator).
_kir = nml_str("rt_kir_rt15", "")
KT13 = (_kir.strip().lower().startswith(".t") if _kir else (KAPPA_SC is not None))
