...
subroutine grow_sink(ilevel,on_creation)
  ...
  real(dp):: ms_lifetime, star_age_myr, star_met, star_met_fe
  logical:: is_hn, is_sn
     ...
     if (msink(isink)>0.)then ! if msink=0 then there was no accretion and thus no shift
        ...
        if (evolution_flag(isink).eq.0) then
           ...
           ! Pop II case
              ...
              if (star_age_Myr.gt.ms_lifetime) then
                 if (myid.eq.1) then
                  write(*,*)'star_age_Myr:',star_age_Myr,'ms_lifetime:',ms_lifetime
              ...
              ! <<WJ>> In case of main-sequence, get properties from MIST
                 ...
                 ! loc_metal_yield(10) = -mist_prop(15) ! these do not need to be divided?
                 ! is `loc_metal_yield` only for the mass loss, not injecting to the gas cell??
        ...
        ! ?? star_met could have been changed?
              ...
              if (evolution_flag(isink).eq.0) then
                 ...
                 if (star_met.lt.z_crit_pop3) then
                    ...
                    if (star_age_Myr.gt.ms_lifetime) then
                       ...
                       ! Is this a hypernova
                       ! ?? inconsistent with the method in `grow_sink`?
                       is_hn = .false.
